  ...
  try {
    ...
    for (const block of blocks.results) {
      if (block.type === 'heading_2' && block.heading_2.rich_text.length > 0) {
        ...
        if (
          text.includes('PRIORITY 1') ||
          text.includes('PRIORITY 2') ||
          text.includes('PRIORITY 3')
        ) {
    ...
    if (masterDatabase) {
      ...
      console.log(
        'You can now view them at: https://www.notion.so/' + masterDatabase.replace(/-/g, '')
      );
    } else {
