    ...
    try {
      ...
      for (const item of response.results) {
        if (item.object === 'page') {
          ...
          } else if (titleLower.includes('template') || titleLower.includes('daily focus') || titleLower.includes('development session')) {
          ...
          } else if (titleLower.includes('playlist pulse') || titleLower.includes('playlist pulse')) {
            pageCategories['Playlist Pulse'].push(pageInfo);
          } else if (titleLower.includes('business') || titleLower.includes('strategy') || titleLower.includes('market') || titleLower.includes('revenue') || titleLower.includes('pricing')) {
            pageCategories['Business Strategy'].push(pageInfo);
          } else if (titleLower.includes('pricing') || titleLower.includes('£') || titleLower.includes('pricing')) {
            pageCategories['Pricing'].push(pageInfo);
          } else if (titleLower.includes('development') || titleLower.includes('technical') || titleLower.includes('claude') || titleLower.includes('cursor') || titleLower.includes('github')) {
            pageCategories['Development'].push(pageInfo);
          } else if (titleLower.includes('customer') || titleLower.includes('feedback') || titleLower.includes('beta')) {
            pageCategories['Customer Feedback'].push(pageInfo);
          } else if (titleLower.includes('partnership') || titleLower.includes('liberty') || titleLower.includes('demarco')) {
            pageCategories['Partnerships'].push(pageInfo);
          } else if (titleLower.includes('content') || titleLower.includes('marketing') || titleLower.includes('newsletter') || titleLower.includes('social')) {
            pageCategories['Content & Marketing'].push(pageInfo);
          } else if (titleLower.includes('technical') || titleLower.includes('api') || titleLower.includes('integration')) {
  ...
  async executeConsolidation() {
    try {
      ...
      for (const page of emptyPages.slice(0, limit)) {
        try {
          if (dryRun) {
            console.log(`🔎 [DRY-RUN] Would archive: ${page.properties?.title?.title?.[0]?.text?.content || 'Untitled'}`);
          } else {
            ...
            archivedCount++;
            console.log(`✅ Archived: ${page.properties?.title?.title?.[0]?.text?.content || 'Untitled'}`);
          }
        } catch (error) {
      ...
      if (radioPromoPages.length > 1) {
        ...
        for (const page of archivePages) {
          try {
            if (dryRun) {
              console.log(`🔎 [DRY-RUN] Would archive duplicate: ${page.properties?.title?.title?.[0]?.text?.content}`);
            } else {
              await notion.pages.update({
                page_id: page.id,
                archived: true
    ...
    try {
      ...
      if (tidySidebar) {
        ...
        for (const pageInfo of structurePages) {
          try {
            if (dryRun) {
              console.log(`🔎 [DRY-RUN] Would create: ${pageInfo.title} under parent ${workspaceId}`);
            } else {
