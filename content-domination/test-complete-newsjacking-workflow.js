...
const NEWSJACKING_TEST_CASES = [
  {
    ...
    test: async (mockData) => {
      ...
      if (detectionTime > 5000) { // 5 seconds for mock processing
...

  {
    ...
    test: async (mockData) => {
      ...
      console.log(`✅ Scored ${scoringResults.length} news items, ${highRelevanceCount} high-relevance`);
      ...
      return {
        ...
        highRelevanceCount,
        accurateScoring,
        averageScore: scoringResults.reduce((sum, r) => sum + r.relevanceScore, 0) / scoringResults.length,
        scoringResults
      };
    }
  },

  {
    ...
    test: async (mockData) => {
      ...
      const avgVoiceConsistency = fusionResults.reduce((sum, r) => sum + r.voiceConsistency, 0) / fusionResults.length;
      ...
      console.log(`✅ Generated ${totalContentPieces} content pieces with ${avgVoiceConsistency.toFixed(1)}/10 voice consistency`);
      
      return {
        success: true,
        totalContentPieces,
        averageVoiceConsistency: avgVoiceConsistency,
        averageExpertiseDepth: fusionResults.reduce((sum, r) => sum + r.expertiseDepth, 0) / fusionResults.length,
...

  {
    ...
    test: async (mockData) => {
      ...
      for (const news of MOCK_BREAKING_NEWS) {
        ...
        const withinTimeLimit = totalPipelineTime * 1000 <= TEST_CONFIG.performance.maxContentGenerationTime;
      ...
      const averagePipelineTime = pipelineResults.reduce((sum, r) => sum + r.pipelineTime, 0) / pipelineResults.length;
      
      if (withinTimeLimitCount < pipelineResults.length) {
        console.warn(`⚠️ ${pipelineResults.length - withinTimeLimitCount} pipelines exceeded 30-minute limit`);
      }
      
      console.log(`✅ Average pipeline time: ${averagePipelineTime.toFixed(1)}s (${withinTimeLimitCount}/${pipelineResults.length} within limit)`);
...

  {
    ...
    test: async (mockData) => {
      ...
      const highPriorityOpportunities = firstMoverResults.filter(r => r.priority === 'critical').length;
      
      console.log(`✅ Found ${firstMoverOpportunities} first-mover opportunities, ${highPriorityOpportunities} critical`);
      
      return {
        success: true,
        firstMoverOpportunities,
        highPriorityOpportunities,
        averageResponseWindow: firstMoverResults.reduce((sum, r) => sum + r.responseWindow, 0) / firstMoverResults.length,
        firstMoverResults
      };
    }
  },

  {
    ...
    test: async (mockData) => {
      ...
      const averageStrength = integrationResults.reduce((sum, r) => sum + r.positioningStrength, 0) / integrationResults.length;
      
      console.log(`✅ ${strongPositioning} strong positioning opportunities, avg strength: ${averageStrength.toFixed(2)}`);
...

  {
    ...
    test: async (mockData) => {
      ...
      const avgPlatformsPerNews = platformResults.reduce((sum, r) => sum + r.totalPlatforms, 0) / platformResults.length;
      const perfectMatches = platformResults.filter(r => r.platformMatch).length;
      
      console.log(`✅ Average ${avgPlatformsPerNews.toFixed(1)} platforms per news item, ${perfectMatches} perfect matches`);
    ...
    test: async (mockData) => {
      ...
      for (const news of MOCK_BREAKING_NEWS) {
        ...
        voiceResults.push({
          ...
          passesThreshold: voiceConsistencyScore >= TEST_CONFIG.performance.minVoiceConsistencyScore
        });
        
        if (voiceConsistencyScore < 7.0) {
          console.log(`⚠️ Low voice consistency: ${news.title} (${voiceConsistencyScore.toFixed(1)})`);
        }
      }
      
      const avgVoiceScore = voiceResults.reduce((sum, r) => sum + r.voiceConsistencyScore, 0) / voiceResults.length;
      ...
      console.log(`✅ Average voice consistency: ${avgVoiceScore.toFixed(1)}/10 (${passThresholdCount}/${voiceResults.length} pass threshold)`);
      ...
      return {
        ...
        averageProfessionalTone: voiceResults.reduce((sum, r) => sum + r.professionalTone, 0) / voiceResults.length,
        voiceResults
      };
    }
  }
...
function generateMockContent(news, platform = 'general') {
  ...
  const randomCredibility = industryCredibility[Math.floor(Math.random() * industryCredibility.length)];
...
function countBritishPhrases(fusedContent) {
  const britishPhrases = ['right, so', 'here\'s the thing', 'to be fair', 'in my experience', 'let\'s be honest'];
  let count = 0;
...
function calculateCTARelevance(news) {
  const text = `${news.title} ${news.content}`.toLowerCase();
  if (text.includes('automation') || text.includes('playlist') || text.includes('contact')) return 0.9;
  return 0.6;
}
...
function countPersonalAnecdotes(content) {
  const anecdoteMarkers = ['in my experience', 'i remember', 'last month', 'i helped', 'i worked with'];
  let count = 0;
...
async function runCompleteNewsjackingTests() {
  ...
  results.systemReadiness = results.failed === 0 ? 'production-ready' : 
                           results.failed <= 2 ? 'needs-minor-fixes' : 'needs-major-work';
  ...
  if (detectionTest && detectionTest.details) {
    console.log(`📡 News Detection: ${detectionTest.details.detectedCount} items in ${detectionTest.details.detectionTime}ms`);
  }
  
  const timingTest = results.testResults.find(r => r.name.includes('Rapid Response Timing'));
  if (timingTest && timingTest.details) {
    console.log(`⚡ Average Pipeline: ${timingTest.details.averagePipelineTime.toFixed(1)}s (target: <30min)`);
    console.log(`🎯 Within Time Limit: ${timingTest.details.withinTimeLimitCount}/${timingTest.details.totalPipelines}`);
  }
