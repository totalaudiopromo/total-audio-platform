...
class NewsMonitoringSetup {
  ...
  initializeTwitterMonitoring() {
    ...
    this.twitterKeywords = [
      '#musicbusiness', '#musicindustry', '#spotify', '#applemusic',
      '#musicmarketing', '#musicpromo', '#playlistpitching', '#musictech',
      '#artistdevelopment', '#musicstreaming', '#recordlabel', '#musicdistribution'
    ];
  ...
  async scanRSSFeeds() {
    ...
    for (const feed of this.rssFeeds) {
      try {
        ...
        if (recentItems.length > 0) {
          log.monitoring(`${feed.name}: ${recentItems.length} new items, ${opportunitiesDetected} opportunities`);
        }
    ...
    if (opportunitiesDetected > 0) {
      log.newsjack(`🔥 SCAN COMPLETE: ${opportunitiesDetected} new opportunities detected in ${scanTime}ms`);
    }
  }
  ...
  async scanTwitterTrends() {
    ...
    try {
      ...
      const trendsResponse = await axios.get('https://api.twitter.com/1.1/trends/place.json?id=23424975', { // UK trends
        headers: {
      ...
      const musicTrends = trends.filter(trend => 
        this.isMusicRelated(trend.name) || trend.tweet_volume > 10000
      );

      for (const trend of musicTrends.slice(0, 5)) { // Process top 5 relevant trends
  ...
  calculateRelevanceScore(title, content) {
    ...
    const highValueKeywords = [
      'spotify', 'apple music', 'playlist', 'streaming', 'music marketing',
      'artist development', 'record label', 'music business', 'promotion',
      'automation', 'ai', 'algorithm', 'discovery', 'playlist pitching'
    ];

    // Medium-value keywords (0.2 each)
    const mediumValueKeywords = [
      'music industry', 'musicians', 'artists', 'distribution', 'royalties',
      'sync licensing', 'music tech', 'social media', 'fan engagement',
      'music video', 'tour', 'concert', 'festival'
    ];
  ...
  async handleNewsjackingOpportunity(opportunity) {
    log.newsjack(`NEW OPPORTUNITY: ${opportunity.title} (Score: ${opportunity.relevanceScore.toFixed(2)})`);
      ...
      case 'slack':
        await axios.post(process.env.SLACK_WEBHOOK_URL, {
          ...
          attachments: [{
            color: 'danger',
            fields: [
              { title: 'Relevance Score', value: opportunity.relevanceScore.toFixed(2), short: true },
...
`;
  ...
  isMusicRelated(text) {
    const musicKeywords = [
      'music', 'spotify', 'apple music', 'playlist', 'artist', 'song', 'album',
      'streaming', 'record', 'label', 'tour', 'concert', 'festival', 'musician'
    ];
