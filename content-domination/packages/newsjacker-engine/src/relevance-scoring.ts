...
class RelevanceScoringEngine {
    ...
    this.scoringCriteria = {
      automationKeywords: {
        core: [
          ...
          { keyword: 'playlist pitching', weight: 0.3, context: ['curator', 'submission', 'spotify'] },
          { keyword: 'contact research', weight: 0.25, context: ['outreach', 'database', 'CRM'] },
          { keyword: 'data analytics', weight: 0.2, context: ['streaming', 'performance', 'insights'] },
          ...
          { keyword: 'campaign management', weight: 0.2, context: ['promotion', 'tracking', 'ROI'] }
        ],
        secondary: [
      ...
      industryKeywords: {
        ...
        marketingConcepts: [
          { keyword: 'discovery', weight: 0.2, application: ['playlist', 'algorithm', 'recommendation'] },
          ...
          { keyword: 'targeting', weight: 0.15, application: ['audience', 'advertising', 'campaigns'] },
          { keyword: 'attribution', weight: 0.1, application: ['analytics', 'tracking', 'ROI'] }
        ]
      },

      urgencyIndicators: [
        { pattern: '(breaking|announced today|just launched)', urgencyLevel: 'immediate', timeWindow: 120, multiplier: 1.5 },
        { pattern: '(new feature|beta|rolling out)', urgencyLevel: 'same-day', timeWindow: 480, multiplier: 1.3 },
        { pattern: '(coming soon|planned|will launch)', urgencyLevel: 'weekly', timeWindow: 2880, multiplier: 1.1 },
        { pattern: '(partnership|acquisition|merger)', urgencyLevel: 'same-day', timeWindow: 720, multiplier: 1.4 },
        { pattern: '(policy change|update|algorithm)', urgencyLevel: 'immediate', timeWindow: 180, multiplier: 1.6 },
        { pattern: '(study shows|research reveals|data suggests)', urgencyLevel: 'weekly', timeWindow: 1440, multiplier: 1.2 }
      ],
      ...
      audienceRelevance: {
        primaryAudience: ['independent artists', 'indie musicians', 'self-releasing artists', 'DIY artists'],
        secondaryAudience: ['music marketers', 'PR agencies', 'playlist curators', 'music tech companies'],
        painPoints: ['playlist pitching', 'contact research', 'campaign tracking', 'data analysis', 'time management', 'scaling promotion'],
        interests: ['automation', 'efficiency', 'growth', 'discovery', 'streaming success', 'fan engagement']
      }
    };
  }
  ...
  private generateContentAngles(newsItem: NewsItem, text: string): string[] {
    ...
    angles.push(`Why ${title.split(' ').slice(0, 4).join(' ')} changes everything for music marketing`);
   ...
   * Public API methods
   */
  public async batchScoreNews(newsItems: NewsItem[]): Promise<Array<NewsItem & { scoringResult: ScoringResult }>> {
    const results = [];
