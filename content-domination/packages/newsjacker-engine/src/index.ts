...
import TrendDetectionEngine, { NewsItem, TrendAlert, NewsSource, AutomationOpportunity } from './trend-detection';
...
class NewsjackerEngine {
  ...
  async processNewsOpportunity(newsItem: NewsItem): Promise<NewsjackingOpportunity> {
    ...
    try {
      ...
      if (scoringResult.totalScore < this.config.relevanceScoring.minimumScore) {
        console.log(`⏭️ Skipping low-relevance news (score: ${scoringResult.totalScore.toFixed(2)})`);
        return null;
      }
      ...
      console.log(`🎯 Score: ${scoringResult.totalScore.toFixed(2)}, Content pieces: ${fusedContent.length}, Alert: ${alertLevel}`);
  ...
  async executeOpportunity(opportunityId: string): Promise<boolean> {
    ...
    if (opportunity.status !== 'ready') {
      console.log(`⚠️ Opportunity ${opportunityId} not ready for execution (status: ${opportunity.status})`);
      return false;
    }
  ...
  private startPipelineProcessing(): void {
    ...
    setInterval(async () => {
      ...
      try {
        ...
        const newTrends = activeTrends.filter(trend => 
          !this.pipeline.opportunities.has(`newsjack_${trend.id}_*`)
        );
  ...
  private async createNewsjackingAlert(
    ...
    const alert: NewsjackingAlert = {
      ...
      responseDeadline: new Date(Date.now() + opportunity.competitiveContext.responseWindow * 60 * 1000),
   ...
   * Utility methods
   */
  private determineAlertLevel(scoringResult: ScoringResult): 'critical' | 'high' | 'medium' | 'low' {
    if (scoringResult.totalScore > 0.8 && scoringResult.breakdown.urgencyBoost > 0.7) return 'critical';
    if (scoringResult.totalScore > 0.7 && scoringResult.breakdown.competitiveAdvantage > 0.6) return 'high';
