...
{{url}}`,
        variables: ['industry_context', 'headline', 'automation_opportunity', 'audio_intel_angle', 'url'],
...
Source: {{url}}`,
        variables: ['headline', 'industry_context', 'key_points', 'automation_opportunity', 'audio_intel_angle', 'url'],
...
Source: {{url}}`,
        variables: ['headline', 'automation_opportunity', 'audio_intel_angle', 'call_to_action', 'url'],
...
*Spotted a manual process that could be automated? Reply and let me know - I might build it next.*`,
        variables: ['headline', 'industry_context', 'key_points', 'automation_opportunity', 'audio_intel_angle', 'call_to_action', 'url'],
  ...
  private generateCallToAction(automationAngle: string): string {
    const ctas = {
      ...
      analytics_automation: 'Tired of manual analytics? Audio Intel tracks everything automatically.',
  ...
  private updateTemplateUsage(templateId: string, success: boolean): void {
    ...
    if (template) {
      ...
      template.successRate = (oldSuccessRate * (template.usageCount - 1) + (success ? 1 : 0)) / template.usageCount;
   ...
   */
  getTemplateStats(): Array<{templateId: string; name: string; usageCount: number; successRate: number}> {
