...
import { generatePersonalizedPrompt, getPersonalityGreeting, getTemperatureAdjustment } from './agentPersonalities';
...
class CustomAgentStore {
  ...
  // Team management
  async createTeam(userId: string, team: Omit<AgentTeamConfig, 'id' | 'userId' | 'createdAt'>): Promise<AgentTeamConfig> {
...
export class CustomAgentService {
  ...
  async updateAgent(agentId: string, userId: string, updates: Partial<CustomAgent>): Promise<CustomAgent> {
  ...
  async getAgentAnalytics(agentId: string, userId: string): Promise<{
    ...
    return {
      ...
      performanceMetrics: {
        ...
        userSatisfaction: ratings.length > 0 ? ratings.reduce((a, b) => a + b, 0) / ratings.length / 5 : 0
      }
    };
  }
  ...
  private getBasePromptForRole(role: string): string {
    const prompts: Record<string, string> = {
      'content-generation-agent': 'Create professional music industry content including press releases, social media posts, and marketing materials.',
      'analytics-agent': 'Analyze music industry data and provide actionable insights for performance optimization.',
      'intel-research-agent': 'Research industry contacts, competitive intelligence, and market opportunities.',
      'music-industry-strategist': 'Provide strategic guidance for music career development and industry positioning.',
      ...
      'music-marketing-mastermind': 'Develop comprehensive marketing strategies for music promotion.',
  ...
  private async callAIService(
    ...
    const adjustedTemperature = Math.max(0.1, Math.min(1.0, baseTemperature + temperatureAdjustment));
