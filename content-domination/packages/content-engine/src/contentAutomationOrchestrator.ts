...
import ContentSchedulingOrchestrator, { ScheduledContent, ContentBatch } from './contentSchedulingOrchestrator';
...
class ContentAutomationOrchestrator {
  ...
  async processNewsletter(newsletterId?: string): Promise<ContentGenerationResult> {
    try {
      ...
      console.log(`🔍 Auto-approved: ${approvedContent.length}, Requires review: ${requiresApproval.length}`);
      ...
      if (approvedContent.length > 0 && this.config.automationSettings.autoSchedulingEnabled) {
        ...
        console.log(`📅 Scheduled content batch ${batchId} for completion by ${estimatedCompletionTime.toLocaleString()}`);
      }
  ...
  private async checkForNewNewsletters(): Promise<void> {
    try {
      ...
      const newsletterSequences = sequences.sequences.filter(seq => 
  ...
  private async shouldAutoApprove(content: PlatformContent): Promise<boolean> {
    ...
    if (this.config.automationSettings.requireApprovalForAudioIntelMentions &&
   ...
   */
  private convertToScheduledContent(content: PlatformContent, newsletterId?: string): ScheduledContent {
    return {
  ...
  private async startPerformanceTracking(content: PlatformContent[]): Promise<void> {
    ...
    content.forEach(piece => {
      ...
      setTimeout(async () => {
        try {
          await this.performanceAnalytics.collectPerformanceData(piece.id, piece.platform, '1_hour');
        } catch (error) {
      ...
      setTimeout(async () => {
        try {
          await this.performanceAnalytics.collectPerformanceData(piece.id, piece.platform, '24_hours');
        } catch (error) {
      ...
      setTimeout(async () => {
        try {
          await this.performanceAnalytics.collectPerformanceData(piece.id, piece.platform, '7_days');
        } catch (error) {
  ...
  private async generateDailyPerformanceReport(): Promise<PerformanceReport> {
    ...
    console.log(`📈 Summary: ${report.summary.totalContent} content, ${report.summary.averageEngagementRate.toFixed(1)}% avg engagement`);
  ...
  public async approveContent(contentId: string): Promise<boolean> {
    try {
      ...
      if (content) {
        ...
        await this.schedulingOrchestrator.createContentBatchSchedule([scheduledContent], 'manual-approval');
        return true;
      }
      return false;
    } catch (error) {
  ...
  public async testWorkflow(): Promise<{
    ...
    try {
      ...
      const voiceConsistencyScore = result.generatedContent.reduce(
        (avg, content) => avg + (content.voiceAuthenticityScore || 8.0), 0
