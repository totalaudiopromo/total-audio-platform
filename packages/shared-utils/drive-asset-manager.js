...
class DriveAssetManager {
  ...
  async processTypeformAssets(typeformData, folderIds) {
    ...
    if (assets.pressRelease) {
      await this.downloadAndUpload(assets.pressRelease.url, 'press-release.pdf', folderIds.pressRelease);
    }
  ...
  extractAssetsFromTypeform(typeformData) {
    return {
      audioFiles: typeformData.answers.find(a => a.field.type === 'file_upload' && 
        a.field.title.toLowerCase().includes('audio'))?.file_urls || [],
      pressRelease: typeformData.answers.find(a => a.field.type === 'file_upload' && 
        a.field.title.toLowerCase().includes('press'))?.file_url,
      promoPhotos: typeformData.answers.find(a => a.field.type === 'file_upload' && 
        a.field.title.toLowerCase().includes('photo'))?.file_urls || [],
      artwork: typeformData.answers.find(a => a.field.type === 'file_upload' && 
        a.field.title.toLowerCase().includes('artwork'))?.file_urls || [],
      soundcloudUrl: typeformData.answers.find(a => a.field.title.toLowerCase().includes('soundcloud'))?.url
    };
  }
