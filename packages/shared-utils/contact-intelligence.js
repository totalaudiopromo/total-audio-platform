...
class ContactIntelligenceEngine {
  ...
  async selectContactsForCampaign(campaignData) {
    ...
    const selectedContacts = this.selectOptimalContactCount(prioritizedContacts, budget, campaignType);
  ...
  matchesGenre(contact, campaignGenre) {
    ...
    return contact.preferredGenres.some(genre => 
      acceptedGenres.includes(genre.toLowerCase())
    );
  }
  ...
  explainSelection(selectedContacts, campaignData) {
    const reasoning = {
      ...
      avgResponseRate: (selectedContacts.reduce((sum, c) => sum + c.responseRate, 0) / selectedContacts.length).toFixed(1) + '%',
