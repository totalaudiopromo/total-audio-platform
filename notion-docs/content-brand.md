...
.texture-pulse {
  ...
  **Layer Structure:**
  ...
  1. **Colour Overlay**: Tool-specific colour at 10-20% opacity
  1. **Gradient Accent**: Directional gradient for depth
  1. **Content Layer**: Clean typography and UI elements
...
.intel-active {
  ...
  **Pricing Page:**
  ...
  1. Implement new colour variables in CSS
  1. Test DRS texture integration on key pages
  1. Create dark mode toggle functionality
  ### Development Priorities (Next Sprint)
  1. DRS texture asset optimization
  1. Dark mode implementation across all components
  1. New competitor comparison pages
  1. Freemium tier integration
  ### Testing Strategy
  1. A/B test new positioning against current messaging
  1. User preference testing: textured vs flat backgrounds
  1. Dark mode usage analytics
  1. Conversion rate impact of realistic competitor positioning
  ---
...
<!-- unhandled child_page -->
  ...
  1. Create new integration called "Total Audio Social Automation"
  1. Copy the "Internal Integration Token" (starts with `secret_`)
  1. **Capabilities needed**: Read content, Update content, Insert content
  ### **Step 2: Share Database with Integration**
  1. Go to your Social Media Content database: [Social Media Content - User Acquisition](https://www.notion.so/75c7e9291f49412ca546b4e880291455)
  1. Click "Share" → "Add people, emails, groups, or integrations"
  1. Add your "Total Audio Social Automation" integration
  1. Give it "Full access"
  ...
  try {
    ...
    if (response.results.length > 0) {
      ...
      console.log('Sample post:', {
        title: post.properties.Title?.title[0]?.text?.content || 'No title',
        platforms: post.properties.Platform?.multi_select?.map((p: any) => p.name) || [],
        content: post.properties.Content?.rich_text[0]?.text?.content?.substring(0, 100) + '...' || 'No content'
      });
    }
...
<!-- unhandled child_page -->
  ...
  1. Export to CSV when ready to post
  1. Cursor automation reads from CSV files
  1. Much simpler, more reliable, easier to debug
  ## **How It Works**
  ### **Step 1: Export Your Ready Content**
  From your Social Media Content database:
  1. Create a view filtered by "Status = Ready to Post"
  1. Export as CSV
  1. Save to a folder like `/social-automation/content/ready-posts.csv`
  ...
  for (const post of posts) {
    if (shouldPostNow(post.scheduledTime)) {
      ...
      const finalContent = processedContent.replace(
        ...
        `intel.totalaudiopromo.com${post.utmTracking}`
      );
...
setInterval(runAutomation, 15 * 60 * 1000);
  ...
  1. **Simple Debugging**: CSV files are easy to read and modify
  1. **Version Control**: CSV files can be committed to git
  1. **Reliable**: No API token issues or integration failures
  1. **Fast Development**: Cursor can build this quickly
  1. **Easy Updates**: Just re-export CSV when content changes
  ...
  1. **Monday**: Export "Ready to Post" view as CSV
  1. **Automation**: Runs every 15 minutes, posts at scheduled times
  1. **Friday**: Check status logs, plan next week
  ## **File Structure**
  ```javascript
social-automation/
├── content/
