  ...
  if (status === 'loading') {
    return (
      <div className="mx-auto w-full max-w-3xl">
        <div className="glass-panel px-6 py-12 text-center text-white/70">Loading your workspace…</div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
      ...
      <section className="grid gap-6 md:grid-cols-2">
        <div className="glass-panel px-6 py-8">
          ...
          <dl className="mt-4 space-y-3 text-sm text-white/70">
            <div>
              <dt className="font-medium uppercase tracking-[0.35em] text-white/40">Name</dt>
              <dd className="mt-1 text-white">{session.user?.name ?? '—'}</dd>
            </div>
            <div>
              <dt className="font-medium uppercase tracking-[0.35em] text-white/40">Email</dt>
        ...
        <div className="glass-panel px-6 py-8">
          ...
          <ul className="mt-4 space-y-3 text-sm text-white/70">
            <li className="rounded-2xl bg-white/5 px-4 py-3">Replace this copy with your tool’s onboarding checklist.</li>
            <li className="rounded-2xl bg-white/5 px-4 py-3">Drop cards, charts, or tables here with your product data.</li>
            <li className="rounded-2xl bg-white/5 px-4 py-3">Hook into your API routes using the authenticated session.</li>
          </ul>
        </div>
      </section>
      ...
      <div className="glass-panel flex flex-col gap-4 px-6 py-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-semibold uppercase tracking-[0.35em] text-white/40">Session details</p>
          <p className="mt-1 text-xs text-white/60">Token strategy: JWT · Provider: {session.user?.name ? 'OAuth / credentials hybrid' : 'Credentials'}</p>
        </div>
