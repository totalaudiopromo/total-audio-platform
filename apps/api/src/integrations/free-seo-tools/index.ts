...
export class FreeSEOToolsService {
  ...
  async getTrendsData(keyword: string, timeframe: string = 'today 12-m'): Promise<{
    success: boolean;
  ...
  async getSearchConsoleData(domain: string, startDate: string, endDate: string): Promise<{
    success: boolean;
    ...
    try {
      ...
      const searchConsoleData: SearchConsoleData = {
        ...
        ctr: data.rows.reduce((sum: number, row: any) => sum + (row.ctr || 0), 0) / data.rows.length,
        averagePosition: data.rows.reduce((sum: number, row: any) => sum + (row.position || 0), 0) / data.rows.length,
  ...
  async researchKeywords(seedKeyword: string): Promise<{
    ...
    try {
      ...
      for (const keyword of allKeywords.slice(0, 20)) { // Limit to 20 keywords
  ...
  private async getRelatedTopics(keyword: string): Promise<string[]> {
    try {
      ...
      return parsedData.default?.rankedList?.[0]?.rankedKeyword?.map((item: any) => item.query) || [];
    } catch {
      return [];
    }
  }
  ...
  private determineTrend(data: any): 'rising' | 'falling' | 'stable' {
    ...
    const recentAvg = recent.reduce((acc: number, item: any) => acc + item.value[0], 0) / recent.length;
    const olderAvg = older.reduce((acc: number, item: any) => acc + item.value[0], 0) / older.length;
  ...
  private async analyzeSingleCompetitor(competitorDomain: string, originalDomain: string): Promise<CompetitorAnalysisData | null> {
    try {
      ...
      return {
        ...
        linkOpportunities: [`${originalDomain} vs ${competitorDomain}`, `Alternative to ${competitorDomain}`],
        socialPresence: {
