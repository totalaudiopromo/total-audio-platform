...
export function CampaignCard({ campaign }: { campaign: Campaign }) {
  return (
    <Link href={`/campaigns/${campaign.id}`}>
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border-2 border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-lg hover:border-blue-300 dark:hover:border-amber-700 transition-all group cursor-pointer">
          ...
          {Array.isArray(campaign.platforms) && campaign.platforms.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-3">
              {campaign.platforms.slice(0, 3).map((platform) => (
                <span
                  key={platform}
                  className="px-2 py-1 bg-blue-100 dark:bg-amber-900/30 text-amber-700 dark:text-blue-300 rounded-lg text-xs font-medium"
                >
                  {platform}
                </span>
              ))}
        ...
        <div className="pt-4 border-t border-slate-200 dark:border-slate-700">
          <div className="flex gap-3">
            <span className="text-amber-600 dark:text-blue-400 text-sm font-semibold group-hover:underline">
              View Details →
            </span>
          </div>
        </div>
