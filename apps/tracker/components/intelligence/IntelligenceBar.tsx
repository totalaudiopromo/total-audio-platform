...
export function IntelligenceBar({ patterns }: IntelligenceBarProps) {
  ...
  const getPatternColor = (type: string) => {
    switch (type) {
      ...
      case 'platform_effectiveness':
        return 'bg-blue-50 border-teal-500 text-teal-900';
      case 'budget_optimization':
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 via-amber-50 to-pink-50 rounded-2xl p-6 md:p-8 border-4 border-black shadow-brutal">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-12 h-12 bg-teal-600 rounded-xl flex items-center justify-center shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
          <svg
            className="w-6 h-6 text-white"
            fill="none"
            stroke="currentColor"
      ...
      {patterns.length === 0 && (
        <div className="text-center py-8">
          <p className="text-base font-bold text-gray-600">
            Complete more campaigns to unlock personalised insights and pattern
            recognition
          </p>
        </div>
      )}
