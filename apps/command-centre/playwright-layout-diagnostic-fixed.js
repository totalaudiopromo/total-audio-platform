...
async function measureLayoutElements(page) {
  try {
    ...
    const measurements = await page.evaluate(() => {
      ...
      results.elements.sidebar = sidebarFound || { found: false, testedSelectors: sidebarSelectors };
      ...
      results.elements.mainContent = mainContentFound || { found: false, testedSelectors: mainContentSelectors };

      // Measure navigation items
      const navItems = document.querySelectorAll('nav a, [role="navigation"] a, .nav-item, .sidebar a');
      if (navItems.length > 0) {
        results.elements.navigationItems = {
          found: true,
          count: navItems.length,
          items: Array.from(navItems).slice(0, 5).map((item, index) => {
      ...
      if (layoutContainer) {
        results.elements.layoutContainer = measureElement('body > div:first-child', 'layout-container');
      }
    ...
    for (const pageConfig of PAGES) {
      ...
      try {
        ...
        if (measurements.elements?.mainContent?.found) {
          console.log(`    📏 Main content width: ${measurements.elements.mainContent.dimensions.width}px`);
        }
...
function generateSummaryReport(results) {
  ...
  Object.entries(results.pages).forEach(([pageKey, pageData]) => {
    if (pageData.success && pageData.measurements?.layoutIssues) {
      allIssues.push(...pageData.measurements.layoutIssues.map(issue => ({
        ...issue,
  ...
  if (desktopMeasurements.length > 0) {
    ...
    desktopMeasurements.forEach(m => {
      ...
      const contentWidth = m.mainContent?.found ? `${m.mainContent.dimensions.width}px` : 'Not found';
      console.log(`    ${m.page.padEnd(20)} | Sidebar: ${sidebarWidth.padEnd(12)} | Content: ${contentWidth}`);
    });
  }
  ...
  Object.entries(sidebarWidths).forEach(([page, widths]) => {
    if (widths.desktop && widths.tablet && widths.mobile) {
      if (widths.desktop === widths.tablet && widths.tablet === widths.mobile) {
        responsiveIssues.push(`${page}: Sidebar width doesn't adapt to viewport (${widths.desktop}px on all sizes)`);
      }
    }
  });
...
${Object.entries(issuesByType).map(([type, issues]) => 
  `${type.replace('-', ' ').toUpperCase()}: ${issues.length} occurrences`
).join('\n')}
