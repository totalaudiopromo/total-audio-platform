...
class AudioIntelAutomation {
  ...
  getNextPost() {
    ...
    const availablePosts = AUTHENTIC_AUDIO_INTEL_POSTS.filter(post =>
  ...
  async runDailyPosting() {
    try {
      ...
      await this.log(`⏰ Next optimal time: ${nextTime.hour}:${nextTime.minute.toString().padStart(2, '0')} (${nextTime.description})`);
  ...
  async showStatus() {
    ...
    UK_PREMIUM_TIMES.forEach(time => {
      console.log(`  ${time.hour}:${time.minute.toString().padStart(2, '0')} - ${time.description}`);
    });
  }
}
