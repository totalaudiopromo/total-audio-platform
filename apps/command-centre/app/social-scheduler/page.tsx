...
import { Calendar, Clock, CheckCircle, XCircle, RefreshCw, Plus, Filter, Trash2 } from 'lucide-react';
  ...
  const loadPosts = async () => {
    try {
      ...
      const mockPosts: ScheduledPost[] = [
        ...
        {
          id: 'post_3',
          content: "Audio Intel beta update: 12,847 emails validated, 3,672 contacts enriched, 97.4% delivery rate.",
          platforms: ['twitter'],
    ...
    <div className="postcraft-page postcraft-container">
      ...
      <div className="postcraft-section">
        <div className="postcraft-section-header text-center">
          <h1 className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent mb-2">Social Scheduler</h1>
          <p className="text-gray-600">Manage your scheduled social media posts</p>
          <div className="flex items-center justify-center gap-3 mt-4">
            <button onClick={loadPosts} disabled={loading} className={`postcraft-button ${loading ? 'opacity-60 cursor-not-allowed' : ''}`}>
              <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin' : ''}`} />
              Refresh
            </button>
            <button onClick={() => setShowAddForm(!showAddForm)} className="postcraft-button bg-green-100 text-green-800 hover:bg-green-200">
        ...
        <div className="postcraft-section">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <button onClick={handleAddPost} className="postcraft-button bg-purple-100 text-purple-800 hover:bg-purple-200">
                <Calendar className="w-4 h-4" />
                Schedule Post
              </button>
              <button onClick={() => setShowAddForm(false)} className="postcraft-button bg-gray-100 text-gray-800 hover:bg-gray-200">
                Cancel
              </button>
            </div>
          </div>
      ...
      <div className="postcraft-section">
        ...
        <div className="text-sm text-gray-700 leading-6">
          <p className="mb-3">
            <strong>1. Schedule Posts:</strong> Add new posts with content, select platforms, and set timing.
          </p>
          <p className="mb-3">
            <strong>2. Automatic Posting:</strong> Posts are automatically published at the scheduled time.
          </p>
          <p className="mb-3">
            <strong>3. Platform Support:</strong> Twitter, LinkedIn, Bluesky, and Instagram integration.
          </p>
          <p className="mb-3">
