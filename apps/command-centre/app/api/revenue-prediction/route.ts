...
function predictConversionTime(user: UserBehaviour, conversionScore: number): number {
  ...
  return Math.max(1, Math.round(baseTime * engagementFactor * limitsFactor * paymentFactor * scoreFactor));
}
...
function generateRecommendedActions(user: UserBehaviour, conversionScore: number): string[] {
  ...
  if (user.engagement.lastActiveDate < new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString()) {
...
export async function GET() {
  try {
    ...
    const predictions: PredictionModel[] = users.map(user => {
      ...
      if (user.engagement.lastActiveDate < new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString()) {
    ...
    const mediumProbabilityUsers = predictions.filter(p => p.conversionScore >= 40 && p.conversionScore < 70).length;
    ...
    const averageConversionTime = predictions.reduce((sum, p) => sum + p.timeToConversion, 0) / predictions.length;
    ...
    const segments = [
      ...
      },
      {
        ...
        conversionRate: 60,
        averageValue: 150,
        totalPotentialRevenue: predictions.filter(p => p.likelyTier === 'enterprise').length * 150 * 0.6
      },
      {
        segment: 'Heavy Users',
        ...
        conversionRate: 75,
        averageValue: 45,
        totalPotentialRevenue: predictions.filter(p => p.topFactors.includes("Heavy export usage")).length * 45 * 0.75
      },
      {
        ...
        conversionRate: 90,
        averageValue: 35,
        totalPotentialRevenue: predictions.filter(p => p.topFactors.includes("Hit free tier limits")).length * 35 * 0.9
      }
    ];
