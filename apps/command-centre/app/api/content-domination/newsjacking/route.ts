...
const getMockOpportunities = (): NewsjackingOpportunity[] => [
  {
    ...
    summary: 'Spotify updates its playlist algorithm to give more weight to artist engagement metrics rather than just play counts.',
  ...
  },
  {
    ...
    summary: 'TikTok launches "Sound On" program connecting record labels with viral creators for music placement.',
  ...
  },
  {
    ...
    summary: 'Former Universal Music executive Sarah Mitchell announces "Curated Sounds" focusing on genre-specific playlists.',
    source: 'Variety',
  ...
  try {
    ...
    if (action === 'generate_more_content') {
      ...
      return NextResponse.json({
        ...
        generatedContent: {
          platform: 'instagram',
          content: 'Visual post concept: Infographic showing the shift from play count to engagement metrics with Audio Intel branding',
    ...
    return NextResponse.json(
      { error: 'Failed to process newsjacking action' },
      { status: 500 }
    );
  }
}