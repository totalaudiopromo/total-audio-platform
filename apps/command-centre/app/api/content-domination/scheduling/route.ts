    ...
    scheduledPieces: [
      {
        ...
        contentType: 'thread',
        content:
          'Just launched Audio Intel! Transform your messy contact spreadsheets into organised databases in minutes. Thread 1/5 🧵',
      ...
      },
      {
        ...
        contentType: 'article',
        content:
          'Why Independent Artists Are Drowning in Contact Chaos (And How Audio Intel Fixes It)',
      ...
      },
      {
        ...
        contentType: 'post',
        content:
          'Before/After: 10 chaotic Excel files → One organized contact database ✨ #AudioIntel',
    ...
    scheduledPieces: [
      {
        ...
        contentType: 'single',
        content:
          'This week in music marketing: Playlist submission strategies that actually work 🎵',
    ...
    scheduledPieces: [
      {
        ...
        contentType: 'thread',
        content:
          'THREAD: Why the new Spotify algorithm changes everything for independent artists...',
...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = {
      ...
      avgEngagementRate:
        batches.length > 0
          ? batches.reduce((sum, batch) => sum + batch.performance.avgEngagementRate, 0) /
            batches.length
          : 0,
      statusBreakdown: {
    ...
    return NextResponse.json({ error: 'Failed to fetch content batches' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to process scheduling action' }, { status: 500 });
  }
}
