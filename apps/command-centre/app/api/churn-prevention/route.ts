  ...
  try {
    ...
    const automatedActions = {
      triggered: [
        ...
        `Payment retry sequences launched for ${
          atRiskCustomers.filter(c => c.churnSignals.some(s => s.metric === 'payment_issues'))
            .length
        } customers`,
        `Re-engagement workflows triggered for ${
          atRiskCustomers.filter(c => c.churnSignals.some(s => s.signalType === 'behavioral'))
            .length
