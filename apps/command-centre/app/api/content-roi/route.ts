...
function generateMockContentData(): ContentPiece[] {
  const contentTypes = ['blog_post', 'video', 'social_post', 'email', 'guide', 'case_study'] as const;
...
function calculateContentROI(content: ContentPiece): number {
  const totalRevenue = content.attribution.directRevenue + (content.attribution.assistedRevenue * 0.3);
...
function generateInsights(content: ContentPiece[]): {
  ...
  content.forEach(c => {
    c.channels.forEach(channel => {
      channelConversions[channel] = (channelConversions[channel] || 0) + c.conversions.paidConversions;
    });
  });
...
function generateRecommendations(content: ContentPiece[], insights: any): string[] {
  ...
  if (insights.highROIContent.length > 0) {
    ...
    recommendations.push(`Double down on "${bestContent.type}" content - your highest ROI format at ${calculateContentROI(bestContent)}% return`);
  }
  
  // Channel optimization
  if (insights.topConversionChannels.length > 0) {
    recommendations.push(`Focus marketing budget on ${insights.topConversionChannels[0]} - your highest converting channel`);
  }
  ...
  if (totalAssistedRevenue > totalDirectRevenue * 2) {
    recommendations.push("Content heavily assists conversions - ensure full customer journey attribution");
  }
...
export async function GET() {
  try {
    ...
    const totalDirectRevenue = content.reduce((sum, c) => sum + c.attribution.directRevenue, 0);
    const totalAssistedRevenue = content.reduce((sum, c) => sum + c.attribution.assistedRevenue, 0);
    const totalCosts = content.reduce((sum, c) => sum + c.costs.productionCost + c.costs.promotionCost, 0);
    ...
    const data: ContentROIData = {
      ...
      summary: {
        ...
        revenuePerPound: totalCosts > 0 ? Math.round((totalDirectRevenue + totalAssistedRevenue * 0.3) / totalCosts * 100) / 100 : 0
      },
      insights,
      recommendations
    };
