...
function generateRevenueAlerts(): Alert[] {
  ...
  alerts.push({
    ...
    description:
      'Free to paid conversion rate has dropped 23% over the past 7 days, potentially due to recent pricing changes or competitor activity.',
  ...
  alerts.push({
    ...
    description:
      '12 users have hit their free tier limits and spent significant time on pricing pages. Prime for conversion with targeted outreach.',
  ...
  alerts.push({
    ...
    description:
      '8 paid subscribers showing strong churn signals: decreased usage, no logins in 5+ days, and support complaints.',
  ...
  alerts.push({
    ...
    description:
      'Blog post "How to Get on Spotify Playlists" generated 340% increase in trial signups with 67% conversion rate.',
  ...
  alerts.push({
    ...
    description:
      'ZeroBounce has reduced pricing by 30% for their basic plan, potentially affecting our competitive positioning.',
  ...
  alerts.push({
    ...
    description:
      'PR agency users showing 45% higher lifetime value and 60% lower churn than independent artists.',
  ...
  alerts.push({
    ...
    description:
      'Manchester-based users showing 35% higher engagement and 28% better conversion rates than London users.',
...
function generateAlertsSummary(alerts: Alert[]) {
  const criticalAlerts = alerts.filter(
    a => a.type === 'critical' || a.priority === 'urgent'
  ).length;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ error: 'Failed to generate revenue alerts' }, { status: 500 });
  }
}
