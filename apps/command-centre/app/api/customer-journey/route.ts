...
function generateMockCustomerJourneys(): CustomerJourney[] {
  const channels = ['Organic Search', 'Social Media', 'Email Marketing', 'Paid Search', 'Direct', 'Referral', 'Content Marketing'];
  const sources = ['google', 'facebook', 'twitter', 'linkedin', 'email', 'direct', 'blog', 'youtube'];
  const campaigns = ['summer-promo', 'beta-launch', 'content-series', 'webinar-follow-up', 'retargeting'];
  ...
  return Array.from({ length: 150 }, (_, i) => {
    ...
    for (let j = 0; j < numTouchPoints; j++) {
      ...
      touchPoints.push({
        ...
        campaign: Math.random() > 0.6 ? campaigns[Math.floor(Math.random() * campaigns.length)] : undefined,
        content: Math.random() > 0.5 ? contentTypes[Math.floor(Math.random() * contentTypes.length)] : undefined,
        sessionDuration,
        pageViews,
        interactions: {
        ...
        conversionEvents: j === numTouchPoints - 1 && status === 'converted' ? ['purchase'] : 
                         touchpointType === 'trial' ? ['signup', 'trial-start'] :
                         touchpointType === 'consideration' ? ['email-signup'] : [],
        value: status === 'converted' && j === numTouchPoints - 1 ? 
    ...
    return {
      ...
      totalInteractions: touchPoints.reduce((sum, tp) => 
        sum + tp.interactions.clicks + tp.interactions.downloads + tp.interactions.formSubmissions, 0
      ),
      channelsUsed: uniqueChannels,
      campaignsInteracted: uniqueCampaigns,
      keyMilestones: {
        firstVisit: touchPoints[0].timestamp,
        signupDate: signupTouchPoint?.timestamp || touchPoints[Math.floor(numTouchPoints * 0.3)]?.timestamp || touchPoints[0].timestamp,
...
function calculateJourneyAnalytics(journeys: CustomerJourney[]): JourneyAnalytics {
  ...
  const averageJourneyLength = convertedJourneys.length > 0 ?
    Math.round(convertedJourneys.reduce((sum, j) => sum + (j.timeToConversion || 0), 0) / convertedJourneys.length) : 0;
  ...
  journeys.forEach(journey => {
    const length = Math.floor((new Date(journey.endDate).getTime() - new Date(journey.startDate).getTime()) / (1000 * 60 * 60 * 24));
  ...
  const optimalLength = Object.entries(journeyLengthConversions)
...
function generateVisualisationData(journeys: CustomerJourney[]) {
  ...
  Object.entries(channelTransitions).forEach(([from, transitions]) => {
    Object.entries(transitions).forEach(([to, count]) => {
      if (count >= 3) { // Only show significant transitions
  ...
  const funnelData = [
    ...
    {
      stage: 'Consideration',
      users: journeys.filter(j => j.touchPoints.some(tp => tp.touchpointType === 'consideration')).length,
      conversionRate: 0,
      averageTime: 3
    },
    {
  ...
  for (let i = 1; i < funnelData.length; i++) {
    funnelData[i].conversionRate = Math.round((funnelData[i].users / funnelData[i-1].users) * 100 * 100) / 100;
  }
  
  // Channel attribution
  const channelStats: Record<string, { firstTouch: number; lastTouch: number; assisted: number; totalValue: number }> = {};
  ...
  journeys.forEach(journey => {
    ...
    journey.channelsUsed.forEach(channel => {
      ...
      if (channel === lastChannel && journey.status === 'converted') channelStats[channel].lastTouch++;
