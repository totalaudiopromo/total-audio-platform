...
function generateMarketTrends(): MarketTrend[] {
  ...
  return months.map((month, i) => ({
    ...
    seasonalityFactor:
      month === 'Dec' ? 1.4 : month === 'Jan' ? 0.8 : 1.0 + Math.sin(i * 0.2) * 0.2,
...
function generateRevenueForecasts(
  ...
  return months.map((month, i) => {
    ...
    const contentImpact =
      contentSignals.reduce(
        (sum, signal) => sum + (signal.conversionTrend * signal.qualityScore) / 100,
        0
    ...
    const totalGrowthFactor =
      (marketImpact / 100 + contentImpact / 100 + conversionImpact / 100) / 3;
    ...
    return {
      ...
      riskFactors: [
        ...
        conversionPatterns.retention.monthlyChurnRate > 5
      ...
      opportunityFactors: [
        ...
        conversionPatterns.tierUpgrades.trend > 20
          ? 'High upgrade momentum indicates pricing power'
          : null,
        marketTrends[i]?.digitalToolsAdoption > 20
          ? 'Market adoption trends favour digital tools'
          : null,
...
export async function GET() {
  try {
    ...
    const baseForecasts = generateRevenueForecasts(
      marketTrends,
      contentSignals,
      conversionPatterns
    );
    ...
    const projectedRevenue =
      baseForecasts[baseForecasts.length - 1]?.predictedRevenue || currentRevenue;
    ...
    return NextResponse.json({ error: 'Failed to generate revenue forecasts' }, { status: 500 });
  }
}
