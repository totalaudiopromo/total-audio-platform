  ...
  if (performanceData.length === 0) {
    const mockData: ContentPerformance[] = [
      ...
      },
      {
        ...
        audioIntelAngle:
          'Algorithm change = new contact strategies needed. Audio Intel adapts fastest',
        urgencyLevel: 'immediate',
...
export async function GET(request: NextRequest) {
  try {
    ...
    filteredData = filteredData.filter(item => new Date(item.timing.postedAt) >= cutoffDate);

    // Calculate aggregate metrics
    const totalImpressions = filteredData.reduce(
      (sum, item) => sum + item.postMetrics.impressions,
      0
    );
    const totalEngagement = filteredData.reduce(
      (sum, item) => sum + item.postMetrics.engagement,
      0
    );
    const totalClicks = filteredData.reduce((sum, item) => sum + item.postMetrics.clicks, 0);
    const totalConversions = filteredData.reduce(
      (sum, item) => sum + item.businessMetrics.conversions,
      0
    );
    const totalTraffic = filteredData.reduce(
      (sum, item) => sum + item.businessMetrics.websiteTraffic,
      0
    );
    const totalSignups = filteredData.reduce((sum, item) => sum + item.businessMetrics.signups, 0);

    const avgTimeToPost =
      filteredData.reduce((sum, item) => sum + item.timing.timeToPost, 0) / filteredData.length ||
      0;
    const avgEngagementRate =
      filteredData.reduce((sum, item) => sum + item.performance.engagementRate, 0) /
        filteredData.length || 0;
    const avgViralityScore =
      filteredData.reduce((sum, item) => sum + item.performance.viralityScore, 0) /
        filteredData.length || 0;
    ...
    const analytics = {
      overview: {
        ...
        overallROI:
          totalSignups > 0 ? Number(((totalConversions / totalSignups) * 100).toFixed(1)) : 0,
      },
      insights: {
        topPerformers,
...
function getTopPerformingAngles(
  data: ContentPerformance[]
): Array<{ angle: string; avgViralityScore: number; count: number }> {
...
function getPlatformPerformance(
  data: ContentPerformance[]
): Array<{ platform: string; metrics: any }> {
    ...
    .map(([platform, data]) => ({
      ...
      metrics: {
        ...
        avgEngagementRate: Number(
          ((data.totalEngagement / data.totalImpressions) * 100).toFixed(2)
        ),
...
function getUrgencyPerformance(
  data: ContentPerformance[]
): Array<{ urgency: string; metrics: any }> {
  ...
  return Array.from(urgencyMap.entries()).map(([urgency, data]) => ({
    urgency,
    metrics: {
      posts: data.posts,
