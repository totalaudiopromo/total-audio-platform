...
captureScreenshots().then(results => {
  ...
  console.log(`📸 Total screenshots: ${results.pages.reduce((acc, page) => acc + page.screenshots.length, 0)}`);
  console.log(`❌ Total errors: ${results.pages.reduce((acc, page) => acc + page.errors.length, 0)}`);
}).catch(error => {
