...
class PerformanceReportGenerator {
  ...
  async generateCampaignReport(
    ...
    try {
      ...
      const executiveSummary = await this.generateExecutiveSummary(
        campaignId, performanceData, playlistData, socialData, emailData
      );
  ...
  async generateAgencyReport(
    ...
    try {
      console.log(`Generating agency report for ${agencyId} covering ${campaigns.length} campaigns`);
      ...
      const agencyReport = await this.generateAgencyAggregation(agencyId, campaignReports, branding);
  ...
  private async generateEmailMarketingSection(email: any): Promise<any> {
    return {
      campaigns: email.top_performing_emails?.map((e: any) => ({
  ...
  private async generatePerformanceAnalysis(campaignId: string, data: any): Promise<any> {
    return {
      top_performing_content: data.social?.contentAnalysis?.topPerformingContent?.slice(0, 10) || [],
      audience_insights: {
        demographics: {
          age_distribution: [
  ...
  private async generateRecommendations(campaignId: string, analysis: any): Promise<RecommendationSection[]> {
    return [
      {
        category: 'content',
        priority: 'high',
        title: 'Optimize Content Mix for Higher Engagement',
        description: 'Analysis shows video content performs 3x better than static posts. Increase video content production.',
  ...
  private async generateAgencyAggregation(
    ...
    const totalStreams = campaignReports.reduce((sum, report) => sum + report.streaming.total_streams, 0);
    const totalFollowers = campaignReports.reduce((sum, report) => sum + report.social_media.cross_platform_summary.total_followers, 0);
    const totalRevenue = campaignReports.reduce((sum, report) => sum + (report.email_marketing.conversion_metrics.revenue_attributed || 0), 0);
    ...
    return {
      ...
      summary: {
        ...
        average_roi: campaignReports.reduce((sum, report) => sum + report.executiveSummary.roi_summary.roi_percentage, 0) / campaignReports.length
      },
      top_performing_campaigns: campaignReports
        .sort((a, b) => b.executiveSummary.roi_summary.roi_percentage - a.executiveSummary.roi_summary.roi_percentage)
        .slice(0, 5),
      agency_insights: {
  ...
  private async generateExecutiveDashboard(campaignReports: CampaignReport[]): Promise<any> {
    return {
      key_metrics: {
        ...
        combined_roi: campaignReports.reduce((sum, r) => sum + r.executiveSummary.roi_summary.roi_percentage, 0) / campaignReports.length
      },
      performance_trends: {
        weekly_growth: 15.5,
  ...
  private async generateHTMLReport(report: CampaignReport, branding?: any): Promise<string> {
    ...
    <body>
      <div class="header">
        ...
        <h2>${report.reportType.charAt(0).toUpperCase() + report.reportType.slice(1)} Performance Report</h2>
        <p>Period: ${new Date(report.reportPeriod.start).toDateString()} - ${new Date(report.reportPeriod.end).toDateString()}</p>
      </div>
      
      <div class="metric">
        <h3>Executive Summary</h3>
        <p>${report.executiveSummary.overview}</p>
        <p><strong>ROI:</strong> ${report.executiveSummary.roi_summary.roi_percentage.toFixed(1)}%</p>
      ...
      <footer style="margin-top: 50px; color: #666; font-size: 12px;">
        Generated by Audio Intel Performance Monitor on ${new Date(report.generated_at).toLocaleString()}
      </footer>
    </body>
    </html>
    `;
