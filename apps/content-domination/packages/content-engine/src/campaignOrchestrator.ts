...
class CampaignOrchestrator {
  ...
  async generateIntelDrivenCampaign(request: CampaignRequest): Promise<CampaignPlan> {
    try {
      ...
      const enhancedCampaign = await this.enhanceCampaignWithIntelligence(campaignPlan, intelInsights);
...
5. Platform-specific strategies and timing recommendations`;
   ...
   */
  private async generateCampaignStrategy(request: CampaignRequest, intelInsights: any): Promise<any> {
...
Generate a detailed 6-8 week campaign plan optimized with intelligence insights.`;
   ...
   */
  private async enhanceCampaignWithIntelligence(campaignPlan: any, intelInsights: any): Promise<CampaignPlan> {
  ...
  private extractMarketInsights(intelResponse: string): any {
    const competitiveLevel = intelResponse.includes('high competition') ? 'High' :
                           intelResponse.includes('low competition') ? 'Low' : 'Medium';
    
    const optimalTiming = intelResponse.includes('immediate release') ? 'Fast track' :
                         intelResponse.includes('delayed release') ? 'Extended timeline' : 'Standard';
  ...
  private extractContactStrategy(intelResponse: string): any {
    const primaryTargets = intelResponse.includes('playlist curators') ? 'Playlist curators' :
                          intelResponse.includes('radio DJs') ? 'Radio contacts' : 'Genre curators';
    
    const outreachTiming = intelResponse.includes('early outreach') ? '8 weeks pre-release' :
                          intelResponse.includes('late outreach') ? '2 weeks pre-release' : 'Pre-release';
  ...
  private extractTimingRecommendations(intelResponse: string): any {
    const startPhase = intelResponse.includes('extended campaign') ? '8 weeks pre-release' : '6 weeks pre-release';
    const peakEffort = intelResponse.includes('sustained effort') ? 'Release + 2 weeks' : 'Release week';
