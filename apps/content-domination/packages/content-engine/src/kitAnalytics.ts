...
class KitAnalytics {
  ...
  async getEmailMetrics(emailId: string): Promise<EmailMetrics> {
    try {
      ...
      // Calculate rates
      const open_rate = safeStats.delivered_count > 0 ? 
        (safeStats.opened_count / safeStats.delivered_count) * 100 : 0;
      
      const click_rate = safeStats.opened_count > 0 ? 
        (safeStats.clicked_count / safeStats.opened_count) * 100 : 0;
      
      const delivery_rate = safeStats.sent_count > 0 ? 
        (safeStats.delivered_count / safeStats.sent_count) * 100 : 0;
      
      const unsubscribe_rate = safeStats.delivered_count > 0 ? 
        (safeStats.unsubscribed_count / safeStats.delivered_count) * 100 : 0;
  ...
  async getSequenceMetrics(sequenceId: string): Promise<SequenceMetrics> {
    try {
      ...
      const emailPerformance = await Promise.all(
        (emails.emails as Array<{ id?: string }>).map((email: { id?: string }) => this.getEmailMetrics(email.id as string))
      );

      const total_revenue = emailPerformance.reduce(
        (sum, email) => sum + email.revenue_attributed, 0
      );

      const total_conversions = emailPerformance.reduce(
        (sum, email) => sum + email.conversions, 0
      );

      const conversion_rate = safeSequenceStats.total_subscribers > 0 ?
        (total_conversions / safeSequenceStats.total_subscribers) * 100 : 0;
      ...
      return {
        ...
        completion_rate: safeSequenceStats.total_subscribers > 0 ?
          (safeSequenceStats.completed_subscribers / safeSequenceStats.total_subscribers) * 100 : 0,
        total_revenue,
  ...
  async getABTestResults(testId: string): Promise<ABTestResults> {
    try {
      ...
      const status = test.status === 'completed' ? 'completed' :
        test.status === 'running' && winner.statistical_significance > 95 ? 'completed' :
        'running';
    ...
    try {
      ...
      for (let i = 0; i < 50; i++) {
        attributions.push({
          ...
          attributed_at: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString(),
    ...
    try {
      ...
      const sequencePerformance = await Promise.all(
        sequences.sequences.slice(0, 5).map(seq => 
          this.getSequenceMetrics(seq.id!)
        )
      );
      ...
      const averageOpenRate = allEmails.length > 0 ?
        allEmails.reduce((sum, email) => sum + email.open_rate, 0) / allEmails.length : 0;
      const averageClickRate = allEmails.length > 0 ?
        allEmails.reduce((sum, email) => sum + email.click_rate, 0) / allEmails.length : 0;
