...
`);
  ...
  async exchangeCodeForTokens(code) {
    try {
      ...
      if (error.response) {
        throw new Error(`LinkedIn token exchange failed: ${error.response.data.error_description || error.response.data.error || 'Unknown error'}`);
      } else {
  ...
  async fetchProfile() {
    try {
      ...
      if (error.response) {
        throw new Error(`LinkedIn profile fetch failed: ${error.response.data.error_description || error.response.data.error || 'Unknown error'}`);
      } else {
...
`;
  ...
  showSuccess() {
    ...
    console.log(`3. ${colors.blue}npm run start:economy${colors.reset} - Start zero-cost monitoring`);
