...
class CostMonitor {
  ...
  async trackAPICall(service, endpoint, tokens = 0, success = true, metadata = {}) {
    ...
    console.log(`📊 API Call Tracked: ${service}/${endpoint} - £${cost.toFixed(4)} (${tokens} tokens)`);
  ...
  async checkAllLimits() {
    ...
    if (usage.totalCost > this.costThresholds.monthly_budget * this.warningThresholds.warning) {
      alerts.push({
        ...
        severity: usage.totalCost > this.costThresholds.monthly_budget * this.warningThresholds.emergency ? 'emergency' : 'warning'
      });
    }
    ...
    for (const [service, serviceUsage] of Object.entries(usage.serviceBreakdown)) {
      ...
      if (limit && serviceUsage.calls > limit * this.warningThresholds.warning) {
        alerts.push({
          ...
          limit: limit,
          severity: serviceUsage.calls > limit * this.warningThresholds.emergency ? 'emergency' : 'warning'
        });
      }
    }
  ...
  async shouldEmergencyShutdown() {
    const usage = await this.getCurrentMonthUsage();
    const emergencyThreshold = this.costThresholds.monthly_budget * this.warningThresholds.emergency;
  ...
  calculateProjectedCosts(usage) {
    ...
    return {
      ...
      budgetRisk: projectedMonthEnd > this.costThresholds.monthly_budget ? 'high' : 
                  projectedMonthEnd > this.costThresholds.monthly_budget * 0.8 ? 'medium' : 'low'
    };
  }
  ...
  calculateTrends(monthData) {
    ...
    if (recentAvg > earlierAvg * 1.2) return { trend: 'increasing', change: ((recentAvg - earlierAvg) / earlierAvg * 100).toFixed(1) };
    if (recentAvg < earlierAvg * 0.8) return { trend: 'decreasing', change: ((earlierAvg - recentAvg) / earlierAvg * 100).toFixed(1) };
    return { trend: 'stable' };
  }
  ...
  switch (command) {
    case 'dashboard':
      monitor.getDashboardData().then(data => {
        ...
        console.log(`💰 Current Spend: £${data.currentUsage.totalCost.toFixed(2)} / £${data.budgetRemaining + data.currentUsage.totalCost}`);
        console.log(`📈 Projected: £${data.projectedCosts.projected.toFixed(2)} (${data.projectedCosts.budgetRisk} risk)`);
    ...
    default:
      console.log('Usage: node cost-monitor.js [dashboard|emergency-check|track <service> <endpoint> <tokens>]');
  }
}