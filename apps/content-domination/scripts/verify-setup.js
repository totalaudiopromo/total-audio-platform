...
class SetupVerifier {
  ...
  async checkTwitterAPI() {
    ...
    try {
      ...
      if (response.data && response.data.data) {
        ...
        try {
          const postTest = await axios.post('https://api.twitter.com/2/tweets', 
          ...
          log.warning('Twitter posting works, but we sent a test tweet. You may want to delete it.');
        } catch (postError) {
          if (postError.response?.status === 403) {
            log.warning('Twitter API connected but posting may be restricted. Check app permissions.');
            this.results.warnings++;
          }
        }
      }
  ...
  async checkLinkedInAPI() {
    ...
    try {
      ...
      if (response.data) {
        log.success(`LinkedIn API connected. User: ${response.data.localizedFirstName} ${response.data.localizedLastName}`);
        this.results.passed++;
      }
    } catch (error) {
    ...
    try {
      ...
      for (const dbVar of requiredDatabases) {
        ...
        try {
          ...
          if (dbResponse.data) {
            log.success(`Database accessible: ${dbResponse.data.title?.[0]?.plain_text || 'Unnamed'}`);
            this.results.passed++;
          }
        } catch (dbError) {
          log.error(`Database ${dbVar} not accessible: ${dbError.response?.data?.message || dbError.message}`);
          this.results.failed++;
  ...
  async checkClaudeAPI() {
    ...
    try {
      ...
      log.error(`Claude API connection failed: ${error.response?.data?.error?.message || error.message}`);
      this.results.failed++;
    ...
    try {
      ...
      if (response.data) {
        ...
        if (process.env.KIT_NEWSLETTER_FORM_ID) {
          try {
            const formResponse = await axios.get(`https://api.kit.com/v3/forms/${process.env.KIT_NEWSLETTER_FORM_ID}`, {
              headers: {
                'Authorization': `Bearer ${process.env.KIT_API_KEY}`
              }
            });
  ...
  async checkNewsSourceAPIs() {
    ...
    if (process.env.SPOTIFY_CLIENT_ID && process.env.SPOTIFY_CLIENT_SECRET) {
      try {
        ...
        log.error(`Spotify API failed: ${error.response?.data?.error_description || error.message}`);
        this.results.failed++;
      }
    }
  }
  ...
  generateReport() {
    ...
    if (this.results.failed === 0) {
      ...
      console.log(`${colors.green}Your Content Domination System is ready to launch.${colors.reset}`);
    ...
    console.log(`${colors.blue}🔧 Support: Check troubleshooting section in setup guide${colors.reset}\n`);
