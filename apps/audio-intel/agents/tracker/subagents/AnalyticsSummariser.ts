...
export class AnalyticsSummariser {
  ...
  private static calculateRates(metrics: CampaignAnalytics['metrics']) {
    const { totalSubmissions, deliveredCount, openedCount, repliedCount, rejectedCount } = metrics;
  ...
  private static generateInsights(
    ...
    if (rates.deliveryRate >= 95) {
      ...
      insights.push('Low delivery rate - consider validating contact emails before sending');
    }
