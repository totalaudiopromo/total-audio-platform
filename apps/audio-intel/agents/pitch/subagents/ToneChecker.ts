  ...
  private static generateRecommendations(issues: ToneCheckResult['issues']): string[] {
    ...
    if (issues.some(i => i.type === 'corporate_speak')) {
      recommendations.push(
        'Avoid corporate jargon - write like you'd talk to a mate in the industry'
      )
    }

    if (issues.some(i => i.type === 'ai_buzzword')) {
      recommendations.push(
        'Skip the AI hype - focus on what the tool actually does for users'
      )
    }

    if (issues.some(i => i.type === 'inauthentic')) {
      recommendations.push(
        'Be more direct and honest - avoid overly enthusiastic marketing speak'
      )
    }
