...
function addWatermarkToPage(doc: jsPDF, config: WatermarkConfig): void {
  ...
  const lightColor = config.color!.map(c => Math.min(255, c + (255 - c) * (1 - config.opacity!))) as [number, number, number];
...
export const UPGRADE_MESSAGES = {
  afterPreview: {
    ...
    description: 'Get the full report with all contacts, detailed analytics, and professional formatting.',
 ...
 */
export function getUpgradeMessage(context: string, userTier: string = 'free', additionalContext?: any) {
  const baseMessage = UPGRADE_MESSAGES[context as keyof typeof UPGRADE_MESSAGES] || UPGRADE_MESSAGES.afterPreview;
  ...
  if (userTier === 'free' && context === 'afterPreview') {
    return {
      ...
      description: 'Upgrade to Professional for complete reports, no watermarks, and premium features starting at £19/month.'
    };
  }
  ...
  if (userTier === 'professional' && context === 'whiteLabel') {
    return {
      ...
      description: 'Upgrade to Agency tier to add your logo, colors, and create client-ready reports.'
    };
  }
