    ...
    data.forEach((row, rowIndex) => {
      ...
      if (emptyPercentage > 0.8) {
        issues.push({
          type: 'missing_data',
          severity: 'low',
          description: `Row ${rowIndex + 2} is mostly empty (${Math.round(emptyPercentage * 100)}% empty)`,
          row: rowIndex,
...
export class ColumnMappingAgent {
  ...
  static detectColumns(headers: string[], data: any[][]): ColumnMapping {
    ...
    headers.forEach((header, index) => {
      ...
      if (!matched) {
        const sampleData = data.slice(0, 5).map(row => row[index]).filter(Boolean);
        ...
        if (sampleData.length > 0) {
          ...
          else if (sampleData.every(item => typeof item === 'string' && /[\d\s\-\+\(\)]{8,}/.test(item))) {
            mapping.phone = index;
            matched = true;
          }
          // Check if it looks like website
          else if (sampleData.every(item => typeof item === 'string' && /https?:\/\/|www\./i.test(item))) {
...
export class DataNormalisationAgent {
  ...
  private static cleanRole(role: any): string {
    return role.toString().trim().replace(/\b\w/g, (l: string) => l.toUpperCase());
  }
...
export class DeduplicationAgent {
  ...
  static mergeDuplicates(contacts: ProcessedContact[], duplicateMap: Map<number, number[]>): ProcessedContact[] {
  ...
  private static mergeContactData(primary: ProcessedContact, secondary: ProcessedContact): ProcessedContact {
...
export class SpreadsheetProcessingPipeline {
  static async processFiles(files: File[]): Promise<{
    ...
    for (const file of files) {
      ...
      const issues = DataQualityAgent.analyseSpreadsheet(fileData.data, fileData.headers, file.name);
      ...
      const contacts = fileData.data.map(row => 
        DataNormalisationAgent.normaliseContact(row, columnMapping, file.name)
    ...
    const duplicatesFound = Array.from(duplicateMap.values()).reduce((sum, dups) => sum + dups.length, 0);
    const mergedContacts = DeduplicationAgent.mergeDuplicates(allContacts, duplicateMap);
    
    // Calculate summary statistics
    const confidence = mergedContacts.reduce((acc, contact) => {
  ...
  private static calculateFileConfidence(issues: DataIssue[], contacts: ProcessedContact[]): number {
    let score = 100;
