# Apple Music Editorial Playlist Submission Research

**Research Date**: October 2025
**Status**: Current and Verified
**Target Audience**: Independent UK Artists

---

## Executive Summary

**CRITICAL FINDING**: Unlike Spotify, Apple Music does **NOT** offer direct playlist pitching for independent artists. Editorial playlist consideration requires working through approved distributors who have access to the Apple Music Pitch tool.

**Key Differences from Spotify**:

- No public submission portal for artists
- Must work through distributor relationships
- Requires 10+ days advance submission (via distributor)
- Editorial team operates in a "closed loop" system

---

## 1. Apple Music Pitch Tool Access

### Who Has Access?

**Apple Music Partners Only** - The Apple Music Pitch tool is exclusively available to:

- Approved distributors (iTunes Partners)
- Record labels with iTunes Partner accounts
- **NOT available to individual independent artists**

**Source**: [Apple Music Pitch User Guide](https://itunespartner.apple.com/music/support/5391-apple-music-pitch-user-guide)

### Access Requirements

To use Apple Music Pitch, partners must:

1. Be an approved Apple Music Partner
2. Have an Apple ID associated with their iTunes Connect account
3. Receive access granted by company account Admin
4. Fill out Apple's eligibility questionnaire
5. Receive email invitation to the pitching portal

**Source**: [Apple Music Pitch User Guide](https://itunespartner.apple.com/music/support/5391-apple-music-pitch-user-guide)

---

## 2. Apple Music Preferred Distributors (October 2025)

### Official Preferred Partners List

**Verified Apple Music Preferred Distributors** (as of October 2025):

| Distributor                   | Dolby Atmos | Credits | Lyrics | Motion Art | Analytics |
| ----------------------------- | ----------- | ------- | ------ | ---------- | --------- |
| Believe                       |            |        |       |           |          |
| TuneCore                      |            |        |       |           |          |
| TuneCore Japan                |            |        |       |           |          |
| EWWAY                         |            |        |       |           |          |
| Label Engine                  |            |        |       |           |          |
| Golden Dynamic                |            |        |       |           |          |
| AudioSalad                    |            |        |       |           |          |
| Symphonic                     |            |        |       |           |          |
| FUGA                          |            |        |       |           |          |
| Kontor                        |            |        |       |           |          |
| Rock Mobile                   |            |        |       |           |          |
| Stem                          |            |        |       |           |          |
| Metropolitan Groove Merchants |            |        |       |           |          |
| recordJet                     |            |        |       |           |          |
| IDOL                          |            |        |       |           |          |

**Source**: [Apple Music Preferred Partners](https://artists.apple.com/partners)

### Preferred Plus Tier

**"Preferred Plus" Distributors** (highest tier):

- Believe Digital
- FUGA

**Requirements**: Must deliver 40,000+ songs per quarter to qualify for Preferred Plus status.

**Source**: [Digital Music News - Apple Music Preferred Distributors](https://www.digitalmusicnews.com/2020/02/11/apple-music-preferred-distributors/)

### Popular UK-Accessible Distributors

**NEEDS VERIFICATION**: The following distributors are commonly used by UK artists, but their Apple Music Pitch access status requires direct confirmation:

- **DistroKid**: Popular with indie artists, offers Apple Music for Artists verification
- **CD Baby**: Preferred provider status mentioned, but Apple Music Pitch access unclear
- **UnitedMasters**: Claims direct Apple Music editorial team relationships
- **LANDR**: Listed as preferred partner of Apple Music

**Action Required**: Artists should **directly ask their distributor** if they have Apple Music Pitch tool access.

---

## 3. Submission Process (Via Distributor)

### Timeline Requirements

**Official Apple Music Deadlines**:

- **Full Coverage Consideration**: Pitch 10 days before release date
- **Final Deadline (Late Adds)**: Pitch 7 days before release date minimum
- **Recommended Lead Time**: 3-4 weeks for maximum consideration

**Source**: [Apple Music Pitch User Guide](https://itunespartner.apple.com/music/support/5391-apple-music-pitch-user-guide)

### Pitch Information Required

When distributors submit pitches via Apple Music Pitch, they provide:

**Basic Information**:

- Release type (New Release, Pre-Add/Pre-Order, Re-promotion)
- UPC/Catalogue number
- Artist name and track details
- Promotion start date and territories

**Enhanced Metadata** (Highly Recommended):

- Spatial Audio (Dolby Atmos) details
- Motion Artwork
- Lyrics (mandatory for vocal content as of January 2025)
- Contributor credits (Writers, Performers, Production team)

**Promotional Context**:

- Campaign key moments and marketing plans
- Song release schedule and focus tracks
- Media moments and press coverage
- Listening links and press photos

**Source**: [Apple Music Pitch User Guide](https://itunespartner.apple.com/music/support/5391-apple-music-pitch-user-guide)

### New 2024-2025 Requirements

**Effective January 2, 2025 - Mandatory Contributor Credits**:

Each track must include at least one role from each of three groups:

1. **Writers**: Lyricist, Composer, or Arranger
2. **Performers**: Solo and band members
3. **Production & Engineering**: Producer, Recording Engineer, Mixing Engineer, Mastering Engineer, or Immersive Engineer

**Lyrics Requirement**:

- All releases with vocal content must include lyrics
- Even tracks with only one or two words require lyric submission

**Source**: [Symphonic Blog - New Apple Music Requirements](https://blog.symphonic.com/2024/08/01/new-apple-music-requirements-for-artists-2/)

---

## 4. Main Editorial Playlists

### Flagship Global Playlists

**New Music Daily** (Premiere Global Editorial Playlist):

- **Format**: 60 songs, multi-genre
- **Update Frequency**: Daily (Monday-Friday)
- **Structure**: New lead song and cover artist daily
- **Launch**: August 23, 2019 (Taylor Swift as first cover star)
- **Purpose**: Apple's answer to Spotify's "New Music Friday"

**Source**: [Music Business Worldwide - New Music Daily Launch](https://www.musicbusinessworldwide.com/apple-music-launches-new-music-daily-playlist/)

### Genre-Specific Flagship Playlists

**40+ Genre-Specific Editorial Playlists** (2025):

- **Rap Life** (Hip-Hop/Rap)
- **Pop Chill** (Pop)
- **Afrobeats Weekly** (Afrobeats)
- **Country Heat** (Country)
- **danceXL** (Dance/Electronic)
- **ALT CTRL** (Alternative)
- **Rock Hits** (Rock)

**Update Frequency**: Daily or weekly depending on playlist
**Editorial Team**: Global Apple Music curators

**Source**: [Stereofox - Best Apple Music Editorial Playlists 2023](https://www.stereofox.com/articles/best-apple-music-editorial-playlists/)

### UK-Specific Playlists

**NEEDS VERIFICATION**: The following UK playlists exist but specific submission processes require verification:

- UK rap and grime playlists
- UK indie/alternative playlists
- Regional genre-specific playlists

---

## 5. How Independent Artists Can Get Playlisted

### Primary Method: Distributor Pitching

**Step-by-Step Process**:

1. **Choose a Distributor with Apple Music Pitch Access**
   - Contact your current distributor and ask directly: "Do you have access to Apple Music Pitch?"
   - If not, consider switching to a verified preferred partner (see list above)
   - **Timing**: Do this BEFORE your release is scheduled

2. **Prepare Your Release (10+ Days in Advance)**
   - Finalise mastered audio files
   - Create required metadata (see section 3)
   - Prepare contributor credits (mandatory as of January 2025)
   - Upload lyrics for any vocal content
   - Create Motion Artwork (if possible)
   - Prepare Dolby Atmos version (if available)

3. **Provide Promotional Context to Your Distributor**
   - Write compelling artist story and track background
   - Share marketing plans and media coverage
   - Provide press photos and listening links
   - Highlight any unique aspects of the release

4. **Distributor Submits Pitch**
   - Your distributor submits via Apple Music Pitch tool
   - They include all provided metadata and promotional context
   - Submission goes to Apple Music's global editorial team

5. **Wait for Editorial Review**
   - No guaranteed response or feedback
   - Playlisting decisions are made by editorial team
   - Check Apple Music for Artists dashboard for any activity

**Sources**: Multiple verified sources including Apple Music Pitch User Guide and distributor guidance

### Alternative/Supporting Methods

**Method 1: Optimize Apple Music for Artists Profile**

**Essential Profile Elements**:

- High-quality artist image (2400 x 2400 pixels preferred, 800 x 800 minimum)
- Compelling artist biography with keywords
- Artist Content section (musical influences, collaborations, achievements)
- Regular profile updates with new releases
- Consistent branding across social platforms

**Why This Matters**: Well-maintained profiles signal professionalism to editorial curators and improve discoverability.

**Source**: [LALAL.AI - How to Optimize Apple Music Artist Page 2025](https://www.lalal.ai/blog/how-to-optimize-your-apple-music-artist-page-in-2025/)

**Method 2: Build Organic Momentum**

**Engagement Strategies**:

- Drive streams to Apple Music profile from social media
- Encourage fans to add tracks to personal playlists
- Build strong early streaming numbers
- Generate media coverage and press
- Develop industry relationships

**Why This Matters**: High engagement signals quality and can attract organic editorial attention.

**Source**: [Playlist Push - Apple Music Playlists Guide](https://playlistpush.com/blog/how-to-get-on-apple-music-playlists-a-guide-for-independent-artists/)

**Method 3: Industry Relationships**

**Leverage Connections**:

- Manager or PR team with Apple Music relationships
- Publicist with track record of Apple Music placements
- Industry contacts who can make warm introductions
- Music supervisors and A&R contacts

**Why This Matters**: Personal recommendations carry weight with editorial teams.

**Source**: Multiple industry sources

**Method 4: User-Generated Playlists**

**Building Playlist Presence**:

- Target influential user-created playlists first
- Use playlist pitching services (Playlist Push, SubmitHub, etc.)
- Build relationships with independent playlist curators
- Strong user playlist performance can attract editorial attention

**Why This Matters**: Editorial teams monitor user-generated playlists for emerging talent.

**Source**: [iMusician - Playlist Submission Mistakes](https://imusician.pro/en/resources/blog/playlist-submission-mistakes)

---

## 6. Common Mistakes Artists Make

### Critical Errors to Avoid

**1. Submitting Poor Audio Quality**

**The Mistake**:

- Low-quality MP3 files instead of WAV
- Rough demos or poorly mixed tracks
- Unmastered audio
- Inconsistent volume levels

**Why It Fails**: Audio quality standards for playlist inclusion are non-negotiable. Poor quality signals amateur production and guarantees immediate rejection.

**The Fix**: Professional mixing and mastering, proper file formats (WAV/FLAC), quality control before distribution.

**Source**: [iMusician - Playlist Submission Mistakes](https://imusician.pro/en/resources/blog/playlist-submission-mistakes)

**2. Incorrect Submission Timing**

**The Mistake**:

- Submitting 2-3 days before release
- Assuming last-minute submissions are acceptable
- Not accounting for distributor processing time

**Why It Fails**: Apple Music editorial team needs minimum 7-10 days for review. Late submissions miss consideration windows entirely.

**The Fix**:

- Schedule distributor pitch 10+ days before release
- Allow 3-4 weeks for maximum consideration
- Account for distributor processing time (add 2-3 days)

**Source**: [Apple Music Pitch User Guide](https://itunespartner.apple.com/music/support/5391-apple-music-pitch-user-guide)

**3. Misunderstanding Apple Music's Closed System**

**The Mistake**:

- Assuming there's a public submission portal like Spotify
- Trying to contact curators directly via social media
- Expecting to pitch directly without a distributor

**Why It Fails**: Apple Music operates a closed editorial system. Only approved distributors can submit pitches.

**The Fix**:

- Verify your distributor has Apple Music Pitch access
- Work through proper distributor channels
- Build relationships rather than cold outreach

**Source**: [Playlist Push - Apple Music Guide](https://playlistpush.com/blog/how-to-get-on-apple-music-playlists-a-guide-for-independent-artists/)

**4. Incomplete or Poor Metadata**

**The Mistake**:

- Missing contributor credits
- No lyrics for vocal tracks
- Generic artist bio
- Low-resolution artwork
- Missing genre information

**Why It Fails**: As of January 2025, contributor credits and lyrics are mandatory. Incomplete metadata disqualifies releases.

**The Fix**:

- Complete all contributor fields (Writers, Performers, Production)
- Upload lyrics for ALL vocal content
- High-resolution artwork (2400 x 2400 pixels)
- Detailed, professional metadata

**Source**: [Symphonic Blog - New Apple Music Requirements](https://blog.symphonic.com/2024/08/01/new-apple-music-requirements-for-artists-2/)

**5. Not Following Platform Guidelines**

**The Mistake**:

- Ignoring distributor submission requirements
- Incorrect file formats
- Missing deadline requirements
- Incomplete promotional materials

**Why It Fails**: Every platform has specific rules. Non-compliance results in automatic rejection.

**The Fix**:

- Read distributor guidelines thoroughly
- Follow Apple Music technical specifications exactly
- Prepare all materials in advance
- Double-check requirements before submission

**Source**: [iMusician - Playlist Submission Mistakes](https://imusician.pro/en/resources/blog/playlist-submission-mistakes)

**6. Unrealistic Genre Targeting**

**The Mistake**:

- Pitching indie rock to hip-hop playlists
- Targeting playlists with completely different vibes
- Not researching playlist content

**Why It Fails**: Editorial teams instantly reject genre mismatches.

**The Fix**:

- Research target playlists thoroughly
- Match your sound to appropriate playlists
- Be honest about your genre classification
- Target realistic playlist tiers (don't expect New Music Daily immediately)

**Source**: Industry best practices from multiple sources

**7. Weak Artist Profile and Presence**

**The Mistake**:

- Unclaimed Apple Music for Artists profile
- No artist bio or outdated information
- Low-resolution or missing artist image
- No social media links
- Inconsistent branding

**Why It Fails**: Editorial teams check artist profiles. Weak presentation suggests unprofessionalism.

**The Fix**:

- Claim and fully optimize Apple Music for Artists profile
- Professional artist photography (2400 x 2400 pixels)
- Compelling biography with keywords
- Consistent branding across all platforms
- Regular profile updates

**Source**: [LALAL.AI - Optimize Apple Music Artist Page](https://www.lalal.ai/blog/how-to-optimize-your-apple-music-artist-page-in-2025/)

---

## 7. UK Artist-Specific Considerations

### UK Market Advantages

**Geographic Targeting**:

- Apple Music allows territory-specific pitching
- UK artists can target UK-specific editorial playlists
- Geographic analytics available via Apple Music for Artists
- Regional curator teams exist for UK market

**NEEDS VERIFICATION**: Specific UK curator contact methods and regional playlist submission processes

### Brexit and Distribution

**NEEDS VERIFICATION**: Post-Brexit implications for UK artists using EU-based distributors and metadata requirements

### UK-Focused Distributors

**Distributors with Strong UK Presence**:

- Symphonic (Preferred Partner)
- Believe (Preferred Plus Partner)
- Various UK-based aggregators

**Action Required**: Research which distributors have strongest UK editorial relationships

---

## 8. Key Differences: Apple Music vs Spotify

| Feature                       | Apple Music                   | Spotify                       |
| ----------------------------- | ----------------------------- | ----------------------------- |
| **Direct Artist Pitching**    |  No - Distributor only      |  Yes - Spotify for Artists   |
| **Minimum Lead Time**         | 10 days (via distributor)     | 7 days (artist direct)        |
| **Public Submission Portal**  |  No                         |  Yes                         |
| **Editorial Team Access**     | Closed/Private                | Semi-open via portal          |
| **Playlist Update Frequency** | Daily/Weekly varies           | Weekly (New Music Friday)     |
| **Feedback on Submissions**   |  None                       |  None                       |
| **Independent Curators**      |  No user playlists featured |  Yes - large curator network |
| **Analytics Dashboard**       | Apple Music for Artists       | Spotify for Artists           |
| **Lyrics Requirement**        |  Mandatory (Jan 2025)        | Optional but recommended      |
| **Contributor Credits**       |  Mandatory (Jan 2025)        | Optional but recommended      |

**Sources**: Comparison compiled from Apple Music Pitch User Guide and Spotify for Artists documentation

---

## 9. Action Plan for UK Independent Artists

### 30 Days Before Release

**Week 1: Preparation**

- [ ] Verify distributor has Apple Music Pitch access
- [ ] Finalize master and Dolby Atmos version (if possible)
- [ ] Complete contributor credits (Writers, Performers, Production)
- [ ] Write lyrics document for vocal tracks
- [ ] Create Motion Artwork (if possible)
- [ ] Prepare press photos (high resolution)
- [ ] Write compelling artist story and track background
- [ ] Research target playlists and genres

**Week 2: Profile Optimization**

- [ ] Claim Apple Music for Artists profile (if not already done)
- [ ] Upload professional artist image (2400 x 2400 pixels)
- [ ] Write compelling artist biography with keywords
- [ ] Add Artist Content (influences, collaborations, achievements)
- [ ] Link social media accounts
- [ ] Ensure consistent branding across platforms

**Week 3: Distributor Submission**

- [ ] Deliver all assets to distributor (10+ days before release)
- [ ] Provide promotional context and marketing plans
- [ ] Confirm distributor has submitted pitch via Apple Music Pitch
- [ ] Receive confirmation from distributor
- [ ] Set up pre-save/pre-add campaign

**Week 4: Pre-Release Marketing**

- [ ] Drive social media engagement to Apple Music profile
- [ ] Share listening links with press and industry contacts
- [ ] Pitch to user-generated playlists
- [ ] Build early momentum with strategic content
- [ ] Monitor Apple Music for Artists dashboard

### Release Week

- [ ] Check Apple Music for Artists for any playlist adds
- [ ] Encourage fans to stream and add to personal playlists
- [ ] Share release across all social platforms
- [ ] Monitor analytics for geographic and demographic insights
- [ ] Continue user-generated playlist pitching

### Post-Release (Ongoing)

- [ ] Track performance in Apple Music for Artists dashboard
- [ ] Use geographic insights to plan promotion strategy
- [ ] Build relationships with curators and industry contacts
- [ ] Continue profile optimization and updates
- [ ] Learn from analytics for next release

---

## 10. Sources and References

### Official Apple Resources

1. **Apple Music Pitch User Guide**
   URL: https://itunespartner.apple.com/music/support/5391-apple-music-pitch-user-guide
   Content: Official pitch tool documentation, timelines, requirements

2. **Apple Music Preferred Partners**
   URL: https://artists.apple.com/partners
   Content: Complete list of preferred distributors with capabilities

3. **Apple Music for Artists Support**
   URL: https://artists.apple.com/support/
   Content: Official artist dashboard documentation and support

### Industry Sources (2024-2025)

4. **Symphonic Blog - New Apple Music Requirements (August 2024)**
   URL: https://blog.symphonic.com/2024/08/01/new-apple-music-requirements-for-artists-2/
   Content: Mandatory contributor credits and lyrics requirements

5. **LALAL.AI - Optimize Apple Music Artist Page (2025)**
   URL: https://www.lalal.ai/blog/how-to-optimize-your-apple-music-artist-page-in-2025/
   Content: Profile optimization guide with current best practices

6. **Playlist Push - Apple Music Playlists Guide**
   URL: https://playlistpush.com/blog/how-to-get-on-apple-music-playlists-a-guide-for-independent-artists/
   Content: Independent artist strategies for Apple Music

7. **iMusician - Playlist Submission Mistakes**
   URL: https://imusician.pro/en/resources/blog/playlist-submission-mistakes
   Content: Common errors and how to avoid them

8. **Music Business Worldwide - New Music Daily Launch**
   URL: https://www.musicbusinessworldwide.com/apple-music-launches-new-music-daily-playlist/
   Content: Information about Apple Music's flagship playlist

9. **Digital Music News - Apple Music Preferred Distributors**
   URL: https://www.digitalmusicnews.com/2020/02/11/apple-music-preferred-distributors/
   Content: Preferred Plus tier requirements and distributor expansion

10. **That Eric Alper - Playlist Pitching Guide 2025**
    URL: https://www.thatericalper.com/2025/09/15/how-to-pitch-yourself-to-spotify-apple-music-and-playlists-in-2025-the-ultimate-guide-for-independent-artists/
    Content: Comprehensive 2025 pitching strategies

---

## 11. Areas Requiring Further Verification

### Questions for Follow-Up Research

1. **Specific UK Regional Playlists**
   - Need complete list of UK-specific editorial playlists
   - UK curator team structure and regional focus
   - Brexit implications for UK artists using EU distributors

2. **Distributor Pitch Access Confirmation**
   - DistroKid's current Apple Music Pitch access status
   - CD Baby's Apple Music Pitch capabilities
   - RouteNote and other UK-popular distributors

3. **Success Rates and Benchmarks**
   - Typical Apple Music editorial playlist acceptance rates
   - Average streams needed to attract editorial attention
   - Case studies of independent artist playlist success

4. **UnitedMasters Claims Verification**
   - Verify claims of direct Apple Music editorial team access
   - Understand their specific submission process
   - Compare with other distributor relationships

5. **Post-2025 Policy Changes**
   - Monitor for any changes to submission requirements
   - Track new metadata or technical requirements
   - Watch for distributor policy updates

---

## Final Notes

**Last Updated**: October 2025
**Research Confidence**: High for official Apple policies, Medium for specific distributor capabilities
**Recommended Action**: Artists should directly contact their distributor to confirm Apple Music Pitch access

**Key Takeaway**: Apple Music editorial playlisting for independent artists is 100% dependent on distributor relationships. Unlike Spotify, there is NO direct artist submission portal. Success requires:

1. Choosing a distributor with Apple Music Pitch access
2. Following strict metadata and timing requirements
3. Building organic momentum and engagement
4. Maintaining a professional Apple Music for Artists profile
5. Allowing 10+ days for editorial review

**This research represents current best practices as of October 2025. Always verify requirements directly with your distributor and monitor official Apple Music channels for updates.**
