...
export async function GET(request: NextRequest) {
  try {
    ...
    const highRelevanceEpisodes = curatedEpisodes.filter(
      ep => ep.musicIndustryRelevance === 'High'
    );
    const mediumRelevanceEpisodes = curatedEpisodes.filter(
    ...
    console.log(
      `✅ Loaded ${curatedEpisodes.length} curated episodes (${highRelevanceEpisodes.length} high-relevance, ${mediumRelevanceEpisodes.length} medium-relevance)`
    );
    ...
    return NextResponse.json({
      ...
      podcast: {
        ...
        description:
          'Hand-picked episodes from your favorite podcasts focusing on AI and technology insights for the music industry',
