...
export async function GET() {
  try {
    ...
    const subscribersResponse = await fetch(`https://api.convertkit.com/v3/tags/10182443/subscribers?api_secret=${CONVERTKIT_API_SECRET}`);
    ...
    const broadcastsResponse = await fetch(`https://api.convertkit.com/v3/broadcasts?api_secret=${CONVERTKIT_API_SECRET}`);
    let recentIssues = 0;
    ...
    if (broadcastsResponse.ok) {
      ...
      if (broadcastsData.broadcasts && broadcastsData.broadcasts.length > 0) {
        ...
        const totalOpens = recentBroadcasts.reduce((sum: number, broadcast: any) => sum + (broadcast.stats?.opens || 0), 0);
        const totalClicks = recentBroadcasts.reduce((sum: number, broadcast: any) => sum + (broadcast.stats?.clicks || 0), 0);
        const totalSent = recentBroadcasts.reduce((sum: number, broadcast: any) => sum + (broadcast.stats?.recipients || 0), 0);
