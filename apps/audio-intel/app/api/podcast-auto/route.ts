...
const CURATED_HIGH_RELEVANCE_EPISODES = [
  {
    ...
    aiInsights: ["AI can generate 50+ social media posts from one song", "Automated playlist pitching has 3x higher success rates"],
    techFeatures: ["Automated content generation for all platforms", "AI playlist curator matching"],
    actionableTips: ["Use AI to create content variations for each platform", "Automate your playlist pitching with AI tools"],
  ...
  },
  {
    ...
    musicIndustryRelevance: "High" as const,
    aiInsights: ["AI email sequences have 40% higher open rates", "Automated content creation saves 15+ hours per week"],
    techFeatures: ["Automated email sequence generation", "AI content calendar optimization"],
    actionableTips: ["Use AI to write your press release emails", "Automate your fan outreach sequences"],
  ...
  },
  {
    ...
    musicIndustryRelevance: "High" as const,
    aiInsights: ["AI can now generate full songs in under 2 minutes", "AI mastering tools rival professional engineers"],
    techFeatures: ["Suno's latest model supports 15+ genres", "AI mastering with professional quality"],
...
function parseRSSFeed(xmlText: string): PodcastFeed | null {
  try {
    ...
    const descriptionMatch = xmlText.match(/<description><!\[CDATA\[(.*?)\]\]><\/description>|<description>(.*?)<\/description>/);
    ...
    if (itemMatches) {
      itemMatches.slice(0, 10).forEach(item => { // Limit to 10 most recent
        const episodeTitleMatch = item.match(/<title><!\[CDATA\[(.*?)\]\]><\/title>|<title>(.*?)<\/title>/);
        const episodeDescMatch = item.match(/<description><!\[CDATA\[(.*?)\]\]><\/description>|<description>(.*?)<\/description>/);
...
function analyzeEpisodeForAI(title: string, description: string): {
  ...
  const aiKeywords = ['ai', 'artificial intelligence', 'machine learning', 'automation', 'chatgpt', 'gpt', 'claude', 'perplexity'];
  const techKeywords = ['technology', 'software', 'app', 'platform', 'tool', 'digital', 'online', 'algorithm'];
  const musicKeywords = ['music', 'artist', 'band', 'song', 'album', 'streaming', 'spotify', 'playlist', 'radio'];
...
export async function GET(request: NextRequest) {
  try {
    ...
    highRelevanceEpisodes.sort((a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime());
    mediumRelevanceEpisodes.sort((a, b) => new Date(b.publishedAt).getTime() - new Date(a.publishedAt).getTime());
    
    // Generate newsletter content
    const newsletterContent = generateNewsletterContent(highRelevanceEpisodes, mediumRelevanceEpisodes);
    
    console.log(`✅ Auto-update complete: ${allEpisodes.length} total episodes (${highRelevanceEpisodes.length} high-relevance, ${mediumRelevanceEpisodes.length} medium-relevance)`);
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        lowRelevance: allEpisodes.length - highRelevanceEpisodes.length - mediumRelevanceEpisodes.length
      },
...
function generateNewsletterContent(highRelevance: PodcastEpisode[], mediumRelevance: PodcastEpisode[]): string {
  ...
  content += '*Why this matters:* These are the latest episodes from industry podcasts. They often discover new AI features and technology trends before they become mainstream.*\n\n';
