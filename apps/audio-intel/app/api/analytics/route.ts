...
export async function POST(request: NextRequest) {
  try {
    ...
    if (custom_parameters?.funnel_step) {
      const funnelStep = analyticsData.conversionFunnel.find(
        step => step.step === custom_parameters.funnel_step
      );
      if (funnelStep) {
        funnelStep.count++;
        // Calculate conversion rate
        const previousStep =
          analyticsData.conversionFunnel[analyticsData.conversionFunnel.indexOf(funnelStep) - 1];
        if (previousStep) {
    ...
    switch (event) {
      case 'contact_enrichment':
        if (custom_parameters?.enrichedContacts && custom_parameters?.processingTime) {
          updateEnrichmentAnalytics(
            custom_parameters.enrichedContacts,
            custom_parameters.processingTime
          );
        }
        break;
