    ...
    const response = await fetch('https://api.perplexity.ai/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a music industry expert who analyzes podcast content for AI and technology insights relevant to independent artists and music professionals. Focus on actionable, practical advice.'
          },
          {
            role: 'user',
            content: prompt
...
function analyzeEpisodeFallback(episode: PodcastEpisode): PodcastEpisode {
  ...
  const aiKeywords = ['ai', 'artificial intelligence', 'machine learning', 'automation', 'chatgpt', 'claude', 'perplexity', 'suno', 'udio'];
  const techKeywords = ['tool', 'platform', 'software', 'app', 'feature', 'update', 'new', 'beta', 'launch'];
  const musicKeywords = ['music', 'artist', 'promotion', 'marketing', 'social media', 'spotify', 'tiktok', 'instagram'];
  
  const hasAI = aiKeywords.some(keyword => title.includes(keyword) || description.includes(keyword));
  const hasTech = techKeywords.some(keyword => title.includes(keyword) || description.includes(keyword));
  const hasMusic = musicKeywords.some(keyword => title.includes(keyword) || description.includes(keyword));
  ...
  const newsletterContent = relevance !== 'Low' 
  ...
  try {
    ...
    if (!feedUrl) {
      return NextResponse.json({
        ...
        example: '/api/podcast-monitor?feedUrl=https://feeds.apple.com/podcasts/your-podcast-feed'
      }, { status: 400 });
    }
    ...
    const highRelevanceEpisodes = analyzedEpisodes.filter(ep => ep.musicIndustryRelevance === 'High');
    const mediumRelevanceEpisodes = analyzedEpisodes.filter(ep => ep.musicIndustryRelevance === 'Medium');

    // Generate newsletter content
    const newsletterContent = generatePodcastNewsletterContent(highRelevanceEpisodes, mediumRelevanceEpisodes);

    console.log(`✅ Found ${highRelevanceEpisodes.length} high-relevance episodes, ${mediumRelevanceEpisodes.length} medium-relevance`);
    ...
    return NextResponse.json({
      ...
      stats: {
        ...
        lowRelevance: analyzedEpisodes.length - highRelevanceEpisodes.length - mediumRelevanceEpisodes.length
      },
  ...
  try {
    ...
    if (!Array.isArray(feedUrls) || feedUrls.length === 0) {
      return NextResponse.json({
        ...
        example: { feedUrls: ['https://feeds.apple.com/podcast1', 'https://feeds.apple.com/podcast2'] }
      }, { status: 400 });
    }
    ...
    const results = await Promise.all(
      feedUrls.map(async (feedUrl: string) => {
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3001'}/api/podcast-monitor?feedUrl=${encodeURIComponent(feedUrl)}`);
    ...
    const mediumRelevanceEpisodes = allEpisodes.filter(ep => ep.musicIndustryRelevance === 'Medium');
