...
async function fetchKitSubscribers(apiKey: string) {
  try {
    const response = await fetch(
      `https://api.convertkit.com/v3/subscribers?api_key=${apiKey}`,
      { method: 'GET' }
    );
    ...
    const weeklySignups = subscribers.filter((s: any) => 
...
function getDefaultMetrics() {
  return {
    ...
    competitiveIntel: { opportunities: 0, mentions: 0, threats: 0, lastScan: new Date().toISOString() }
  };
}