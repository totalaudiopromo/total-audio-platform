...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const totalScore = Object.values(agentMetrics).reduce((sum, agent: any) => sum + agent.performance_score, 0);
    reportData.overall_performance = Math.round(totalScore / Object.keys(agentMetrics).length);

    // Key Business Metrics
    reportData.key_metrics = {
      total_reddit_opportunities: Object.values(agentMetrics).reduce((sum: number, agent: any) => 
        sum + (agent.opportunities_found || 0), 0),
      email_engagement_rate: agentMetrics['Email Scheduler'].open_rate,
      content_viral_rate: Math.round((agentMetrics['Content Generator'].viral_content / agentMetrics['Content Generator'].content_pieces) * 100),
