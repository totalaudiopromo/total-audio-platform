...
async function generateNewsletterContent(): Promise<NewsletterContent> {
  ...
  const newsResponse = await fetch(
    `${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3001'}/api/newsjacker`
  );
...
Quality over quantity. Always.`,
  ...
  return {
    ...
    preview:
      "Spotify's new discovery features, TikTok algorithm changes, and why I'm automating my contact research (with real results).",
    sections,
    cta: 'Try Audio Intel Free (Beta)',
    personalNote:
      "Thanks for being part of The Unsigned Advantage community. Every week I'm sharing the insights that are actually moving the needle for indie artists. Keep hustling. - Chris",
  };
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!includeCaseStudy) {
      newsletterContent.sections = newsletterContent.sections.filter(
        section => section.type !== 'case_study'
      );
    }

    if (!includeNews) {
      newsletterContent.sections = newsletterContent.sections.filter(
        section => section.type !== 'news'
      );
    }
