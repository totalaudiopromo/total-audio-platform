...
import { ArrowRight, Music, Star, Users, CheckCircle, ChevronDown, ChevronUp, Zap, Upload, Download, User, Mail, Headphones, BarChart, Shield, Smile, Award, Briefcase, Globe, Phone, ExternalLink, Check, Play, Target } from "lucide-react";
...
export default function HomePageClient() {
  ...
  const [paymentPlan, setPaymentPlan] = useState<'starter' | 'professional' | 'agency' | null>(null);
  ...
  const handleEnrich = async () => {
    ...
    fetch('/api/analytics', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ event: 'demo_email_collected', data: { email: userEmail, timestamp: new Date().toISOString() } }),
    });
    try {
    ...
    try {
      ...
      if (data.success) {
        ...
        fetch('/api/analytics', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ event: 'email_sent', data: { email: userEmail, count: enriched.length, timestamp: new Date().toISOString() } }),
        });
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
          ...
          <Card className="border-4 border-black rounded-2xl shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-white bg-opacity-95 backdrop-blur-md hover:shadow-[10px_10px_0px_0px_rgba(0,0,0,1)] transition-all duration-200">
            <div className="flex justify-between items-center px-8 py-5">
              <div className="flex items-center gap-4">
                ...
                <span className="text-3xl font-black text-gray-900 tracking-tight">Audio Intel</span>
              </div>
              ...
              <nav className="hidden md:flex items-center gap-8">
                ...
                <a href="#faq" className="text-gray-700 hover:text-black font-bold text-lg transition-colors">
                  FAQ
                </a>
                <Button
                  variant="outline"
      ...
      <section className="w-full px-4 sm:px-6 lg:px-8 pt-24 pb-12 flex flex-col items-center text-center text-white relative overflow-hidden dark:bg-gray-900 max-w-screen-xl mx-auto">
        ...
        <div className="absolute inset-0 w-full h-full -z-10">
          <Image src={`/${currentGradient}`} alt="Luma gradient background" fill style={{ objectFit: 'cover', objectPosition: 'center' }} priority />
          <Image src={`/${currentTexture}`} alt="Magazine texture overlay" fill style={{ objectFit: 'cover', objectPosition: 'center', opacity: 0.3 }} priority />
        ...
        <div className="max-w-3xl mx-auto z-10">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-8 mb-8 relative w-full">
            <div className="bg-white bg-opacity-10 rounded-xl p-6 flex flex-col items-center shadow-lg min-w-0 w-full sm:w-auto dark:bg-white dark:bg-opacity-10">
              ...
              <div className="bg-white text-[#312e81] font-mono px-4 py-2 rounded-lg text-lg shadow-inner dark:bg-white dark:text-[#312e81] break-words">john@bbc.co.uk</div>
            </div>
        ...
        <Music className="absolute top-8 left-8 w-32 h-32 text-white/10 rotate-12" aria-label="music note background" />
        <Music className="absolute bottom-8 right-8 w-32 h-32 text-white/10 -rotate-12" aria-label="music note background" />
      </section>
        ...
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-bold text-white">New! Try Playlist Pulse for playlist curator discovery</h3>
                <p className="text-sm text-white/80">Get instant access to 50,000+ verified playlist curators with AI-powered pitch generation</p>
              </div>
            </div>
            <Link 
      ...
      <section className="w-full px-4 sm:px-6 lg:px-8 py-16 bg-white flex flex-col items-center dark:bg-gray-900 max-w-screen-xl mx-auto overflow-x-hidden">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-6 text-center dark:text-white break-words">The Music Industry Contact Research Nightmare</h2>
        <div className="flex flex-col sm:flex-row gap-8 max-w-4xl w-full justify-center">
          <div className="flex flex-col items-center text-center flex-1 min-w-0">
            ...
            <div className="font-semibold mb-1 dark:text-white break-words">Spending hours researching playlist curators and radio contacts manually</div>
          </div>
          <div className="flex flex-col items-center text-center flex-1 min-w-0">
            <Mail className="w-10 h-10 text-[#818cf8] mb-2" aria-label="Mail icon" />
            <div className="font-semibold mb-1 dark:text-white break-words">Missing crucial submission guidelines and contact preferences</div>
          </div>
          <div className="flex flex-col items-center text-center flex-1 min-w-0">
            <Headphones className="w-10 h-10 text-[#818cf8] mb-2" aria-label="Headphones icon" />
            <div className="font-semibold mb-1 dark:text-white break-words">Cold pitching without knowing their music taste or recent activity</div>
          </div>
        </div>
        <p className="mt-8 max-w-2xl text-center text-lg text-[#312e81]/80 dark:text-white/80 break-words">
          Music PR professionals waste 15+ hours per week on contact research that AI can do in minutes. Meanwhile, your competition is already pitching while you're still googling.
        </p>
      </section>

      {/* SOLUTION SECTION */}
      <section ref={howItWorksRef} className="w-full px-4 sm:px-6 lg:px-8 py-16 bg-gradient-to-b from-white to-[#ede9fe] flex flex-col items-center max-w-screen-xl mx-auto overflow-x-hidden">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-8 text-[#312e81] text-center dark:text-white break-words">How Audio Intel Works</h2>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-8 w-full max-w-4xl">
          <div className="flex flex-col items-center text-center flex-1 min-w-0">
            ...
            <div className="text-base dark:text-white break-words">Drop your email list (CSV, Excel, or paste emails)</div>
          </div>
          ...
          <div className="flex flex-col items-center text-center flex-1 min-w-0">
            ...
            <div className="text-base dark:text-white break-words">Our AI analyses each contact for music industry intelligence</div>
          </div>
          <ArrowRight className="w-8 h-8 text-[#818cf8] hidden sm:block" />
          <div className="flex flex-col items-center text-center flex-1 min-w-0">
            ...
            <div className="text-base dark:text-white break-words">Download complete contact profiles with pitch-ready insights</div>
          </div>
        </div>
      ...
      <section className="w-full px-4 sm:px-6 lg:px-8 py-16 bg-gradient-to-b from-[#ede9fe] to-white flex flex-col items-center dark:bg-gray-900 max-w-screen-xl mx-auto overflow-x-hidden">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-8 text-[#312e81] text-center dark:text-white break-words">See The Transformation</h2>
        <div className="flex flex-col sm:flex-row gap-8 items-center justify-center mb-8 w-full">
          <div className="bg-white rounded-xl p-6 shadow-lg min-w-0 w-full sm:w-auto dark:bg-white">
            ...
            <div className="bg-[#f1f5f9] text-[#312e81] font-mono px-4 py-2 rounded-lg text-lg shadow-inner dark:bg-[#f1f5f9] dark:text-[#312e81] break-words">john@bbc.co.uk</div>
          </div>
          <ArrowRight className="w-8 h-8 text-[#818cf8] hidden sm:block" />
          <div className="bg-white rounded-xl p-6 shadow-lg min-w-0 w-full sm:w-auto dark:bg-white">
      ...
      <section className="w-full px-4 sm:px-6 lg:px-8 py-16 bg-white flex flex-col items-center dark:bg-gray-900 max-w-screen-xl mx-auto overflow-x-hidden">
        ...
        <div className="flex flex-wrap gap-6 justify-center items-center opacity-60">
          <div className="w-28 h-10 bg-[#ede9fe] rounded flex items-center justify-center text-[#6366f1] font-bold text-sm dark:bg-gray-800 dark:text-white">BBC</div>
          <div className="w-28 h-10 bg-[#ede9fe] rounded flex items-center justify-center text-[#6366f1] font-bold text-sm dark:bg-gray-800 dark:text-white">Spotify</div>
          <div className="w-28 h-10 bg-[#ede9fe] rounded flex items-center justify-center text-[#6366f1] font-bold text-sm dark:bg-gray-800 dark:text-white">NME</div>
          <div className="w-28 h-10 bg-[#ede9fe] rounded flex items-center justify-center text-[#6366f1] font-bold text-sm dark:bg-gray-800 dark:text-white">Apple Music</div>
          <div className="w-28 h-10 bg-[#ede9fe] rounded flex items-center justify-center text-[#6366f1] font-bold text-sm dark:bg-gray-800 dark:text-white">Radio X</div>
        </div>
      </section>

      {/* PRICING SECTION */}
      <section className="w-full px-4 sm:px-6 lg:px-8 py-16 bg-gradient-to-b from-[#ede9fe] to-white flex flex-col items-center dark:bg-gray-900 max-w-screen-xl mx-auto overflow-x-hidden">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-2 text-[#312e81] text-center dark:text-white break-words">Simple, Transparent Pricing</h2>
        <p className="text-lg text-[#312e81]/80 mb-8 text-center dark:text-white/80 break-words">Start your <span className='font-bold'>7-day free trial</span>. Credit card required. All features unlocked during trial. You’ll be charged automatically at the end of your trial unless you cancel. Cancel anytime during your trial to avoid charges. We’ll send you a reminder before your trial ends.</p>
        {/* Pricing toggle */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl w-full mb-4">
          <PricingCard
            title="Free Forever"
            price="£0"
            features={["5 contact enrichments per month", "Perfect for testing and small projects", "All core features included"]}
            cta="Start Free Now"
            highlight={false}
            onClick={() => {
              setModalOpen(true);
          ...
          <PricingCard
            ...
            features={["50 contact enrichments per month", "All AI research features", "Export to CSV, Excel", "Email support", "Perfect for: Independent artists, small labels"]}
          ...
          <PricingCard
            ...
            features={["200 contact enrichments per month", "Priority processing", "Advanced export options", "CRM integrations", "Perfect for: Music PR agencies, growing labels"]}
            cta={pricingMode === 'monthly' ? 'Start Free Trial' : 'Start Free Trial'}
            highlight={true}
            onClick={() => {
          ...
          <PricingCard
            ...
            features={["500 contact enrichments per month", "Fastest processing", "White-label exports", "Phone + email support", "Perfect for: Full-service PR agencies"]}
            cta={pricingMode === 'monthly' ? 'Start Free Trial' : 'Start Free Trial'}
            highlight={false}
            onClick={() => {
        ...
        <div className="text-[#312e81]/80 text-sm mt-2 dark:text-white/80 break-words">{pricingMode === 'yearly' ? 'Billed yearly. Save 20%!' : 'Save 20% with annual billing'}</div>
        ...
        {showSignupModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-xl shadow-xl p-8 max-w-sm w-full text-center relative dark:bg-gray-800">
              <button className="absolute top-4 right-4 text-gray-400 hover:text-gray-700 text-2xl font-bold dark:text-white" onClick={() => setShowSignupModal(false)}>&times;</button>
              <h2 className="text-2xl font-bold mb-4 text-[#312e81] dark:text-white break-words">Signup Coming Soon</h2>
              <p className="mb-4 text-[#312e81]/80 text-center dark:text-white/80">Paid plan signup and checkout is launching soon. For early access, <a href="mailto:info@audiointel.com" className="underline text-[#6366f1] dark:text-brand-blue">contact us</a>.</p>
              <button className="px-6 py-2 min-h-[44px] rounded-lg font-semibold bg-[#6366f1] text-white hover:bg-[#818cf8] transition-colors dark:bg-brand-blue dark:hover:bg-brand-blue-dark" onClick={() => setShowSignupModal(false)}>Close</button>
            </div>
          </div>
        )}
      </section>
      ...
      {/* FAQ SECTION */}
      <section className="w-full px-4 sm:px-6 lg:px-8 py-16 bg-white flex flex-col items-center dark:bg-gray-900 max-w-screen-xl mx-auto overflow-x-hidden">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-8 text-[#312e81] text-center dark:text-white break-words">Frequently Asked Questions</h2>
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)} className="fixed z-50 inset-0 overflow-y-auto">
        <div className="flex items-center justify-center min-h-screen px-4">
          ...
          <Dialog.Panel className="relative bg-white rounded-xl shadow-xl max-w-lg w-full mx-auto p-8 z-10 dark:bg-gray-800">
            <button className="absolute top-4 right-4 text-gray-400 hover:text-gray-700 text-2xl font-bold dark:text-white" onClick={() => setModalOpen(false)}>&times;</button>
            {/* Email gating step */}
            {!userEmail ? (
              <div className="flex flex-col items-center">
                <h2 className="text-2xl font-bold mb-4 text-[#312e81] text-center dark:text-white">Try Audio Intel Demo</h2>
                <p className="mb-4 text-[#312e81]/80 text-center dark:text-white/80">Enter your email to access the free demo (no spam, just one-time access link).</p>
                <input
                  type="email"
                ...
                <div className="text-xs text-gray-400 mt-2 dark:text-white/80">No spam. Required for demo access.</div>
                <div className="w-full flex flex-col items-center mt-6">
                  <div className="text-sm text-[#312e81]/80 dark:text-white/80 mb-2">Want to sign up for the full experience?</div>
                  <button
                  ...
                  <div className="text-xs text-gray-400 mt-2 dark:text-white/80 text-center">7-day free trial. Credit card required. All features unlocked. You’ll be charged automatically at the end of your trial unless you cancel. Cancel anytime. We’ll send you a reminder before your trial ends.</div>
                </div>
              </div>
            ) : (
              // Demo UI (existing modal content, but remove email input from results area)
              <div>
                <h2 className="text-2xl font-bold mb-4 text-[#312e81] text-center dark:text-white">Try Audio Intel Free</h2>
                <p className="mb-6 text-[#312e81]/80 text-center max-w-xl dark:text-white/80">Upload or paste up to 5 music industry emails and see real AI-powered enrichment in action. No signup required.</p>
                <div className="w-full flex flex-col gap-4">
                  <div className="mb-2 text-[#6366f1] text-center font-semibold text-base dark:text-white">
                    Get exclusive music industry tips, new feature updates, and your free enrichment results. No spam, unsubscribe anytime.
                  </div>
                  <input
                    type="email"
                  ...
                  <div className="text-center text-[#6366f1] font-semibold dark:text-white">or paste emails below</div>
                  <textarea
                  ...
                  />
                  {showUpgrade && (
                    <div className="text-red-600 text-sm font-semibold mb-2 dark:text-white/80">Free demo is limited to 5 contacts. <span className="underline cursor-pointer" onClick={() => window.scrollTo({top: document.body.scrollHeight, behavior: 'smooth'})}>See pricing</span> for more.</div>
                  )}
                  <button
                  ...
                  >
                    {loading ? "Enriching..." : `Enrich ${contacts.length} Contact${contacts.length !== 1 ? "s" : ""}`}
                  </button>
                  {loading && <ProgressBarWithDog current={progress} total={contacts.length} />}
                  {error && <div className="text-red-600 text-sm mt-2 dark:text-white/80">{error}</div>}
                  {enriched.length > 0 && !loading && (
                    <div className="mt-8 flex flex-col items-center">
                      <div className="mb-4 text-[#312e81] text-center font-semibold dark:text-white">Want more than 5 enrichments? Unlock unlimited access and advanced features.</div>
                      ...
                      <div className="text-xs text-[#6366f1] dark:text-white">Payments powered by Stripe. Secure & instant access.</div>
                    </div>
                  )}
                  {enriched.length > 0 && !loading && (
                    <div className="mt-6">
                      <h3 className="font-bold mb-2 text-[#312e81] dark:text-white">Enriched Results</h3>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {enriched.map((c, idx) => (
                          <div key={idx} className="bg-white rounded-xl shadow-lg p-6 flex flex-col gap-2 border border-[#ede9fe] dark:bg-gray-800 dark:border-gray-700">
                            <div className="flex items-center gap-3 mb-2">
                              <User className="w-6 h-6 text-[#6366f1]" />
                              <span className="font-bold text-lg text-[#312e81] dark:text-white">{c.name || <span className="text-gray-400 dark:text-gray-500">—</span>}</span>
                            </div>
                      ...
                      {showEmail && (
                        <div className="mt-4 flex flex-col items-center">
                          ...
                          {emailSent && <div className="text-green-600 text-sm mt-2 dark:text-white/80">Email sent!</div>}
                        </div>
                      )}
                    </div>
                  )}
      ...
      {paymentPlan && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-8 max-w-sm w-full text-center relative dark:bg-gray-800">
            <button className="absolute top-4 right-4 text-gray-400 hover:text-gray-700 text-2xl font-bold dark:text-white" onClick={() => setPaymentPlan(null)}>&times;</button>
            <h2 className="text-2xl font-bold mb-4 text-[#312e81] dark:text-white">Subscribe to {paymentPlan.charAt(0).toUpperCase() + paymentPlan.slice(1)} Plan</h2>
            <input
              type="email"
              className="border border-[#ede9fe] rounded-lg p-3 text-base mb-4 w-full max-w-xs dark:bg-gray-700 dark:border-gray-600 dark:text-white"
      ...
      <section className="w-full px-4 sm:px-6 lg:px-8 py-16 bg-white flex flex-col items-center dark:bg-gray-900 max-w-screen-xl mx-auto overflow-x-hidden">
        ...
        <div className="space-y-4 max-w-4xl w-full">
          {faqs.map((faq, index) => (
            <div key={index} className="bg-white border-4 border-gray-300 rounded-2xl p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] hover:-translate-x-1 hover:-translate-y-1 transition-all dark:bg-gray-800 dark:border-gray-700">
              <button 
              ...
              {openFaq === index && (
                <div className="mt-4 text-[#312e81]/80 dark:text-white/80 font-bold">
                  {faq.a}
                </div>
              )}
            </div>
          ...
          <div className="mb-8 p-6 bg-gradient-to-r from-yellow-400/10 to-yellow-600/10 border border-yellow-400/20 rounded-xl">
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-bold text-[#312e81] dark:text-white">Need playlist curator discovery?</h3>
                  <p className="text-[#312e81]/80 dark:text-white/80">Try Playlist Pulse - our sister tool for finding and pitching playlist curators</p>
                </div>
              </div>
              <Link 
                href="https://pulse.totalaudiopromo.com" 
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-[#312e81]/80 dark:text-white/80">
                ...
                <li><a href="mailto:info@audiointel.com" className="hover:text-[#6366f1] dark:hover:text-brand-blue transition-colors">Contact Support</a></li>
                <li><a href="#" className="hover:text-[#6366f1] dark:hover:text-brand-blue transition-colors">Privacy Policy</a></li>
                <li><a href="#" className="hover:text-[#6366f1] dark:hover:text-brand-blue transition-colors">Terms of Service</a></li>
              </ul>
            </div>
          </div>
        </div>
...
const PricingCard: React.FC<{ title: string; price: string; features: string[]; cta: string; highlight: boolean; onClick: () => void; showAnnualBadge?: boolean }> = ({ title, price, features, cta, highlight, onClick, showAnnualBadge }) => {
  return (
    <div className={`flex flex-col items-center rounded-2xl p-8 shadow-lg border-2 ${highlight ? "border-[#6366f1] bg-[#ede9fe] scale-105 z-10 dark:bg-gray-800" : "border-[#ede9fe] bg-white dark:bg-gray-800 dark:border-gray-700"} transition-transform relative`}>
      {highlight && <div className="mb-2 px-3 py-1 bg-[#6366f1] text-white text-xs rounded-full font-bold uppercase tracking-wider dark:bg-[#6366f1] dark:text-white">Most Popular</div>}
      {showAnnualBadge && (
        <div className="absolute top-4 right-4 bg-green-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-lg z-20 animate-bounce dark:bg-green-500 dark:text-white">Save 20%</div>
      )}
      <div className="text-2xl font-bold mb-2 text-[#312e81] dark:text-white">{title}</div>
      <div className="text-3xl font-extrabold mb-4 text-[#6366f1] dark:text-brand-blue">{price}</div>
      <ul className="mb-6 space-y-2 text-[#312e81]/90 dark:text-white">
        {features.map((f, i) => (
          <li key={i} className="flex items-center gap-2"><CheckCircle className="w-5 h-5 text-[#6366f1] dark:text-brand-blue" /> {f}</li>
        ))}
      </ul>
      <button
