# Content Performance Tracker

## Simple Tracking Template

### Post Performance Log

| Date       | Platform | Topic            | Type             | Engagement           | Clicks | Notes           |
| ---------- | -------- | ---------------- | ---------------- | -------------------- | ------ | --------------- |
| 2025-01-16 | LinkedIn | Contact research | Problem-focused  | 15 likes, 3 comments | 5      | Good engagement |
| 2025-01-16 | Blue Sky | Time-saving tips | Solution-focused | 8 likes, 2 replies   | 2      | Decent reach    |
| 2025-01-17 | LinkedIn | Personal story   | Story-focused    | 22 likes, 7 comments | 8      | High engagement |
| 2025-01-17 | Blue Sky | Industry insight | Insight-focused  | 12 likes, 4 replies  | 3      | Good discussion |

## Engagement Metrics to Track

### LinkedIn

- **Likes**: Target 10+ per post
- **Comments**: Target 3+ per post
- **Shares**: Target 1+ per post
- **Clicks**: Track via UTM parameters
- **New Connections**: Track weekly

### Blue Sky

- **Likes**: Target 5+ per post
- **Replies**: Target 2+ per post
- **Reposts**: Target 1+ per post
- **Clicks**: Track via UTM parameters
- **New Followers**: Track weekly

### X (When Added)

- **Likes**: Target 15+ per post
- **Retweets**: Target 3+ per post
- **Replies**: Target 5+ per post
- **Clicks**: Track via UTM parameters
- **New Followers**: Track weekly

## Content Type Performance

### Problem-Focused Content

- **Best for**: LinkedIn, X
- **Typical engagement**: High
- **Best times**: Monday morning, Wednesday afternoon
- **Notes**: Resonates with pain points

### Solution-Focused Content

- **Best for**: All platforms
- **Typical engagement**: Medium-High
- **Best times**: Tuesday morning, Thursday afternoon
- **Notes**: Shows value proposition

### Story-Focused Content

- **Best for**: LinkedIn, X
- **Typical engagement**: High
- **Best times**: Wednesday morning, Friday afternoon
- **Notes**: Builds personal connection

### Competitive Content

- **Best for**: LinkedIn, X
- **Typical engagement**: Medium
- **Best times**: Thursday morning, Friday afternoon
- **Notes**: Positions against competitors

## Weekly Performance Summary

### Week of: [DATE]

#### LinkedIn Performance

- **Posts**: X
- **Total Engagement**: X likes, X comments, X shares
- **Average Engagement**: X per post
- **Clicks to Website**: X
- **New Connections**: X

#### Blue Sky Performance

- **Posts**: X
- **Total Engagement**: X likes, X replies, X reposts
- **Average Engagement**: X per post
- **Clicks to Website**: X
- **New Followers**: X

#### Overall Performance

- **Total Posts**: X
- **Total Engagement**: X
- **Website Clicks**: X
- **Beta Signups**: X
- **New Followers**: X

## Content Ideas That Worked

### High-Performing Posts

1. **"Stop wasting your weekends researching contacts"** - 25+ engagement
2. **"Built by a producer who hated manual research"** - 30+ engagement
3. **"94% of research time is wasted on outdated info"** - 20+ engagement

### What Made Them Work

- **Clear problem statement**
- **Personal story element**
- **Specific statistics**
- **Relatable pain point**
- **Strong call-to-action**

## Content Ideas That Didn't Work

### Low-Performing Posts

1. **"Generic industry advice"** - 5 engagement
2. **"Too technical explanation"** - 3 engagement
3. **"No clear value proposition"** - 4 engagement

### What Went Wrong

- **Too generic**
- **Too technical**
- **No clear benefit**
- **Weak call-to-action**
- **Wrong timing**

## Optimization Tips

### Based on Performance

1. **Problem-focused content** performs best on LinkedIn
2. **Personal stories** get highest engagement
3. **Specific statistics** increase credibility
4. **Monday morning** posts get most engagement
5. **Questions** increase comments and replies

### Content Improvements

1. **Always include specific statistics**
2. **Use personal story elements**
3. **End with engaging questions**
4. **Post at optimal times**
5. **Include clear call-to-action**

## Monthly Review

### Month: [MONTH]

#### Goals vs Results

- **Goal**: X posts per week
- **Actual**: X posts per week
- **Goal**: X engagement per post
- **Actual**: X engagement per post
- **Goal**: X beta signups
- **Actual**: X beta signups

#### What Worked

- [Success 1]
- [Success 2]
- [Success 3]

#### What to Improve

- [Improvement 1]
- [Improvement 2]
- [Improvement 3]

#### Next Month Focus

- [Focus 1]
- [Focus 2]
- [Focus 3]

## Quick Performance Check

### Daily (5 minutes)

- [ ] Check engagement on yesterday's posts
- [ ] Note any comments or replies
- [ ] Track clicks via UTM parameters
- [ ] Update performance log

### Weekly (15 minutes)

- [ ] Review all posts from the week
- [ ] Identify best and worst performers
- [ ] Plan next week's content based on insights
- [ ] Update content ideas list

### Monthly (30 minutes)

- [ ] Calculate total engagement metrics
- [ ] Compare to previous month
- [ ] Identify trends and patterns
- [ ] Plan content strategy for next month
