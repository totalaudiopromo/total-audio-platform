...
export const TextureOverlay: React.FC<TextureOverlayProps> = ({
  ...
  const cssTextureClass = textureType === 'paper' ? 'texture-paper-css' : 
                         textureType === 'grain' ? 'texture-grain-css' : 
                         'texture-luma-css';
...
export const TextureCard: React.FC<{
  ...
  const baseClasses = 'relative bg-white rounded-xl shadow-texture-soft transition-all duration-500 ease-out overflow-hidden';
  const animationClasses = floating ? 'animate-texture-float' : entrance ? 'animate-texture-entrance' : '';
