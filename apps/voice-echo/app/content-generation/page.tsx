...
export default function ContentGenerationPage() {
  ...
  const [qualityScores, setQualityScores] = useState<{[key: string]: {authenticity: number, engagement: number}}>({})
  ...
  const transformForTwitter = (input: string) => {
    ...
    if (base.includes('preggers') || base.includes('buzzin')) {
      ...
      transformed += " Been working on this for months and finally ready to share it with you all. Link in bio 👍"
      ...
      transformed = `🎉 ${transformed}`
      transformed += " The response has been incredible! Thank you all for the love. Link in bio 👆"
  ...
  const transformForInstagram = (input: string) => {
    ...
    if (base.includes('preggers') || base.includes('buzzin')) {
      ...
      transformed += " This one's been brewing for months and I'm so excited to finally share it with you all! Link in bio for the full story 🙌"
    } else if (base.includes('released') || base.includes('new')) {
      transformed = `Behind the scenes of my new single 🎵 ${transformed}`
      transformed += " This one's been brewing for months and I'm so excited to finally share it with you all! Link in bio for the full story 🙌"
    } else if (base.includes('feedback') || base.includes('amazing')) {
      transformed = `The journey behind this track 📸 ${transformed}`
      transformed += " Every late night in the studio, every moment of doubt, every breakthrough - it's all been worth it. Link in bio for the full story 🙌"
    } else {
      transformed = `Behind the scenes of my new single 🎵 ${transformed}`
      transformed += " This one's been brewing for months and I'm so excited to finally share it with you all! Link in bio for the full story 🙌"
    }
  ...
  const transformForThreads = (input: string) => {
    ...
    if (base.includes('preggers') || base.includes('buzzin')) {
      ...
      transformed += " This track has been my baby for the past few months. Can't wait for you to hear it. Link in bio 👆"
    } else if (base.includes('released') || base.includes('new')) {
      transformed = `New music alert! 🚨 ${transformed}`
      transformed += " This track has been my baby for the past few months. Can't wait for you to hear it. Link in bio 👆"
    } else if (base.includes('feedback') || base.includes('amazing')) {
      transformed = `The response has been incredible! 🎉 ${transformed}`
      transformed += " Every message, every share, every comment - you've made this release so special. Link in bio 👆"
    } else {
      transformed = `New music alert! 🚨 ${transformed}`
      transformed += " This track has been my baby for the past few months. Can't wait for you to hear it. Link in bio 👆"
    }
  ...
  const transformForTikTok = (input: string) => {
    ...
    if (base.includes('preggers') || base.includes('buzzin')) {
      ...
      transformed += " Months of work finally ready for your ears 👂 Link in bio #NewMusic #UKArtist #BritishMusic"
    } else if (base.includes('released') || base.includes('new')) {
      transformed = `New single just dropped! 🎵 ${transformed}`
      transformed += " Months of work finally ready for your ears 👂 Link in bio #NewMusic #UKArtist"
    } else if (base.includes('feedback') || base.includes('amazing')) {
      ...
      <section className="container px-4 py-24 mx-auto text-center">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-xl text-gray-600 mb-12 max-w-3xl mx-auto leading-relaxed font-bold">
            See how Voice Echo takes your basic music news and transforms it into authentic, platform-optimized content that sounds exactly like you.
          </p>
        </div>
      </section>
            ...
            <Card className="border-4 border-purple-500 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] bg-gradient-to-br from-purple-50 to-white">
              <CardHeader>
                ...
                <CardDescription className="text-lg font-bold text-purple-700">
                  {isAnalyzingVoice ? 'AI is analyzing your writing patterns and personality traits' : 'AI has learned your authentic voice patterns and personality'}
                </CardDescription>
              </CardHeader>
              <CardContent>
                {isAnalyzingVoice && (
                  <div className="mb-6">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-bold text-purple-700">Voice Analysis Progress</span>
                      <span className="text-sm font-bold text-purple-600">{voiceAnalysisProgress}%</span>
                    </div>
                    ...
                    <div className="mt-3 text-sm text-purple-600 font-bold">
                      {voiceAnalysisProgress >= 20 && voiceAnalysisProgress < 40 && 'Analyzing writing patterns...'}
                      {voiceAnalysisProgress >= 40 && voiceAnalysisProgress < 60 && 'Detecting personality traits...'}
                      {voiceAnalysisProgress >= 60 && voiceAnalysisProgress < 80 && 'Learning voice characteristics...'}
                      {voiceAnalysisProgress >= 80 && voiceAnalysisProgress < 100 && 'Optimizing for platforms...'}
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {Object.entries(voiceAnalysis).map(([key, value]) => (
                    <div key={key} className="bg-white p-4 rounded-lg border-2 border-purple-200 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                ...
                {!isAnalyzingVoice && (
                  <div className="mt-6 p-4 bg-green-50 border-2 border-green-200 rounded-lg">
                    ...
                    <p className="text-sm text-green-600">
                      Your voice profile has been successfully learned. Content will now be generated in your authentic voice.
                    </p>
                  </div>
                )}
              </CardContent>
                          ...
                          <div className="flex justify-between items-center">
                            <div className="flex gap-4">
                              <div className="text-center">
                                <div className="text-lg font-black text-purple-600">{scores.authenticity}%</div>
                                <div className="text-xs font-bold text-gray-600">Authenticity</div>
                              </div>
                              <div className="text-center">
                                <div className="text-lg font-black text-green-600">{scores.engagement}%</div>
              ...
              {Object.keys(generatedContent).length > 0 && (
                <Card className="border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                  <CardHeader>
                    ...
                    <CardDescription className="text-lg font-bold">
                      See how Voice Echo transforms your basic input into authentic, platform-optimized content
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    ...
                    <div className="bg-purple-50 border-2 border-purple-200 rounded-lg p-4">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="font-bold text-purple-700">Voice Transformation Applied:</span>
                      </div>
                      <div className="text-sm text-purple-800 space-y-1">
                        <div>• Added passion markers (🔥) and authentic voice elements</div>
                        <div>• Applied platform-specific optimization (Twitter: concise, Instagram: visual storytelling)</div>
                        <div>• Enhanced with strategic emoji usage and industry hashtags</div>
                        <div>• Maintained your core message while amplifying engagement potential</div>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <span className="text-gray-700"> Analyzes 10-15 of your best posts to understand your unique style</span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <CheckCircle className="w-5 h-5 text-green-500 mt-1 flex-shrink-0" />
                    <div>
                      ...
                      <span className="text-gray-700"> Adapts your voice for each platform's unique requirements</span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <CheckCircle className="w-5 h-5 text-green-500 mt-1 flex-shrink-0" />
                    <div>
                      ...
                      <span className="text-gray-700"> Shows how well the content matches your voice (95%+ typical)</span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <CheckCircle className="w-5 h-5 text-green-500 mt-1 flex-shrink-0" />
                    <div>
                      ...
                      <span className="text-gray-700"> Content that sounds exactly like you, not ChatGPT</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
