...
export default function PitchCustomization() {
  ...
  const generatePreviewPitch = () => {
    ...
    if (professionalism[0] > 70) {
      pitch += "I've been following your [Playlist Name] and deeply appreciate your curatorial expertise in the [Genre] space. "
    } else {
      pitch += "I've been vibing with your [Playlist Name] playlist and love your taste in [Genre] music! "
    }
    
    // Personal touch
    if (personalTouch[0] > 60) {
      pitch += "Your track selection demonstrates a clear understanding of what your [Follower Count] followers connect with emotionally. "
    }
    
    // Creative elements based on creativity setting
    if (creativity[0] > 70) {
      pitch += "My latest track is a sonic journey that weaves [Musical Elements] with unexpected twists that I believe would create those perfect playlist moments your audience craves. "
    } else {
      pitch += "My latest track combines [Musical Elements] that would fit perfectly with your playlist's aesthetic. "
    }
    
    // Professional closing
    if (professionalism[0] > 60) {
      pitch += "I'd be honored to have you consider it for inclusion. Thank you for your time and for the incredible work you do in music discovery."
    } else {
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  />
                  <div className="text-white/60 text-xs mt-1">
                    Statistics and metrics emphasis
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                ...
                <div className="text-white/70 text-sm">
                  {state.pitchSettings.professionalism[0] > 70 ? 'Very Professional' :
                   state.pitchSettings.professionalism[0] > 40 ? 'Balanced' : 'Casual & Friendly'}
                </div>
              </div>
              <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                <div className="text-white font-medium mb-2">Expected Response</div>
                <div className="text-white/70 text-sm">
                  {(state.pitchSettings.professionalism[0] + state.pitchSettings.personalTouch[0]) / 2 > 60 ? 
