...
export default function PricingSection({ className }: PricingSectionProps) {
  ...
  useEffect(() => {
    ...
    if (pricingConfig?.pricing) {
      ...
      const updatedPlans = basePlans.map(plan => ({
        ...
        annualPrice: monthlyPrices[plan.id] * (1 - annualDiscounts[plan.id] / 100) || plan.annualPrice
      }));
      ...
      return (
        <Badge 
          className={cn(
            ...
            badges.mostPopular.position === 'top-center' && '-top-2 left-1/2 transform -translate-x-1/2',
            ...
            badges.mostPopular.color === 'gradient' && 'bg-gradient-to-r from-purple-500 to-pink-500 text-white'
          )}
        >
  ...
  const formatPrice = (plan: PricingPlan) => {
    ...
    const savings = isAnnual ? Math.round(((plan.monthlyPrice - plan.annualPrice) / plan.monthlyPrice) * 100) : 0;
          ...
          {plans.map((plan) => (
            ...
            >
              ...
              <Button
                ...
                onClick={() => handleCtaClick(plan.id, isAnnual ? plan.annualPrice : plan.monthlyPrice)}
              >
                {plan.id === 'starter' ? 'Start Free Trial' :
                 plan.id === 'professional' ? 'Get Professional' :
                 'Contact Sales'}
              </Button>
        ...
        <div className="mt-16 text-center">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            <div className="text-center">
              ...
              <p className="text-gray-400 text-sm">Start immediately with no hidden costs or setup charges</p>
            </div>
            <div className="text-center">
              ...
              <p className="text-gray-400 text-sm">No long-term commitments. Cancel or change plans anytime</p>
            </div>
            <div className="text-center">
              ...
              <p className="text-gray-400 text-sm">Get help from our UK-based music industry experts</p>
            </div>
          </div>
        </div>
