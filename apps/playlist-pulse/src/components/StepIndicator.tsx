...
export default function StepIndicator() {
  ...
  const getStepStatus = (stepId: string) => {
    if (stepId === 'upload') {
      return state.uploadComplete ? 'completed' : state.currentStep === 'upload' ? 'current' : 'upcoming'
    }
    if (stepId === 'customize') {
      if (state.showResults) return 'completed'
      return state.currentStep === 'customize' ? 'current' : state.uploadComplete ? 'upcoming' : 'disabled'
    }
    if (stepId === 'results') {
