          ...
          <div className="bg-gradient-to-br from-purple-500/20 via-blue-500/20 to-indigo-500/20 backdrop-blur-xl border border-white/30 rounded-3xl p-8 mb-8 shadow-2xl">
            <div className="flex items-center space-x-6 mb-6">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-xl font-bold text-white mb-3 truncate" title={state.uploadedFile.name}>
                  {state.uploadedFile.name}
                </h3>
                <div className="flex items-center space-x-4 text-white/90 text-sm">
                  <span className="bg-white/15 backdrop-blur-xl px-3 py-2 rounded-xl border border-white/20">{formatFileSize(state.uploadedFile.size)}</span>
                  <span className="bg-white/15 backdrop-blur-xl px-3 py-2 rounded-xl border border-white/20">{state.uploadedFile.type.split('/')[1].toUpperCase()}</span>
                  <span className="bg-white/15 backdrop-blur-xl px-3 py-2 rounded-xl border border-white/20">{formatDuration(state.uploadedFile)}</span>
                </div>
              </div>
            </div>
            ...
            {state.isUploading && (
              <div className="mt-6">
                ...
                />
                <p className="text-white/90 text-sm mt-3 font-semibold">{Math.round(state.uploadProgress)}% uploaded</p>
              </div>
            )}
          </div>
        </CardContent>
      ...
      <Card className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-8 shadow-2xl">
        <CardContent className="p-0">
          ...
          <div className="text-center">
            ...
            <p className="text-white/90 mb-8 text-xl font-medium">Your track has been analyzed and is ready for promotion</p>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-6 flex-1 min-w-0">
                  ...
                  <div className="text-left flex-1 min-w-0">
                    <div className="text-white font-bold text-lg truncate" title={state.uploadedFile.name}>
                      {state.uploadedFile.name}
                    </div>
                    <div className="text-white/80 text-sm mt-1">
                      {formatFileSize(state.uploadedFile.size)} • {formatDuration(state.uploadedFile)}
                    </div>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-2 gap-6 text-sm">
              <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
                ...
                <div className="text-white font-bold text-lg">{state.uploadedFile.type.split('/')[1].toUpperCase()}</div>
              </div>
              <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
