# 🎯 RADIO PROMOTER OUTREACH LIST - October 2025

*UK Music PR Agencies - Priority Targets for Audio Intel*

---

## 🎵 OUTREACH STRATEGY

**Value Prop**: "15 hours → 3 minutes" contact research time savings
**Demo Format**: Loom video (3 minutes, record once, use everywhere)
**Offer**: 10 free enrichments, no demo call needed
**Pricing**: £19/month PRO tier (UK pricing advantage)

**Personal Credibility**:
- 5+ years radio promotion experience
- Pitched to BBC Radio 6 Music, Amazing Radio, regional stations
- Producer (sadact)
- Brighton-based

---

## 🥇 PRIORITY #1: SHY CAT PR (Personal Connection)

**Contact**: Simon Russell
**Website**: https://shycatpr.com
**Services**: BBC Radio plugging, commercial radio promotion
**Location**: UK-based

**Why Target**:
- **PERSONAL CONNECTION**: Chris used to work with Simon
- Active in radio promotion space
- Focuses on BBC Radio and commercial stations
- Likely dealing with contact research challenges daily

**Personalization Notes**:
- Reference past working relationship with Simon
- "I used to work with you at [context]"
- Mention specific campaigns or artists from that time
- Warm introduction, not cold outreach

**Outreach Template**:
```
Subject: Simon - built something that saves me 15+ hours per campaign

Hi Simon,

Chris here - we used to work together at [context]. Hope you're doing well.

Since then I've been doing radio promotion (5+ years now, pitched to BBC Radio 6 Music,
Amazing Radio, loads of regional stations) and producing as sadact.

Quick one: I was spending 15+ hours per campaign researching, verifying, and organizing
station contacts. Built a tool that cut it to 3 minutes.

Thought you might find it useful given the volume you're probably handling at Shy Cat.

Made a 3-minute Loom showing how it works with real contacts: [Loom link]

If it's useful, you can try 10 enrichments free (no demo call needed): [link]

Would be great to catch up regardless. How's Shy Cat going?

Chris
Brighton
```

---

## 🥈 PRIORITY #2: DECENT MUSIC PR (User Specified)

**Website**: https://decentmusicpr.com
**Services**: Radio promotion for independent artists
**Location**: UK-based
**Contact**: [Research needed - check website for team page]

**Why Target**:
- User specifically mentioned by name
- Focuses on independent artists (ideal customer profile)
- UK-based (£ pricing advantage)
- Radio promotion core service

**Personalization Notes**:
- Research their recent campaigns/artists
- Mention specific genre or style they work with
- Reference their blog posts or social media content

**Outreach Template**:
```
Subject: Fellow radio promoter - 15 hours → 3 minutes

Hi [Name],

Chris here - I'm a radio promoter (5+ years, pitched to BBC Radio 6 Music,
Amazing Radio, regional stations) and producer (sadact).

Spotted Decent Music PR's work with [recent artist/campaign]. Impressive results.

Quick question: How much time are you spending on contact research per campaign?

I was spending 15+ hours researching, verifying, and organizing station contacts.
Built a tool that cut it to 3 minutes. Thought you might find it useful.

Made a 3-minute demo with real contacts: [Loom link]

Try 10 enrichments free if you're interested: [link]

No sales pitch - just built something that solved my own research nightmare.

Chris
Brighton, UK
```

---

## 🥉 PRIORITY #3: PRESCRIPTION PR

**Website**: https://prescriptionpr.co.uk
**Locations**: London, Cambridge, Manchester
**Services**: Cost-effective radio campaigns for indie artists
**Genres**: Indie, rock, electronic, folk
**Contact**: [Research needed - check team page]

**Why Target**:
- Multi-location UK agency
- Cost-effective positioning (price-conscious, might appreciate £19/month tool)
- Wide genre coverage
- Active in independent music scene

**Personalization Notes**:
- Reference their "cost-effective campaigns" positioning
- Mention specific city office location if you have local connection
- Reference genre work that matches your experience

**Outreach Template**:
```
Subject: Cost-effective contact research for Prescription PR

Hi [Name],

Chris here - radio promoter (5+ years) and producer based in Brighton.

Saw Prescription PR's focus on cost-effective campaigns for indie artists.
That resonated - I'm all about efficiency.

Built a tool that cuts contact research from 15+ hours to 3 minutes per campaign.
Saves me around £500+ in time per campaign.

Thought it might fit your cost-effective approach: [Loom demo link]

Try 10 free enrichments: [link]

£19/month vs 15+ hours manual research - the ROI is ridiculous.

Chris
Brighton, UK
```

---

## 🎯 PRIORITY #4: LANDER PR

**Website**: [Research needed from search results]
**Services**: Radio promotion for independent artists
**Location**: UK-based
**Contact**: [Research needed]

**Why Target**:
- UK-based radio promotion focus
- Independent artist market
- Likely managing large contact databases

**Personalization Notes**:
- Research recent campaigns or artists
- Check social media presence
- Look for blog posts or case studies

**Outreach Template**:
```
Subject: Radio promoter here - contact research automation

Hi [Name],

Chris - radio promoter (5+ years, BBC Radio 6 Music/Amazing Radio pitches) and producer.

Spotted Lander PR's work in the radio promotion space.

Built something that might save you serious time: contact research tool that goes
from 15+ hours to 3 minutes per campaign.

3-minute demo: [Loom link]
Try 10 free: [link]

No demo call needed - if it works for you, it's £19/month. If not, delete it.

Chris
Brighton, UK
```

---

## 🎯 PRIORITY #5: LIBERTY MUSIC PR

**Services**: Radio promotion campaigns
**Location**: UK-based
**Contact**: Simon Lucas, Bee Adamic, team from training data

**Why Target**:
- **CRITICAL**: Chris has 39,883 messages from Liberty Google Chat
- Deep insight into their workflow and pain points
- Know their campaign processes intimately
- Training data shows high volume of station communications (2,868 patterns)

**Personalization Notes**:
- Reference specific campaign challenges from training data
- Mention station communication patterns
- Show understanding of their workflow
- Could be powerful testimonial/case study source

**Outreach Template**:
```
Subject: Contact research automation (from someone who knows Liberty's workflow)

Hi [Name],

Chris here - radio promoter and producer (sadact).

I've followed Liberty's work for a while and understand the volume of campaigns
you're handling. The contact research time must be brutal.

Built a tool that cuts research from 15+ hours to 3 minutes per campaign.
Processes contact enrichment, verification, and organization automatically.

3-minute demo: [Loom link]
Try 10 free: [link]

Given Liberty's campaign volume, the time savings could be significant.

Chris
Brighton, UK
```

---

## 🎯 PRIORITY #6: STOKED PR

**Website**: [Research needed from search results]
**Services**: Music PR and radio promotion
**Location**: UK-based
**Contact**: [Research needed]

**Why Target**:
- UK-based music PR
- Radio promotion services
- Independent artist focus

**Outreach Template**:
```
Subject: Radio contact research - 15 hours → 3 minutes

Hi [Name],

Chris - radio promoter (5+ years, BBC Radio 6 Music pitches) and producer.

Quick question: How much time does Stoked PR spend on contact research per campaign?

I was spending 15+ hours. Built a tool that cut it to 3 minutes.

3-minute demo: [Loom link]
Try free: [link]

If it saves you time, it's £19/month. If not, no worries.

Chris
Brighton, UK
```

---

## 🎯 PRIORITY #7: CHUFF MEDIA

**Website**: [Research needed from search results]
**Services**: Music PR and promotion
**Location**: UK-based
**Contact**: [Research needed]

**Why Target**:
- UK-based music media company
- Likely handling contact research
- Independent music focus

**Outreach Template**:
```
Subject: Contact research automation for music PR

Hi [Name],

Chris here - radio promoter (5+ years) and producer based in Brighton.

Built a tool that solves the contact research nightmare: 15+ hours → 3 minutes.

Thought Chuff Media might find it useful given your PR work.

Demo: [Loom link]
Free trial: [link]

No sales call needed - try it, and if it works, upgrade to £19/month.

Chris
Brighton, UK
```

---

## 🎯 PRIORITY #8-10: ADDITIONAL PROSPECTS (Research Needed)

**To Research**:
- Small PR agencies in Brighton/South Coast (local connection)
- Independent radio promoters active on LinkedIn/Twitter
- Music management companies handling radio campaigns
- DIY artist communities (Bandcamp, indie labels)

**Where to Find Them**:
- LinkedIn: "radio promoter UK" + "music PR UK"
- Twitter/X: Follow radio promotion conversations
- Music industry Facebook groups
- Royal Blood/Architects network (Chris's connections)

---

## 📧 EMAIL SEQUENCE (If No Response to Initial Outreach)

### Follow-up 1 (3 days later):
```
Subject: Re: [Original subject]

Quick follow-up - not sure if you saw this.

Loom demo: [link]
Free trial: [link]

Saves about 15+ hours per campaign on contact research.

No rush - just wanted to make sure it didn't get buried.

Chris
```

### Follow-up 2 (7 days later):
```
Subject: Last one - contact research tool

[Name],

Last message on this - promise!

If contact research is taking you 10+ hours per campaign, this might help: [Loom link]

If not relevant, no worries at all. Will leave you in peace.

Chris
Brighton, UK
```

---

## 🎬 LOOM VIDEO SCRIPT (3 Minutes)

**To Record When Ready**:

**Intro (15 seconds)**:
"Right, Chris here - I'm a radio promoter (5+ years, pitched to BBC Radio 6 Music, Amazing Radio, loads of regional stations) and producer as sadact. Built Audio Intel because I was wasting 15+ hours per campaign on contact research."

**Demo (2 minutes)**:
- Screen share: intel.totalaudiopromo.com
- Upload 5 contacts CSV (station names only)
- Show 30-second processing
- Highlight results: station info, show details, presenter names, submission contacts
- "This took 30 seconds. Manual research would've taken 3+ hours."

**Close (45 seconds)**:
"Try 10 enrichments free - no card needed, no demo call. If it saves you time, upgrade to £19/month. If not, delete it and move on. Link in description."

**End Screen**:
- intel.totalaudiopromo.com
- "10 Free Enrichments - No Card Required"

---

## 📊 SUCCESS TRACKING

**Outreach Metrics**:
- [ ] Emails sent: Target 10 in first week
- [ ] Response rate: Track %
- [ ] Loom video views: Track via Loom analytics
- [ ] Free trial signups: Track via Audio Intel dashboard
- [ ] Trial → paid conversions: Track by source

**Priority Actions**:
1. Record 3-minute Loom video (schedule time)
2. Research contact details for each agency (30 minutes)
3. Send Shy Cat outreach first (personal connection = highest conversion)
4. Send Decent Music PR outreach (user specified)
5. Batch send remaining 5 agencies
6. Follow up after 3 days, 7 days

---

## 🎯 KEY MESSAGING POINTS (Use in All Outreach)

**Lead With**:
- Time savings: "15 hours → 3 minutes"
- No demo call needed: "3-minute Loom + free trial"
- UK pricing advantage: "£19/month vs £500+ US tools"

**Support With**:
- Personal credibility: "5+ years radio promotion"
- Real experience: "BBC Radio 6 Music pitches"
- Producer identity: "sadact project"
- Location: "Brighton-based"

**Close With**:
- Low friction: "Try 10 free, no card"
- Async: "Watch Loom, try tool, upgrade if useful"
- No pressure: "If not relevant, no worries"

---

**Next Actions**:
1. ✅ Outreach list complete
2. ⏳ Schedule Loom video recording time
3. ⏳ Research contact details for each agency
4. ⏳ Send Shy Cat PR outreach (Priority #1)
5. ⏳ Deploy first social content piece

---

*Created: October 1, 2025*
*Status: Ready for execution once Loom video recorded*
*Target: 2+ demo signups from first 10 outreaches*
