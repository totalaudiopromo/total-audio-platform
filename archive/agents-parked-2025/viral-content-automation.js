...
class ViralContentAutomation {
  ...
  async generateContentCalendar(contentData) {
    ...
    for (let day = 0; day < duration; day++) {
      ...
      const dailyContent = await this.generateDailyContent(
        day,
        currentDate,
        isWeekend,
        contentData
  ...
  async generateTweet(day, index, contentData) {
    ...
    const templates = {
      ...
      behind_scenes_update:
        "Studio session today was incredible. Can't wait to share what we're working on 🎤",
  ...
  determineAutomationLevel(dailyContent) {
    ...
    Object.values(dailyContent).forEach(platformContent => {
      if (Array.isArray(platformContent)) {
        ...
        automatedItems += platformContent.filter(
          item => item.automationLevel === 'auto' || item.automationLevel === 'semi'
        ).length;
      } else {
        Object.values(platformContent).forEach(contentArray => {
          if (Array.isArray(contentArray)) {
            totalItems += contentArray.length;
            automatedItems += contentArray.filter(
              item => item.automationLevel === 'auto' || item.automationLevel === 'semi'
            ).length;
          }
        });
  ...
  async scheduleContent(
    ...
    try {
      ...
      for (const day of contentCalendar.calendar) {
        for (const platform of platforms) {
          if (day.content[platform]) {
            try {
              const result = await this.schedulePlatformContent(
                platform,
                day.content[platform],
                day.date
              );
              schedulingResults.byPlatform[platform] =
                (schedulingResults.byPlatform[platform] || 0) + result.scheduled;
      ...
      logger.info(
        `Content scheduling completed: ${schedulingResults.totalScheduled} posts scheduled`
      );
          ...
          calendar: [
            {
              ...
              content: {
                ...
                instagram: {
                  feed: [
                    { type: 'music_announcement', automationLevel: 'manual', postTime: '11:00' },
                  ],
                },
              },
            },
