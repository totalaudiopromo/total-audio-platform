...
export class AirtableMCPServer implements IAirtableMCPServer {
  ...
  private registerResources(): void {
    const template = new ResourceTemplate('airtable://{baseId}/{tableId}/schema', {
      list: async () => {
  ...
  private registerTools(): void {
      ...
      'list_records',
      {
        ...
        inputSchema: {
          ...
          maxRecords: z
            .number()
            .optional()
            .describe('The maximum total number of records that will be returned'),
          ...
          sort: z
            ...
            .optional()
            .describe('A list of sort objects that specifies how the records will be ordered'),
        },
      },
      async args => {
      ...
      'search_records',
      {
        ...
        inputSchema: {
          ...
          fieldIds: z
            .array(z.string())
            .optional()
            .describe('Optional array of field IDs to search in'),
          maxRecords: z
            .number()
            .optional()
            .describe('The maximum total number of records that will be returned'),
    ...
    this.server.registerTool(
      ...
      async args => {
        ...
        const record = await this.airtableService.getRecord(
          parsedArgs.baseId,
          parsedArgs.tableId,
          parsedArgs.recordId
        );
        return {
          content: [
            { type: 'text', text: JSON.stringify({ id: record.id, fields: record.fields }) },
          ],
        };
      }
    );
    ...
    this.server.registerTool(
      ...
      async args => {
        ...
        const records = await this.airtableService.updateRecords(
          parsedArgs.baseId,
          parsedArgs.tableId,
          parsedArgs.records
        );
    ...
    this.server.registerTool(
      ...
      async args => {
        ...
        const records = await this.airtableService.deleteRecords(
          parsedArgs.baseId,
          parsedArgs.tableId,
          parsedArgs.recordIds
        );
