...
test.describe('Intel Total Audio Promo - Health Check', () => {
  test('should load homepage successfully', async ({ page }) => {
    await page.goto('/');

    // Check if page loads without errors
    await expect(page).toHaveTitle(/Audio Intel|Total Audio Promo/);
    ...
    if (linkCount > 0) {
      for (let i = 0; i < Math.min(linkCount, 5); i++) {
        ...
        if (href && !href.startsWith('#')) {
          ...
          expect(currentUrl).not.toBe('https://intel.totalaudiopromo.com/');
        }
      }
    }
  });
  ...
  test('should handle authentication flows', async ({ page }) => {
    ...
    const authElements = page.locator(
      'a[href*="login"], a[href*="signin"], a[href*="auth"], button:has-text("Login"), button:has-text("Sign In")'
    );
  ...
  test('should have working API endpoints', async ({ page }) => {
    ...
    const apiEndpoints = ['/api/health', '/api/status', '/api/analytics', '/api/contacts'];
