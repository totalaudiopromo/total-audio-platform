...
async function testEnrichmentPerformance() {
  ...
  try {
    const response = await axios.post(
      'http://localhost:3001/api/enrich',
      {
    ...
    if (response.data.success) {
      ...
      const highConfidence = response.data.enriched.filter(
        c => c.researchConfidence === 'High'
      ).length;
      const accuracy = Math.round((highConfidence / testContacts.length) * 100);
      console.log(
        `🎯 Accuracy: ${accuracy}% (${highConfidence}/${testContacts.length} high confidence)`
      );
