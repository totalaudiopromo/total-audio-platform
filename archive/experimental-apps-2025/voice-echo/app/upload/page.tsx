    ...
    try {
      ...
      try {
        const saveRes = await fetch('/api/download', {
          ...
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            enriched: results.map(({ name, email, contactIntelligence }) => ({
              name,
              email,
              contactIntelligence,
            })),
      ...
      if (userEmail) {
        try {
          ...
          if (notifyData.success) {
            ...
            setNotifyStatus(
              'Failed to send notification email: ' + (notifyData.error || 'Unknown error')
            );
          }
        } catch (err: any) {
    ...
    <div className="max-w-3xl mx-auto py-10 px-4">
      ...
      <div className="mb-6">
        ...
        <div className="text-xs text-gray-500 mt-1">
          We'll email you when your results are ready. (Optional)
        </div>
      </div>
        ...
        <div className="mt-10">
          {emailSubmitted && (
            <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded text-blue-700">
              We will email you at <span className="font-semibold">{userEmail}</span> when your
              results are ready.
              {notifyStatus && <div className="mt-2 text-sm text-green-700">{notifyStatus}</div>}
            </div>
          )}
          <div className="mb-4 flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <ExportButtons
              enriched={enriched.map(({ name, email, contactIntelligence }) => ({
                name,
                email,
                contactIntelligence,
              }))}
              onExport={() => trackExport('csv', enriched.length)}
            />
          </div>
              ...
              <tbody>
                {enriched.map((c, i) => (
                  <tr key={i}>
                    ...
                    <td
                      className="border px-2 py-1 whitespace-pre-line align-top"
                      style={{ maxWidth: 400 }}
                    >
