...
export const TexturePricingCard: React.FC<
  Omit<TextureCardProps, 'variant'> & { tier?: 'basic' | 'pro' | 'enterprise' }
> = ({ children, tier = 'basic', ...props }) => {
  const tierTextures = {
...
export const TextureFeatureCard: React.FC<
  Omit<TextureCardProps, 'variant'> & { index?: number }
...
export const TextureHeroCard: React.FC<Omit<TextureCardProps, 'variant'>> = props => {
  return (
    <TextureCard variant="hero" texture="paper" animation="entrance" rotation="none" {...props}>
      {props.children}
    </TextureCard>
  );
};
