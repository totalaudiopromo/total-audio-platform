...
const variantClasses = {
  ...
  outline:
    'texture-btn bg-transparent border-2 border-brand-blue text-brand-blue hover:bg-brand-blue hover:text-white',
  ghost: 'texture-btn bg-transparent text-gray-700 hover:bg-gray-100',
  texture:
    'texture-btn bg-gradient-to-r from-brand-blue to-brand-yellow text-white hover:from-brand-blue/90 hover:to-brand-yellow/90',
};
...
export const TextureButton: React.FC<TextureButtonProps> = ({
  ...
  return (
    ...
    >
      {/* Texture overlay */}
      {texture !== 'none' && <div className="absolute inset-0 texture-overlay opacity-10" />}
      ...
      {/* Content */}
      <span className={cn('relative z-10', { 'opacity-0': loading })}>{children}</span>
    </button>
  );
};
    ...
    >
      {React.Children.map(children, (child, index) => {
        if (React.isValidElement(child)) {
          ...
          const newClassName = cn(childProps.className, {
            ...
            'rounded-none border-l-0 border-r-0':
              !vertical && index > 0 && index < React.Children.count(children) - 1,
