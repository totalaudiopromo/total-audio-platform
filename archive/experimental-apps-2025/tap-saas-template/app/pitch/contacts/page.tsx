...
import { ArrowLeft, Plus, UserPlus, Loader2, Trash2 } from 'lucide-react';
...
export default function ContactsPage() {
  ...
  async function handleAddContact(e: React.FormEvent) {
    ...
    try {
      ...
      const { error } = await supabase.from('intel_contacts').insert({
        user_id: userId,
        name: formData.name,
  ...
  async function handleDeleteContact(contactId: string) {
    ...
    try {
      const { error } = await supabase.from('intel_contacts').delete().eq('id', contactId);
                    ...
                    {contact.genre_tags && contact.genre_tags.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {contact.genre_tags.map(tag => (
                          <span
                            key={tag}
                            className="rounded-full bg-brand-iris/20 px-3 py-1 text-xs text-brand-iris"
                          >
                            {tag}
                          </span>
                        ))}
