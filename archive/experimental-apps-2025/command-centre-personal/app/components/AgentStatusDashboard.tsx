...
export default function AgentStatusDashboard() {
  ...
  return (
    <div className="intel-card">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Agent Status Dashboard</h2>
        <p className="text-gray-600">Real-time status of all platform agents</p>
      </div>

      {/* Summary Cards */}
      <div className="intel-metrics mb-6">
        {getCategorySummary().map(({ category, count, functional }) => (
          <div key={category} className={`intel-metric ${getCategoryColor(category)}`}>
            <div className="text-center">
              <div className="intel-metric-value">{count}</div>
              <div className="intel-metric-label">{getCategoryTitle(category)}</div>
              {category !== 'parked' && (
                <div className="text-xs mt-1 text-gray-500">{functional} functional</div>
              )}
            </div>
          </div>
        ))}
        ...
        {categories.map(category => {
          ...
          return (
            <div key={category} className="intel-card">
              <h3
                className={`text-lg font-semibold mb-3 ${getCategoryColor(category)} p-2 rounded`}
              >
