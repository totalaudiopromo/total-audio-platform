...
export async function POST() {
  try {
    ...
    try {
      ...
      console.log(
        '⚠️ Audio Intel status not available:',
        error instanceof Error ? error.message : 'Unknown error'
      );
    }
    ...
    try {
      const notionUpdateResponse = await fetch(
        'http://localhost:3000/api/notion/business-metrics',
        {
          method: 'POST',
          headers: {
...
export async function GET() {
  try {
    ...
    const notionConfigured = !!(
      process.env.NOTION_API_KEY && process.env.NOTION_BUSINESS_METRICS_DB_ID
    );
