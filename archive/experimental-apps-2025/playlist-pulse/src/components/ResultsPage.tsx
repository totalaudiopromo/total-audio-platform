...
export default function ResultsPage() {
    ...
    .filter(curator => {
      ...
      const matchesFollowers =
        ...
        (filterFollowers === 'medium' &&
          parseInt(curator.followers.replace('K', '')) >= 50 &&
          parseInt(curator.followers.replace('K', '')) < 100) ||
  ...
  const averageCompatibility = Math.round(
    state.curators.reduce((sum, c) => sum + c.compatibility, 0) / state.curators.length
  );
  const totalFollowers = state.curators.reduce(
    (sum, c) => sum + parseInt(c.followers.replace('K', '')),
    0
  );
