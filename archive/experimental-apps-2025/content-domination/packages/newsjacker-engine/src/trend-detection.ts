...
class TrendDetectionEngine {
  ...
  async performFullScan(): Promise<NewsItem[]> {
    ...
    console.log(
      `🎯 Found ${relevantNews.length} relevant news items out of ${allNews.length} total`
    );
  ...
  private async scoreRelevance(newsItem: NewsItem): Promise<void> {
    ...
    const automationMatches = this.automationKeywords.filter(keyword => fullText.includes(keyword));
    score += automationMatches.length * 0.15;

    // Check for high-impact industry keywords
    const impactMatches = this.impactKeywords.filter(keyword => fullText.includes(keyword));
    score += impactMatches.length * 0.2;

    // Specific automation angles
    if (
      fullText.includes('playlist') &&
      (fullText.includes('curator') || fullText.includes('pitch'))
    ) {
      score += 0.3;
  ...
  private async checkHighPrioritySources(): Promise<void> {
    const highPrioritySources = Array.from(this.newsSources.values()).filter(
  ...
  private async checkMediumPrioritySources(): Promise<void> {
    const mediumPrioritySources = Array.from(this.newsSources.values()).filter(
      source => source.priority === 'medium'
    );
  ...
  private generateContentBriefs(newsItem: NewsItem): string[] {
    ...
    briefs.push(
      `LINKEDIN ARTICLE: Professional take on ${newsItem.category} development. Title: "How ${newsItem.title} Changes Everything for Music Marketing Automation" - Authority-building piece with industry analysis and actionable insights.`
    );

    // Newsletter section brief
    if (newsItem.automationOpportunity.audioIntelOpportunity) {
      briefs.push(
        `NEWSLETTER SECTION: "This Week's Industry WTF" - Chris's reaction to the news with practical automation implications. Include reader question format and Audio Intel connection.`
      );
    }
  ...
  private async analyzeTrends(): Promise<void> {
    const recentTrends = Array.from(this.detectedTrends.values()).filter(trend => {
  ...
  public async searchTrends(query: string): Promise<NewsItem[]> {
    ...
    return Array.from(this.detectedTrends.values()).filter(
      trend =>
...
export type { NewsSource, NewsItem, AutomationOpportunity, TrendAlert };
