...
class ContentFusionEngine {
  ...
  private selectOptimalTemplate(
    ...
    const availableTemplates =
      urgencyMatchingTemplates.length > 0 ? urgencyMatchingTemplates : platformTemplates;
    ...
    for (const template of availableTemplates) {
      ...
      if (
        scoringResult.breakdown.automationRelevance > 0.7 &&
        template.engagementType === 'authority'
      ) {
        templateScore += 0.3;
      }
  ...
  private calculateVoiceConsistency(content: string): number {
    ...
    const foundPhrases = britishPhrases.filter(phrase =>
    ...
    const foundDirectPhrases = directPhrases.filter(phrase =>
      lowerContent.includes(phrase.toLowerCase())
    );
    ...
    const foundInsiderPhrases = insiderPhrases.filter(phrase =>
      lowerContent.includes(phrase.toLowerCase())
    );
   ...
   */
  private getVariableReplacement(
    variable: string,
    newsItem: NewsItem,
    scoringResult: ScoringResult
  ): string {
            ...
            structure: [
              {
                type: 'hook',
                content:
                  "🧵 Thread: {news_title} - here's what this REALLY means for indie artists... 1/7",
            ...
            structure: [
              {
                type: 'hook',
                content:
                  "The Hidden Truth About {news_title} That's Costing Independent Artists Thousands",
                variables: ['news_title'],
                optional: false,
              },
              {
  ...
  private determineUrgencyFromNews(newsItem: NewsItem): 'immediate' | 'same-day' | 'weekly' {
    ...
    if (
      ...
      title.includes('announced today') ||
