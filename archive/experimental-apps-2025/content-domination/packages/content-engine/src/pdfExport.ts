...
export class AudioIntelPDFExporter {
  ...
  private addFooter(): void {
    ...
    this.doc.setTextColor(128, 128, 128);
    this.doc.text(
      'Audio Intel - Contact Intelligence Built by Industry Professionals',
      this.margin,
      this.pageHeight - 20
    );
  ...
  addDetailedContactInfo(contact: ContactData): void {
    ...
    if (contact.musicIndustry?.isMusicIndustry) {
      ...
      this.doc.text(
        `Industry Type: ${contact.musicIndustry.industryType}`,
        this.margin,
        this.currentY
      );
      this.currentY += 5;
      ...
      // Recommendations
      if (
        contact.musicIndustry.recommendations &&
        contact.musicIndustry.recommendations.length > 0
      ) {
        this.currentY += 5;
        this.doc.setFontSize(9);
        this.doc.setFont('helvetica', 'bold');
