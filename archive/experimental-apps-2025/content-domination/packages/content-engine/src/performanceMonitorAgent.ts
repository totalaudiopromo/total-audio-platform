...
- Historical success pattern matching`;
  ...
  private calculateOpportunityConfidence(platform: PerformanceMetrics, metric: number): number {
    ...
    const trendStrength =
      Math.abs(platform.trends.growth_24h) + Math.abs(platform.trends.growth_7d);
  ...
  private generateBasicPrediction(
    ...
    const avgGrowth =
      performanceData.reduce((sum, p) => sum + p.trends.growth_7d, 0) / performanceData.length;
  ...
  private generateExecutiveSummary(performanceData: PerformanceMetrics[]): any {
    ...
    const socialData = performanceData.filter(p =>
      ['instagram', 'tiktok', 'twitter'].includes(p.platform)
    );
    ...
    return {
      ...
      conversion_rate:
        ((emailData?.metrics.conversions || 0) / (emailData?.metrics.opens || 1)) * 100,
  ...
  private async generatePlatformBreakdown(
    performanceData: PerformanceMetrics[]
