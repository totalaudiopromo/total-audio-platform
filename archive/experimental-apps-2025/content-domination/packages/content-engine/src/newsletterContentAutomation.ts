...
class NewsletterContentAutomation {
  ...
  private async detectNewNewsletter(newsletterId?: string): Promise<NewsletterContent> {
    try {
      ...
      const newsletterSequence = sequences.sequences.find(
        seq =>
  ...
  private async getNotionVoiceExamples(topic: string): Promise<VoiceExample[]> {
    try {
      const response = await fetch(
        'https://api.notion.com/v1/databases/' + this.notionDatabaseIds.voiceExamples + '/query',
        {
          method: 'POST',
          headers: {
  ...
  private async generateTwitterThread(
    ...
    const twitterTemplates = notionContext.contentTemplates.filter(
      t => t.templateType === 'twitter_thread'
    );

    const bestTemplate = twitterTemplates.sort(
      (a, b) => b.performanceData.avgEngagement - a.performanceData.avgEngagement
    )[0];

    const voiceTrainingPrompt = this.buildVoiceTrainingPrompt(notionContext.voiceExamples);
    const threadPrompt = this.buildTwitterThreadPrompt(
      contentAnalysis,
      bestTemplate,
      voiceTrainingPrompt
    );
...
Generate a Twitter thread that sounds exactly like I wrote it myself, using the voice patterns from my examples above.`;
  ...
  private async generateLinkedInArticle(
    ...
    const linkedinTemplates = notionContext.contentTemplates.filter(
      t => t.templateType === 'linkedin_article'
    );
...
Generate a LinkedIn article that establishes authority while maintaining my authentic voice.`;
  ...
  private getDefaultBrandGuidelines(): BrandGuidelines {
    return {
      ...
      avoidList: ['Corporate speak', 'Overly promotional language', 'Generic marketing phrases'],
    };
  }
  ...
  private async analyzeNewsletterMetadata(
    content: string,
    sections: NewsletterSection[]
  ...
  private async generateEmailSequenceUpdates(
    analysis: any,
    context: NotionContentContext
  ): Promise<PlatformContent[]> {
  ...
  private async analyzeNewsletterContent(
