...
export class EnhancedSocialDetection {
  ...
  private async verifyTwitterProfile(
    profile: any,
    name: string,
    company?: string
  ): Promise<number> {
  ...
  private async verifyInstagramProfile(
    profile: any,
    name: string,
    company?: string
  ): Promise<number> {
    ...
    if (profile.follower_count && profile.follower_count > 100 && profile.follower_count < 100000)
      score += 10;
  ...
  private findBestInstagramMatch(
    results: any[],
    name: string,
    company?: string,
    location?: string
