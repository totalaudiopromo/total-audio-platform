import NotionProgressDashboard, {
  ProgressMetrics,
  BusinessMilestone,
} from './notionProgressDashboard';
  ...
  async generateScheduledReports(): Promise<void> {
    ...
    try {
      ...
      const progressMetrics: ProgressMetrics = {
        ...
        agentPerformance: metricsData.data.agentPerformance || {
          totalJobs: 0,
          successRate: 0,
          avgResponseTime: 0,
          uptime: 0,
        },
        customerSuccess: metricsData.data.customerSuccess || {
          totalCustomers: 0,
          satisfaction: 0,
          retention: 0,
          churn: 0,
        },
        activeCampaigns: metricsData.data.activeCampaigns || {
          total: 0,
          active: 0,
          completed: 0,
          successRate: 0,
      ...
      if (dayOfMonth === 1) {
        const month = new Date(now.getFullYear(), now.getMonth() - 1, 1).toLocaleDateString(
          'en-US',
