...
class FreePremiumEmailValidation {
  ...
  private roleBasedPatterns = {
    ...
    personal: [/^[a-z]+\.[a-z]+@/i, /^[a-z]+[0-9]+@/i, /^[a-z]+_[a-z]+@/i],
  };
  ...
  async validateEmail(email: string): Promise<FreeEmailValidationResult> {
    ...
    try {
      ...
      const [mxCheck, disposableCheck, externalValidation, domainInfo] = await Promise.allSettled([
      ...
      const results = this.aggregateValidationResults({
        ...
        externalValidation:
          externalValidation.status === 'fulfilled' ? externalValidation.value : null,
      ...
      console.log(
        `✅ Email validated in ${duration}ms - Score: ${score}/100 - Confidence: ${confidence}`
      );
  ...
  private aggregateValidationResults(data: any): any {
    const { email, formatValid, mxCheck, disposableCheck, externalValidation, domainInfo } = data;

    const domain = email.split('@')[1];

    // Determine validity based on multiple factors
    const isValid = formatValid && mxCheck && !disposableCheck && domainInfo?.valid;
  ...
  private isMusicIndustryDomain(domain: string): boolean {
    ...
    return musicKeywords.some(keyword => domain.toLowerCase().includes(keyword));
  }
