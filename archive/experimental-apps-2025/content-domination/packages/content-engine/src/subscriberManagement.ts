      ...
      for (const subscriber of subscribers) {
        ...
        for (const pattern of this.BEHAVIOR_PATTERNS) {
          const matchesCriteria = this.matchesCriteria(
            {
              ...subscriber,
              fields: { ...subscriber.fields, engagementScore: engagement.total_score },
            },
            pattern.conditions
          );
  ...
  private async handleTrialExpiration(subscriberId: string) {
    try {
      ...
      if (user?.tags?.some(tag => tag.startsWith('paid_user'))) {
        ...
        await this.kit.addTagsToSubscriber(subscriberId, ['converted_user', 'onboarding_sequence']);
      } else {
        // Didn't convert - move to winback
        await this.kit.removeTagsFromSubscriber(subscriberId, ['trial_user', 'trial_sequence']);

        await this.kit.addTagsToSubscriber(subscriberId, ['churned_user', 'winback_sequence']);
      }
    } catch (error) {
