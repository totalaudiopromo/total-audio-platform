...
class TwitterAutomation {
  ...
  private async optimizeTweetsForTwitter(
    ...
    for (let i = 0; i < tweets.length; i++) {
      ...
      const estimatedEngagement = await this.calculateEstimatedEngagement(
        content,
        optimizedHashtags
      );
  ...
  private async makeAuthenticatedRequest(
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        throw new Error(
          `Twitter API error: ${error.response?.status} ${error.response?.statusText}`
        );
      }
      throw error;
    }
  ...
  private updateRateLimitCounter(): void {
    ...
    const hourlyCount = this.rateLimitTracker.get(hourlyKey) || {
      count: 0,
    ...
    const dailyCount = this.rateLimitTracker.get(dailyKey) || {
      count: 0,
   ...
   */
  private async collectThreadAnalytics(
  ...
  getRateLimitStatus(): any {
    ...
    return {
      hourly: this.rateLimitTracker.get(hourlyKey) || {
        count: 0,
        resetTime: new Date(now.getTime() + 60 * 60 * 1000),
      },
      daily: this.rateLimitTracker.get(dailyKey) || {
        count: 0,
        resetTime: new Date(now.getTime() + 24 * 60 * 60 * 1000),
      },
      limits: {
