...
export class AirtableContactEnrichment {
  ...
  private extractFromContent(content: string): ParsedData {
    ...
    const genrePatterns: RegExp[] = [
      /(?:playing|featuring|music)\s+(?:genres?|styles?)\s*:?\s*([^.\n]+)/gi,
      /(?:format|style)\s*:?\s*([^.\n]+)/gi
    ];
    ...
    const submissionPatterns: RegExp[] = [
      /(?:submission|demo|music)\s+(?:guidelines?|policies?|requirements?)\s*:?\s*([^.\n]+)/gi,
      /(?:send|submit)\s+(?:your|demo|music)\s+to\s*:?\s*([^.\n]+)/gi
    ];
    ...
    const contactPatterns: RegExp[] = [
      /(?:contact|submit|demo)\s+(?:form|page|portal)\s*:?\s*([^.\n]+)/gi,
      /(?:email|send)\s+(?:to|at)\s*:?\s*([^\s]+@[^\s]+)/gi
    ];
  ...
  private async updateEnrichedRecords(enrichments: EnrichmentData[]): Promise<void> {
    ...
    const updateRecords = enrichments.map(async (enrichment: EnrichmentData) => {
      ...
      if (records.length > 0 && records[0]) {
        const enrichedNotes: string = `Contact Enrichment: ${enrichment.stationName}
Format: ${enrichment.scrapedData.format}
Genres: ${enrichment.scrapedData.genres.join(', ')}
DJs: ${enrichment.scrapedData.djNames.join(', ')}
Submission: ${enrichment.scrapedData.submissionGuidelines}
Contact Forms: ${enrichment.scrapedData.contactForms.join(', ')}
Social Media: ${enrichment.scrapedData.socialMedia.join(', ')}
Additional: ${enrichment.scrapedData.additionalContacts.join(', ')}`;
        return {
          id: records[0].id,
          fields: {
            'Station': enrichment.stationName,
            'Notes': enrichedNotes
          }
        };
      }
