        ...
        children: [
          {
            type: 'paragraph',
            paragraph: {
              rich_text: [{ type: 'text', text: { content: 'This folder contains pages that can be safely deleted to free up blocks. Review each page before deleting.' } }]
            }
          },
          {
            type: 'heading_2',
            heading_2: {
              rich_text: [{ type: 'text', text: { content: '⚠️ IMPORTANT: Review Before Deleting' } }]
            }
          },
          {
            type: 'paragraph',
            paragraph: {
              rich_text: [{ type: 'text', text: { content: 'These pages have been identified as duplicates, old versions, or low-priority content that can be safely removed to free up your block limit.' } }]
            }
          }
        ]
      });
  ...
  async auditWorkspace() {
    try {
      ...
      searchResults.results.forEach(page => {
        ...
        } else if (title.includes('MASTER') || title.includes('CURRENT') || title.includes('Audio Intel')) {
  ...
  async consolidateDuplicates() {
    try {
      ...
      if (radioPromoPages.results.length > 1) {
        ...
        for (const duplicate of duplicates) {
          try {
            ...
            console.log(`   ✅ Moved duplicate to archive: ${duplicate.properties.title?.title?.[0]?.plain_text}`);
          } catch (error) {
            console.log(`   ⚠️ Could not move: ${duplicate.properties.title?.title?.[0]?.plain_text}`);
          }
        }
      }
      ...
      if (masterStructurePages.results.length > 1) {
        ...
        const duplicates = masterStructurePages.results.filter(page => 
          !page.properties.title?.title?.[0]?.plain_text?.includes('CLEAN')
        );
    ...
    try {
      ...
      const cleanNavContent = [
        ...
        },
        {
          type: 'paragraph',
          paragraph: {
            rich_text: [{ type: 'text', text: { content: 'Your workspace has been cleaned and organized. Use the sidebar to navigate between folders.' } }]
          }
        },
        {
          type: 'heading_2',
        ...
        },
        {
          ...
          bulleted_list_item: {
            rich_text: [{ type: 'text', text: { content: '📋 Daily Operations - Your to-dos and priorities' } }]
          }
        },
        {
          type: 'bulleted_list_item',
          bulleted_list_item: {
            rich_text: [{ type: 'text', text: { content: '🎯 Audio Intel Product - Product development' } }]
          }
        },
        {
          type: 'bulleted_list_item',
          bulleted_list_item: {
            rich_text: [{ type: 'text', text: { content: '📊 Business Strategy - Revenue and growth' } }]
          }
        },
        {
          type: 'bulleted_list_item',
          bulleted_list_item: {
            rich_text: [{ type: 'text', text: { content: '🔧 Technical Development - Code and agents' } }]
          }
        },
        {
          type: 'bulleted_list_item',
          bulleted_list_item: {
            rich_text: [{ type: 'text', text: { content: '📝 Content & Marketing - Brand and content' } }]
          }
        },
        {
          type: 'bulleted_list_item',
        ...
        },
        {
          type: 'paragraph',
          paragraph: {
            rich_text: [{ type: 'text', text: { content: 'Duplicates and old content have been moved to the Archive folder. Review the Archive folder and delete pages you no longer need to free up blocks.' } }]
          }
        }
      ];
