      ...
      if (item.object === 'page') {
        ...
        try {
          ...
          content = blocks.results.map(block => {
            ...
            return '';
          }).join(' ').substring(0, 500); // First 500 chars for analysis
        } catch (error) {
        ...
        } else if (titleLower.includes('playlist pulse') || contentLower.includes('playlist pulse')) {
          pageCategories['Playlist Pulse'].push(pageInfo);
        } else if (titleLower.includes('business') || titleLower.includes('strategy') || titleLower.includes('market') || contentLower.includes('revenue') || contentLower.includes('pricing')) {
          pageCategories['Business Strategy'].push(pageInfo);
        } else if (titleLower.includes('pricing') || contentLower.includes('£') || contentLower.includes('pricing')) {
