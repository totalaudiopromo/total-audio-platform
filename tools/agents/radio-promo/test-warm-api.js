...
async function testWarmAPI() {
  ...
  try {
    ...
    stations.slice(0, 5).forEach((station, index) => {
      console.log(`   ${index + 1}. ${station.name || station.stationName || 'Unknown'} (${station.countryCode || 'GB'})`);
    });
    ...
    const csvData = await warmApi.generateCSVReport(testArtist, startDate, new Date().toISOString().split('T')[0]);
