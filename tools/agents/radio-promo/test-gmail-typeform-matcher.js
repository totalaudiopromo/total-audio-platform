  ...
  try {
    ...
    if (campaigns.matchedCampaigns.length > 0) {
      ...
      campaigns.matchedCampaigns.forEach((campaign, index) => {
        console.log(`   ${index + 1}. ${campaign.typeformResponse.data?.artistName || 'Unknown Artist'}`);
        console.log(`      Track: ${campaign.typeformResponse.data?.trackTitle || 'Unknown Track'}`);
    ...
    if (campaigns.matchedCampaigns.length > 0) {
      const testArtist = campaigns.matchedCampaigns[0].typeformResponse.data?.artistName?.split(' ')[0];
      if (testArtist) {
