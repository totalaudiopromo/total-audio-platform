...
async function debugStationsResponse() {
  ...
  try {
    ...
    const response = await fetch('https://public-api.warmmusic.net/api/v1/radio-stations?countryCode=GB&isMonitored=true', {
      headers: { 
    ...
    const housePopKeywords = [
      'bbc radio 1', 'bbc radio 2', 'capital', 'heart', 'kiss', 'magic', 
      'smooth', 'virgin', '1xtra', 'xtra', 'dance', 'electronic', 'ministry',
      'rinse', 'amazing', 'boom', 'union', 'planet'
    ];
    ...
    if (data.hasMore && data.currentPagesEntities) {
      ...
      const page2Response = await fetch('https://public-api.warmmusic.net/api/v1/radio-stations?countryCode=GB&isMonitored=true&page=2', {
        headers: { 
          'Authorization': `Bearer ${process.env.WARM_API_TOKEN}`,
          'User-Agent': 'Liberty-Music-PR-Agent/1.0'
        }
