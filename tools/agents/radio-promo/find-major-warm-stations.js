...
async function findMajorWarmStations() {
  ...
  try {
    ...
    const majorStationKeywords = {
      bbc: ['bbc radio 1', 'bbc radio 2', 'bbc radio 3', 'bbc radio 4', 'bbc 6 music', 'bbc 1xtra', 'radio 1', 'radio 2', 'radio 3', 'radio 4', '6 music', '1xtra'],
      commercial: ['capital', 'heart', 'kiss', 'magic', 'smooth', 'virgin', 'absolute', 'classic fm', 'lbc'],
      ...
      try {
        ...
        if (response.ok) {
          ...
          const majorFound = pageStations.filter(station => {
            const name = station.name.toLowerCase();
            return majorStationKeywords.bbc.some(k => name.includes(k)) ||
          ...
          if (majorFound.length > 0) {
            ...
            majorFound.forEach(station => {
              console.log(`      • ${station.name} (${station.category}) - ${station.city?.name || 'Unknown'}`);
            });
          }
        } else {
    ...
    // Summary
    const totalMajor = majorStations.bbc.length + majorStations.commercial.length + 
                      majorStations.dance.length + majorStations.online.length;
