...
class RadioPortalAutomation {
  constructor(options = {}) {
    ...
    this.discord = {
      ...
      levels: Array.isArray(options.discordLevels) && options.discordLevels.length > 0
        ? options.discordLevels.map(level => String(level).toLowerCase())
        : (envLevels.length > 0 ? envLevels : ['error', 'warn'])
    };
  ...
  normalizeAssetUrl(originalUrl) {
    ...
    try {
      ...
      if (host.includes('drive.google.com')) {
        ...
        this.log('warn', `Google Drive folder link detected; unable to auto-download: ${originalUrl}`);
        return originalUrl;
      }
    } catch (error) {
      return originalUrl;
  ...
  async cleanupAssets(filePaths = []) {
    await Promise.all(filePaths.filter(Boolean).map(async file => {
      try {
  ...
  async findFieldSelectorByLabel(page, labelText) {
    ...
    return page.evaluate(text => {
      ...
      for (const label of matches) {
        let field = null;
        if (label.htmlFor) {
          field = document.getElementById(label.htmlFor) || document.querySelector(`#${label.htmlFor}`);
        }
        if (!field) {
  ...
  async selectOption(page, selectors, value, labelCandidates = []) {
    ...
    for (const selector of candidateSelectors) {
      ...
      try {
        ...
        const normalized = (await page.evaluate((sel, target) => {
          const element = document.querySelector(sel);
          if (!element) return null;
          const options = Array.from(element.options || []);
          const lowerTarget = target.toLowerCase();
          const match = options.find(option => option.text.trim().toLowerCase() === lowerTarget)
  ...
  async clickButton(page, textOptions = []) {
    for (const text of textOptions) {
      const success = await page.evaluate(buttonText => {
        ...
        const target = buttons.find(btn => (btn.textContent || btn.value || '').trim().toLowerCase().includes(buttonText.toLowerCase()));
        if (target) {
          target.click();
          return true;
        }
  ...
  extractCampaignLocations(campaign) {
    ...
    const city = campaign.city || campaign.artistCity || data.artistCity || data['artist_city'] || '';
    const country = campaign.country || campaign.artistCountry || data.artistCountry || data['artist_country'] || '';
    return { city, country };
  }
  ...
  async prepareAssets(campaign) {
    ...
    const data = campaign.original?.data || {};
    const trackCandidates = [...(campaign.downloadLinks || []), data.mp3Link, data['mp3_link'], data['audioFile']].filter(Boolean);
    ...
    const pressCandidates = [data.pressPhoto, data['press_photo'], data['artist_photo'], data['pressImage'], campaign.pressReleaseLink].filter(Boolean);
    ...
    const artworkCandidates = [data.coverArt, data['cover_art'], data['artwork'], data['album_art']].filter(Boolean);
  ...
  async submitToAmazingRadio(campaign, preparedAssets = null) {
    ...
    try {
      const result = await this.withBrowser(async page => {
        ...
        await ensure(
          this.fillField(page, ['input[type="email"]', 'input[name="email"]', '#email'], this.amazing.username, ['Email']),
          'Amazing Radio login email'
        );
        await ensure(
          this.fillField(page, ['input[type="password"]', 'input[name="password"]', '#password'], this.amazing.password, ['Password']),
        ...
        await page.goto('https://amazingradio.com/profile/liberty-music-pr/artists/new', { waitUntil: 'networkidle2' });

        const { city, country } = this.extractCampaignLocations(campaign);
        const bio = this.extractArtistBio(campaign);

        await ensure(this.fillField(page,
          ['input[name="displayName"]', 'input[name="display_name"]', 'input#artist_display_name'],
          campaign.artistName,
          ['Display Name', 'Artist Name']), 'Amazing Radio display name');
        await ensure(this.fillField(page,
          ['textarea[name="bio"]', '#bio', 'textarea#artist_bio'],
          bio,
          ['Bio', "Artist Biography"]), 'Amazing Radio bio');
        await ensure(this.fillField(page,
          ['input[name="town"]', 'input[name="city"]', '#city'],
          city,
          ['Town', 'City']), 'Amazing Radio city');
        await ensure(this.fillField(page,
          ['input[name="country"]', '#country'],
          country,
          ['Country']), 'Amazing Radio country');

        await ensure(this.clickButton(page, ['Create Artist', 'Save']), 'Amazing Radio create artist');
        ...
        if (campaign.facebook || campaign.instagram || campaign.twitter || (campaign.otherLinks && campaign.otherLinks.length)) {
          this.log('info', 'Adding social links to Amazing Radio profile');
          await ensure(
            this.fillField(page, ['input[name="facebook"]', '#facebook'], campaign.facebook, ['Facebook']),
            'Amazing Radio facebook link',
            true
          );
          await ensure(
            this.fillField(page, ['input[name="instagram"]', '#instagram'], campaign.instagram, ['Instagram']),
            'Amazing Radio instagram link',
            true
          );
          await ensure(
            this.fillField(page, ['input[name="twitter"]', '#twitter', 'input[name="x"]'], campaign.twitter, ['Twitter', 'X']),
        ...
        await ensure(this.clickButton(page, ['Submit', 'Save', 'Finish']), 'Amazing Radio submission button');
    ...
    try {
      const result = await this.withBrowser(async page => {
        ...
        await ensure(this.fillField(page,
          ...
          campaign.artistName,
          ['Artist Name', 'Band/Artist Name', 'BAND NAME']), 'Wigwam artist name');
        await ensure(this.fillField(page,
          ['textarea[name="artist_bio"]', '#artist_bio'],
          bio,
          ['Artist Bio', 'Biography']), 'Wigwam artist bio');
        await ensure(this.fillField(page,
          ['input[name="artist_city"]', '#artist_city'],
          city,
          ['Town', 'City']), 'Wigwam city');
        await ensure(this.fillField(page,
          ['input[name="artist_country"]', '#artist_country'],
          country,
          ['Country']), 'Wigwam country');
        if (campaign.genre) {
          await ensure(this.selectOption(page,
            ['select[name="artist_genre"]', '#artist_genre'],
            campaign.genre,
            ['Primary Genre', 'Genre', 'PRIMARY GENRE']), 'Wigwam artist genre');
        }

        if (campaign.facebook || campaign.instagram || campaign.twitter) {
          await ensure(this.fillField(page, ['input[name="artist_facebook"]', '#artist_facebook'], campaign.facebook, ['Facebook']), 'Wigwam facebook link', true);
          await ensure(this.fillField(page, ['input[name="artist_instagram"]', '#artist_instagram'], campaign.instagram, ['Instagram']), 'Wigwam instagram link', true);
          await ensure(this.fillField(page, ['input[name="artist_twitter"]', '#artist_twitter'], campaign.twitter, ['Twitter', 'X']), 'Wigwam twitter link', true);
        }

        if (assets.pressPhoto) {
          await ensure(this.uploadByLabel(page, assets.pressPhoto, ['Artist Photo', 'Band Photo']), 'Wigwam artist photo upload', true);
        }
        if (assets.artwork) {
          await ensure(this.uploadByLabel(page, assets.artwork, ['Artwork', 'Cover Art']), 'Wigwam artwork upload', true);
        }

        await ensure(this.clickButton(page, ['Add Artist', 'Save Artist', 'ADD BAND']), 'Wigwam add artist button');
        await this.sleep(3000);

        await ensure(this.uploadByLabel(page, assets.track, ['Upload Track', 'Audio File', 'Song File', 'CHOOSE FILE']), 'Wigwam audio upload');

        await ensure(this.selectOption(page,
          ['select[name="artist_select"]', 'select[name="choose_band"]', '#artist_select'],
          campaign.artistName,
          ['Choose Band', 'CHOOSE BAND']), 'Wigwam choose band');
        await ensure(this.fillField(page,
          ['input[name="track_title"]', '#track_title'],
          campaign.trackTitle,
          ['Track Title', 'Song Title', 'TRACK TITLE']), 'Wigwam track title');
        if (campaign.genre) {
          await ensure(this.selectOption(page,
            ['select[name="track_genre"]', '#track_genre', 'select[name="primary_genre"]'],
            campaign.genre,
            ['Primary Genre', 'Genre', 'PRIMARY GENRE']), 'Wigwam track genre');
        }
        await ensure(this.fillField(page,
          ['textarea[name="track_description"]', '#track_description'],
          campaign.pitch || '',
          ['Track Description', 'About the Track']), 'Wigwam track description', true);

        if (campaign.otherLinks && campaign.otherLinks.length > 0) {
          await ensure(this.fillField(page, ['textarea[name="other_links"]', '#other_links'], campaign.otherLinks.join('\n'), ['Other Links']), 'Wigwam other links', true);
        }

        await ensure(this.setCheckbox(page,
          ['I CERTIFY', 'certify', 'I certify that I am the artist associated with this track'],
          true), 'Wigwam certification checkbox');

        await ensure(this.clickButton(page, ['Upload & Submit for Airplay', 'Submit for Airplay', 'Submit Track', 'Upload Track']), 'Wigwam submit button');
        await this.sleep(5000);
