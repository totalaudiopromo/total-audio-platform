...
class GoogleChatIntegration {
  constructor() {
    ...
    this.monitoredChannels = {
      successShoutOuts: {
        ...
        webhookUrl:
          'https://chat.googleapis.com/v1/spaces/AAAANDK-SNA/messages?key=AIzaSyDdI0hCZtE6vySjMm-WEfRq3CPzqKqqsHI&token=1gWiexkFRsXU9j0qXKlskMjqaVvDJlHqM7vxaDKek5k',
      },
      radioSuperstars: {
        name: 'Radio Superstars',
        webhookUrl:
          'https://chat.googleapis.com/v1/spaces/AAAACVjpDTI/messages?key=AIzaSyDdI0hCZtE6vySjMm-WEfRq3CPzqKqqsHI&token=-eSob_wB06Io17Rtipx_V93aY2bj8aVDBdMTKiIMYfE',
      },
      campaigns: {
        name: 'Campaigns',
        webhookUrl:
          'https://chat.googleapis.com/v1/spaces/AAAAu3XTTik/messages?key=AIzaSyDdI0hCZtE6vySjMm-WEfRq3CPzqKqqsHI&token=vwWKqWEq5dZSfvs9FtAfJNkrltjEVocK38c85H0An1M',
      },
    };
  ...
  async readCampaignWins(limit = 50) {
    try {
      ...
      console.log(
        `✅ Read ${campaignWins.extractedWins.length} campaign wins from Success Shout Outs`
      );
      return campaignWins;
    } catch (error) {
  ...
  async readIndustryContacts(limit = 50) {
    try {
      ...
      console.log(
        `✅ Read ${industryContacts.extractedContacts.length} industry contacts from Radio Superstars`
      );
  ...
  async readCampaignDiscussions(limit = 50) {
    try {
      ...
      console.log(
        `✅ Read ${campaignDiscussions.extractedInsights.length} insights from campaigns channel`
      );
