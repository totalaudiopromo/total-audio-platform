...
class GmailTypeformMatcher {
      ...
      for (const form of forms) {
        try {
          ...
          const matchingResponses = responses.filter(response => {
            return artistEmails.some(email => this.typeform.responseContainsEmail(response, email));
          });
          ...
          if (matchingResponses.length > 0) {
            ...
            for (const response of matchingResponses) {
              try {
                const campaignBrief = await this.typeform.processFormResponseForCampaign(
                  form.id,
                  response.token
                );
  ...
  matchCampaigns(gmailCampaigns, typeformMatches) {
    ...
    for (const gmailCampaign of gmailCampaigns) {
      ...
      const typeformMatch = typeformMatches.find(tf => tf.matchedEmail === gmailEmail);
    ...
    try {
      ...
      const summary = {
        ...
        artists: [
          ...new Set(
            allCampaigns.matchedCampaigns
              .map(c => c.typeformResponse.data?.artistName || c.gmailCampaign.artistInfo?.name)
              .filter(Boolean)
          ),
        ],
        genres: [
          ...new Set(
            allCampaigns.matchedCampaigns
              .map(c => c.typeformResponse.data?.genre || c.gmailCampaign.artistInfo?.genre)
              .filter(Boolean)
          ),
        ],
        campaigns: allCampaigns.matchedCampaigns.map(campaign => ({
          ...
          track:
            campaign.typeformResponse.data?.trackTitle || campaign.gmailCampaign.artistInfo?.track,
  ...
  async healthCheck() {
    ...
    return {
      ...
      overall:
        gmailHealth.status === 'healthy' && typeformHealth.status === 'healthy'
          ? 'healthy'
          : 'unhealthy',
    };
  }
