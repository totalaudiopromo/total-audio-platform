...
class WarmusicAPI {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 403) {
          throw new Error(`Authentication failed (403): Invalid credentials or account not active. Check email/password and ensure 250-song trial is active.`);
        } else if (response.status === 400) {
          // Check if it's a 400 with a 403 message (nested error)
          if (responseText.includes('403')) {
            throw new Error(`Authentication failed (400→403): Invalid credentials or account not active. Check email/password and ensure 250-song trial is active. Response: ${responseText}`);
          } else {
            throw new Error(`Authentication failed (400): Bad request format. Response: ${responseText}`);
          }
        } else {
          throw new Error(`Authentication failed: ${response.status} ${response.statusText}. Response: ${responseText}`);
        }
      }
  ...
  async getPlaysForArtist(artistName, fromDate, untilDate) {
    try {
      ...
      logger.info(`📻 Fetching plays for ${artistName} (${fromDate || 'all time'} - ${untilDate || 'now'})`);
  ...
  async getUKRadioStations() {
    try {
      ...
      const response = await fetch(`${this.baseURL}/radio-stations?countryCode=GB&isMonitored=true`, {
        headers: { 
          'Authorization': `Bearer ${this.token}`,
          'User-Agent': 'Liberty-Music-PR-Agent/1.0'
        }
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch radio stations: ${response.status} ${response.statusText}`);
      }
  ...
  async generateCampaignPerformanceSummary(artistName, campaignStartDate) {
    try {
      ...
      // Get unique stations
      const stations = [...new Set(plays.map(play => play.radioStation || play.station).filter(s => s))];
  ...
  async healthCheck() {
    try {
      ...
      if (error.code === 'ENOTFOUND' || error.message.includes('fetch failed') || error.message.includes('Cannot resolve host')) {
        return {
          status: 'domain_unavailable',
          error: 'WARM API domain not found - API endpoint may have changed',
          details: error.message,
          apiAvailable: false,
          suggestion: 'Contact WARM support to verify current API endpoint URL. The domain public-api.warmmusic.net appears to be unavailable.'
        };
      }
...
2025-09-11,Absolute Radio,${artistName},Test Track,20:15`;
  ...
  async generateWeeklyReport(artistName, trackName, campaignStartDate, driveAPI = null, campaignData = null) {
    await this.ensureValidToken();
