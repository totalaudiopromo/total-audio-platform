...
class SuccessPredictionEngine {
  ...
  initializeSuccessFactors() {
    return {
      ...
      track: {
        ...
        quality: { weight: 0.10, factors: ['production', 'mixing', 'mastering'] },
      ...
      contacts: {
        ...
        genre_match: { weight: 0.10, factors: ['genre_alignment', 'audience_match'] },
        relationship_strength: { weight: 0.08, factors: ['previous_success', 'response_rate', 'engagement'] }
      },
  ...
  initializeModels() {
    ...
    this.predictionModels.set('station_importance', {
      ...
      'BBC Radio 2': { score: 0.90, reach: 15000000, influence: 0.8 },
      'Capital FM': { score: 0.85, reach: 8000000, influence: 0.7 },
      'Kiss FM': { score: 0.80, reach: 5000000, influence: 0.6 },
      'Radio X': { score: 0.75, reach: 3000000, influence: 0.5 },
      'Regional': { score: 0.60, reach: 500000, influence: 0.4 },
      'Local': { score: 0.40, reach: 100000, influence: 0.3 }
    });
  ...
  async predictCampaignSuccess(campaignData, contacts, options = {}) {
    ...
    try {
      ...
      console.log(`✅ Success prediction complete: ${Math.round(predictions.successProbability * 100)}% probability`);
  ...
  calculateTrackScore(campaignData) {
    ...
    const genreModel = this.predictionModels.get('genre')[genre] || { base_score: 0.5, factors: {} };
  ...
  generateRecommendations(factorScores) {
    ...
    if (factorScores.track < 0.6) {
      recommendations.push({
        ...
        actions: ['Get professional mastering', 'Create high-quality press kit', 'Ensure radio-friendly length (3-4 minutes)']
      });
    }
    ...
    if (factorScores.contacts < 0.6) {
      recommendations.push({
        ...
        actions: ['Target more influential stations', 'Improve genre matching', 'Build stronger relationships']
      });
    }
    ...
    if (factorScores.strategy < 0.6) {
      recommendations.push({
        ...
        actions: ['Choose better launch timing', 'Increase budget per contact', 'Improve personalization']
      });
    }
    ...
    if (factorScores.external < 0.6) {
      recommendations.push({
        ...
        actions: ['Wait for better market conditions', 'Consider seasonal factors', 'Monitor competition']
      });
    }
  ...
  getPredictionAnalytics() {
    ...
    const averageAccuracy = predictions.reduce((sum, p) => sum + (p.accuracy || 0), 0) / totalPredictions;
