        ...
        fs.createReadStream(filePath)
          ...
          .on('data', (row) => {
            ...
            const coverage = {
              ...
              outlet: row.outlet || row.Outlet || row['Media Outlet'] || row.Source || row.Publication || '',
              date: row.date || row.Date || row['Publication Date'] || row.Published || '',
              reach: parseInt(row.reach || row.Reach || row['Estimated Reach'] || '0') || 0,
              impressions: parseInt(row.impressions || row.Impressions || row['Impressions'] || '0') || 0,
          ...
          .on('end', () => {
            ...
            logger.info(`Total Reach: ${totalReach.toLocaleString()}, Total Impressions: ${totalImpressions.toLocaleString()}`);
      ...
      return new Promise((resolve, reject) => {
        fs.createReadStream(filePath)
          ...
          .on('end', () => {
            ...
            const importPromises = Object.entries(campaignGroups).map(async ([campaignName, coverageData]) => {
              this.campaignData.set(campaignName, coverageData);
              this.saveCampaignData(campaignName);
              ...
              logger.success(`Imported ${coverageData.length} entries for ${campaignName} (${campaignReach.toLocaleString()} reach)`);
              return { campaign: campaignName, entries: coverageData.length, reach: campaignReach };
            });
            ...
            Promise.all(importPromises).then(results => {
              ...
              if (topCampaigns.length > 0) {
                ...
                topCampaigns.forEach((campaign, index) => {
                  logger.info(`  ${index + 1}. ${campaign.campaign}: ${campaign.reach.toLocaleString()} reach (${campaign.entries} mentions)`);
                });
              }
  ...
  updateMetrics() {
    ...
    this.campaignData.forEach((coverage, campaign) => {
      ...
      Object.entries(campaignSummary.mediaTypes).forEach(([type, count]) => {
        this.coverageMetrics.mediaTypes[type] = (this.coverageMetrics.mediaTypes[type] || 0) + count;
      });
  ...
  loadAllCampaignData() {
    try {
      ...
      const campaignFiles = files.filter(file => 
        file.startsWith('coveragebook_') && 
        file.endsWith('.json') && 
        !file.includes('report_') // Exclude report files
      );
      ...
      campaignFiles.forEach(file => {
        try {
          ...
          const campaignName = data.length > 0 ? data[0].campaign : file.replace('coveragebook_', '').replace('.json', '');
      ...
      if (campaignFiles.length > 0) {
        logger.success(`Loaded ${campaignFiles.length} existing campaigns with ${this.coverageMetrics.totalMentions} total mentions`);
      }
    } catch (error) {
