...
class UniversalEmailTracker {
  ...
  getSendingInstructions(trackingData) {
    return {
      ...
      mailchimp: {
        ...
        notes:
          "Use the tracked content in your Mailchimp template. You can also use Mailchimp's built-in tracking.",
        trackingUrl: this.trackingBaseUrl,
      },
      manual: {
        method: 'Manual copy/paste',
        notes:
          'Copy the tracked content and paste into your email client. Make sure to send as HTML.',
        trackingUrl: this.trackingBaseUrl,
      },
    };
  }
  ...
  async testTracking() {
    ...
    try {
      ...
      console.log(
        `   Tracking pixel: ${trackedEmail.trackingInfo.pixelId ? 'Added' : 'Not added'}`
      );
      ...
      if (linkIds && linkIds.length > 0) {
        ...
        await this.emailTracking.processClickTracking(firstLinkId, 'Test User Agent', '127.0.0.1');
      }
