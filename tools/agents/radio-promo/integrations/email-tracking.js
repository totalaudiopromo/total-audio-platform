...
class EmailTracking {
  ...
  getCampaignStats(campaignId) {
    const campaignData = Array.from(this.trackingData.values())
  ...
  getContactEngagement(contactId) {
    ...
    if (lastOpen && lastClick) {
      engagement.lastEngagement = lastOpen.timestamp > lastClick.timestamp ? 
    ...
    if (engagement.lastEngagement) {
      const daysSinceLastEngagement = (Date.now() - engagement.lastEngagement.timestamp) / (1000 * 60 * 60 * 24);
  ...
  exportToCSV(campaignId = null) {
    ...
    const headers = [
      'Type', 'Email ID', 'Contact ID', 'Campaign ID', 'Timestamp',
      'Opened/Clicked', 'User Agent', 'IP', 'Original URL', 'Link Text'
    ];
