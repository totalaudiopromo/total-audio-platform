  ...
  try {
    ...
    try {
      if (agent.drive) {
        ...
        console.log(`   ✅ Senior Dunce search works - found ${seniorDunceSearch ? seniorDunceSearch.length : 0} files`);
      ...
      console.log(`   🔧 Fix needed: ${error.message.includes('searchFiles') ? 'searchFiles method not implemented' : 'Drive API not properly initialized'}`);
    }
    ...
    try {
      if (agent.gmail) {
        ...
        console.log(`   ✅ Senior Dunce search works - found ${seniorDunceSearch ? seniorDunceSearch.length : 0} emails`);
      ...
      console.log(`   🔧 Fix needed: ${error.message.includes('searchEmails') ? 'searchEmails method not implemented' : 'Gmail API not properly initialized'}`);
    }
    ...
    try {
      ...
      console.log(`   🔧 Fix needed: ${error.message.includes('listCalendars') ? 'listCalendars method not implemented' : 'Calendar API not properly initialized'}`);
    }
    ...
    try {
      ...
      console.log(`   🔧 Fix needed: ${error.message.includes('listSpaces') ? 'listSpaces method not implemented' : 'Chat API not properly initialized'}`);
    }
    ...
    try {
      if (agent.typeformApi) {
        ...
        if (forms && forms.length > 0) {
          ...
          try {
            ...
            console.log(`   ✅ Form responses access works - found ${responses ? responses.length : 0} responses`);
            
            // Test individual response access
            if (responses && responses.length > 0) {
              const firstResponse = responses[0];
              console.log(`   🧪 Testing individual response access for token: ${firstResponse.token}`);
              
              try {
                const detailedResponse = await agent.typeformApi.getResponse(firstForm.id, firstResponse.token);
                console.log('   ✅ Individual response access works');
                console.log(`   📊 Response has ${detailedResponse.answers ? detailedResponse.answers.length : 0} answers`);
              } catch (responseError) {
      ...
      console.log(`   🔧 Fix needed: ${error.message.includes('getRecentForms') ? 'getRecentForms method not implemented' : 'Typeform API not properly initialized'}`);
    }
    ...
    try {
      ...
      for (const tokenFile of tokenFiles) {
        ...
        if (fs.existsSync(tokenPath)) {
          ...
          console.log(`   📊 Token expires: ${tokenData.expiry_date ? new Date(tokenData.expiry_date).toISOString() : 'Unknown'}`);
        } else {
