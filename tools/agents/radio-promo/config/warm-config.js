...
class WarmConfig {
  ...
  getGenreTargeting(genre) {
    return this.genreTargeting[genre.toLowerCase()] || {
  ...
  determineOptimalPreset(campaignData) {
    ...
    if (campaignData.budget && 
        parseFloat(campaignData.budget.replace(/[£$,]/g, '')) > 5000) {
  ...
  getMilestoneSettings(campaignType = 'standard_campaign') {
    ...
    return {
      playMilestones: preset.notifications.playMilestones || this.notificationThresholds.playMilestones,
