...
async function findHousePopStations() {
  ...
  try {
    ...
    console.log(
      `✅ Found ${stationsData.total} UK radio stations (showing first ${stationsData.stations.length})\n`
    );
        ...
        if (response.ok) {
          ...
          pageStations.forEach(station => {
            ...
            // Commercial stations
              ...
              housePopTargets.commercial.some(target => name.includes(target.toLowerCase()))
            ) {
              foundStations.commercial.push({
                name: station.name,
                category: station.category,
    ...
    if (foundStations.bbc.length === 0 && foundStations.commercial.length === 0) {
      ...
      console.log(
        '   This suggests the major stations might be on later pages or have different names.'
      );
      ...
      stationsData.stations.slice(0, 10).forEach((station, i) => {
        console.log(
          `   ${i + 1}. ${station.name} (${station.category}) - ${station.city?.name || 'Unknown'}`
        );
      });
    }
    ...
    const totalTargets =
      foundStations.bbc.length +
      foundStations.commercial.length +
      foundStations.dance.length +
      foundStations.online.length;
