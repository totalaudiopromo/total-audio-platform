...
class TDDComponentSelector {
  constructor() {
    ...
    this.components = {
      layout: [
        'Card', 'Sheet', 'Dialog', 'Popover', 'Tabs',
        'Accordion', 'Collapsible', 'Separator'
      ],
      navigation: [
        'Navigation-Menu', 'Breadcrumb', 'Pagination',
        'Command', 'Menubar', 'Context-Menu'
      ],
      forms: [
        'Input', 'Button', 'Select', 'Checkbox', 'Radio-Group',
        'Switch', 'Slider', 'Textarea', 'Form', 'Label'
      ],
      data: [
        'Table', 'Data-Table', 'Badge', 'Avatar', 'Progress',
        'Skeleton', 'Alert', 'Toast', 'Hover-Card'
      ],
      feedback: [
        'Alert-Dialog', 'Toast', 'Tooltip', 'Progress',
        'Spinner', 'Badge', 'Alert'
      ]
    };
  ...
  listSelections() {
    ...
    files.forEach(file => {
      try {
        ...
        const components = spec.primary_components?.slice(0, 3).join(', ') || 
                          spec.components?.slice(0, 3).join(', ') || 'N/A';
...
if (require.main === module) {
  ...
  if (args.length === 0) {
    ...
    console.log('Usage:');
    console.log('  node tdd-component-selector.js select "feature name" [audiointel|playlistpulse] [form|list|dashboard|detail]');
