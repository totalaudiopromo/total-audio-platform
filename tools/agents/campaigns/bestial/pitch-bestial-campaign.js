...
async function pitchBestialCampaign() {
  ...
  try {
    ...
    const libertyAudience = audiences.find(aud => 
      aud.name.toLowerCase().includes('liberty') || 
      aud.name.toLowerCase().includes('music pr')
    );
    
    if (libertyAudience) {
      console.log(`📋 Liberty audience: ${libertyAudience.name} (${libertyAudience.member_count} members)`);
      
      // Get recent campaigns to see who we've pitched
      const campaigns = await mailchimp.callMailchimpAPI('/campaigns?count=50');
      const recentCampaigns = campaigns.campaigns.filter(c => 
    ...
    const targetStations = indieStations.filter(station => 
      !playingStations.has(station.name)
    ...
    console.log(`📈 Current Performance: ${pitchStrategy.currentPerformance.totalPlays} plays across ${pitchStrategy.currentPerformance.supportingStations} stations`);
