# Concerta "Consumption" Radio Campaign - Setup Complete

**Campaign ID**: concerta-consumption-2025
**Setup Date**: 7 November 2025
**Status**: ✅ Ready to Launch

---

## 🎵 Artist & Track Information

- **Artist**: Concerta
- **Origin**: South Korea
- **Track**: Consumption
- **Genre**: Dance/Electronic
- **Proposed Release**: 15 November 2025
- **Campaign Duration**: 6 weeks

---

## 🎯 Campaign Strategy

### Target Markets (Priority Order)

1. **Primary**: Eastern European dance/electronic radio stations
2. **Secondary**: South Korean electronic music stations
3. **Tertiary**: Global electronic/dance specialists (BBC Radio 1, Kiss FM, etc.)

### Key Messaging

**Hook**: "South Korean electronic artist bringing fresh Asian influence to European dance scene"

**Unique Angle**: Cross-cultural electronic fusion - Asian production meets European dance culture

**Artist Positioning**: Rising star in Seoul electronic music scene

---

## 📊 Campaign Reach

### Current Contact Database

- ✅ **132 Electronic/Dance Specialists** (Global)
- ⚠️ **0 Eastern European Stations** (needs research)
- ⚠️ **0 South Korean Stations** (needs research)

**Total Ready to Contact**: 132 stations

### Contact Breakdown

The 132 electronic/dance contacts include:

- BBC electronic music shows
- UK dance/electronic specialists (Kiss FM, etc.)
- Independent electronic music stations
- Community radio with electronic programming
- Online electronic music platforms

**Top Contacts Include**:
1. WARM Radio (Denmark) - Rock, Metal, Dance
2. "The New Music Show" - Hip-Hop, Electronic, Dance
3. CJSW - Alternative, Rock, Indie, Electronic, Hip-Hop
4. Radio Fandango - Alternative, Rock, Indie, Pop, Folk, Electronic
5. BBC (Steve White) - Pop, Rock, Folk, Electronic
6. Reducedlistening - Electronic specialist
7. Somethinelse - Alternative, Rock, Indie, Pop, Electronic, Jazz/Funk
8. BBC (Jada Joshi) - Alternative, Indie, Electronic, Hip-Hop

---

## 📦 Campaign Assets (All Verified ✅)

**Asset Location**: `~/Downloads/Liberty_0831/`

### Available Assets

1. **Profile Images** ✅
   - 4 high-resolution variants
   - Location: `Profile image/`
   - Files: CONCERTA_High-res_1.jpg through CONCERTA_High_res_4.jpg

2. **Track Artwork** ✅
   - Anime-inspired artwork (4 variants)
   - Futuristic city artwork (4 variants)
   - Location: `consumption_artwork_final/`
   - Includes typography layouts

3. **Music Video** ✅
   - Location: `consumption_video/`
   - File: CONCERTA_city_03.mp4

4. **Label Assets (SYLQ)** ✅
   - Logo files
   - Instagram icon
   - Location: `SYLQ/`

---

## 📧 Email Templates (Ready to Use)

### Template 1: Global Electronic/Dance Specialists

**Subject**: Fresh Electronic Sound: Concerta (South Korea) - Consumption

**Key Points**:
- Cross-cultural electronic perspective
- Cinematic, futuristic sound design
- Anime-inspired visual identity
- Seoul underground scene credibility

**Best For**: BBC Radio 1, Kiss FM, dance music specialists

---

### Template 2: Eastern Europe Dance (Needs Contacts)

**Subject**: New Dance Track from South Korean Artist: Concerta - Consumption

**Key Points**:
- Asian electronic production aesthetics
- European dance music sensibilities
- Peak-time programming potential
- Cross-cultural fusion angle

**Best For**: Eastern European dance stations (when sourced)

---

### Template 3: South Korea Stations (Needs Contacts)

**Subject**: Concerta - Consumption (Korean Electronic Artist for Global Markets)

**Key Points**:
- Korean artist reaching European markets
- International support from UK PR agency
- Representing Korean electronic music globally
- Cross-cultural success story

**Best For**: South Korean electronic/dance stations (when sourced)

---

## 🚀 Next Steps (Action Plan)

### Immediate Actions Required

1. **Review & Approve Email Templates**
   - Customise messaging based on Concerta's preferences
   - Add any press release information
   - Include streaming links when available

2. **Additional Contact Research**
   - Find Eastern European dance/electronic stations
   - Research South Korean electronic music stations
   - Expand global electronic specialist list

3. **Mailchimp Campaign Setup**
   - Create campaign in Mailchimp
   - Upload campaign assets (artwork, images)
   - Create audience segments:
     - Segment 1: Global Electronic/Dance (132 contacts)
     - Segment 2: Eastern Europe (TBD)
     - Segment 3: South Korea (TBD)

4. **Campaign Schedule**
   - **Week 1**: Global electronic/dance specialists (132 contacts)
   - **Week 2-3**: Eastern European focus (pending contact research)
   - **Week 4**: South Korean stations + follow-ups
   - **Week 5-6**: Second wave follow-ups & airplay tracking

5. **Set Up Tracking Systems**
   - Configure WARM API for airplay tracking
   - Set up Gmail monitoring for replies
   - Create campaign response tracking spreadsheet

---

## 📁 Campaign Files Generated

All campaign files are located in:
`/Users/chrisschofield/workspace/active/total-audio-platform/tools/agents/campaigns/concerta/`

### Files Created

1. **find-concerta-contacts.js** - Airtable contact search script
2. **concerta-contacts.json** - Full contact database (132 contacts)
3. **concerta-liberty-agent.js** - Main campaign automation script
4. **concerta-campaign-report.json** - Detailed campaign configuration
5. **CONCERTA_CAMPAIGN_SUMMARY.md** - This document

---

## ⚠️ Outstanding Items

### Contact Research Needed

1. **Eastern European Dance Stations**
   - Target countries: Poland, Czech Republic, Hungary, Romania, Bulgaria
   - Focus on electronic/dance programming
   - Research online dance music platforms in Eastern Europe

2. **South Korean Electronic Stations**
   - Seoul electronic music radio
   - Korean dance music specialists
   - K-electronic music platforms

### Information Needed from Concerta

1. **Press Release** (check email thread)
2. **Streaming Links** (Spotify, Apple Music, etc.)
3. **Social Media Links** (Instagram, TikTok, etc.)
4. **Artist Biography** (short & long versions)
5. **Previous Coverage/Support** (if any)
6. **Preferred Pitch Angle** (confirm messaging strategy)

---

## 🎬 How to Execute the Campaign

### Option 1: Manual Mailchimp Setup

1. Log into Mailchimp
2. Create new campaign: "Concerta - Consumption"
3. Upload assets from `~/Downloads/Liberty_0831/`
4. Import contacts from `concerta-contacts.json`
5. Use email templates provided above
6. Schedule sends according to timeline

### Option 2: Automated Liberty Agent (Future)

Once press release and additional information is available:

```bash
cd /Users/chrisschofield/workspace/active/total-audio-platform/tools/agents/campaigns/concerta
node concerta-liberty-agent.js --execute
```

This will:
- Create Mailchimp campaign automatically
- Send personalised emails via Gmail API
- Track responses
- Generate weekly reports

---

## 📊 Expected Outcomes

### Conservative Estimates (Based on 132 Contacts)

- **Open Rate**: 35-45% (46-59 opens)
- **Reply Rate**: 10-15% (13-20 replies)
- **Airplay Confirmations**: 5-10% (7-13 stations)

### Optimistic Scenario (With Full Contact List)

If we expand to 300+ contacts (including Eastern Europe & South Korea):

- **Open Rate**: 35-45% (105-135 opens)
- **Reply Rate**: 10-15% (30-45 replies)
- **Airplay Confirmations**: 5-10% (15-30 stations)

---

## 🔧 Technical Details

### Campaign Scripts Location

All scripts are executable and ready to use:

```bash
# Find contacts in Airtable
node find-concerta-contacts.js

# Run full campaign setup
node concerta-liberty-agent.js

# View campaign report
cat concerta-campaign-report.json | jq
```

### Dependencies

- Node.js (installed ✅)
- Airtable API access (configured ✅)
- Mailchimp API (ready to configure)
- Gmail API (configured via Liberty setup)

---

## 📞 Next Discussion Points

1. **Press Release Review** - Can you forward the press release from the email thread?
2. **Contact Preferences** - Does Concerta prefer UK focus or broader European approach?
3. **Timeline Confirmation** - Is 15 November release date confirmed?
4. **Budget Considerations** - Any budget for additional Eastern European contact research?
5. **Asset Preferences** - Which artwork variant should be primary?

---

## ✅ Campaign Readiness Checklist

- [x] Contact database searched (132 electronic/dance contacts)
- [x] Campaign assets verified (images, artwork, video)
- [x] Email templates created (3 variants)
- [x] Campaign scripts developed and tested
- [x] Mailchimp campaign plan created
- [ ] Press release reviewed and integrated
- [ ] Eastern European contacts researched
- [ ] South Korean contacts researched
- [ ] Mailchimp campaign created
- [ ] Campaign scheduled for send

---

**Campaign Prepared By**: Liberty Music PR - Claude Agent
**Date**: 7 November 2025
**Status**: Ready for launch pending press release and contact research

---

_For questions or to proceed with campaign launch, review the email templates above and confirm next steps._
