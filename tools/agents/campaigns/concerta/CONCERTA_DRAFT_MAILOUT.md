# CONCERTA "CONSUMPTION" - RADIO MAILOUT DRAFT

**For Review This Weekend | Send Monday 11 November 2025**

---

## Email Campaign Details

**Campaign Name**: Concerta - Consumption (Electronic/Dance Radio Campaign)
**Send Date**: Monday, 11 November 2025
**Total Recipients**: 132 electronic/dance radio contacts
**From**: Liberty Music PR (chris@libertymusicpr.com)

---

## MAIN EMAIL TEMPLATE

### Subject Line

**Fresh Electronic Sound: Concerta (South Korea) - Consumption**

### Email Body

```
Hi [STATION_NAME],

I hope you're well! I wanted to introduce you to Concerta, a rising South Korean electronic artist who's bringing a unique cross-cultural perspective to dance music.

"Consumption" is out now everywhere and showcases:
• Authentic Asian electronic production
• Cinematic, futuristic sound design
• Fresh perspective on European dance music
• Stunning anime-inspired visual identity

This track represents an exciting bridge between Asian and European dance cultures - the kind of cross-cultural fusion that's pushing electronic music forward.

I think your audience would really appreciate this fresh take on electronic music.

 Listen now:
[Spotify Link]
[Apple Music Link]
[SoundCloud Link]

 High-res assets & press materials available on request

Would you like to feature "Consumption" on your show?

Best regards,
Chris Schofield
Liberty Music PR
chris@libertymusicpr.com
www.libertymusicpr.com
```

---

## Target Audience (132 Contacts)

### Confirmed Electronic/Dance Stations Include:

**Top Priority UK Stations:**

- BBC (Steve White) - Pop, Rock, Folk, Electronic
- BBC (Jada Joshi) - Alternative, Indie, Electronic, Hip-Hop
- Reducedlistening - Electronic specialist
- Radio Fandango - Alternative, Rock, Indie, Pop, Folk, Electronic
- Kingdom FM - Pop, Rock, Dance
- Somethinelse - Alternative, Rock, Indie, Pop, Electronic, Jazz/Funk
- KMFM (Liam) - Pop, Dance

**International Electronic/Dance:**

- WARM Radio (Denmark) - Rock, Metal, Dance
- "The New Music Show" - Hip-Hop, Electronic, Dance
- CJSW (Canada) - Alternative, Rock, Indie, Electronic, Hip-Hop

**Plus 122 additional electronic/dance specialists**

---

## Assets to Include/Attach

### Required Links

- [ ] Spotify streaming link
- [ ] Apple Music streaming link
- [ ] SoundCloud/Bandcamp link (if available)
- [ ] YouTube video link (if available)
- [ ] Artist socials (Instagram, TikTok, etc.)

### Available Assets (Location: ~/Downloads/Liberty_0831/)

-  4 high-res profile images
-  Anime-inspired artwork (multiple variants)
-  Futuristic city artwork
-  Music video (CONCERTA_city_03.mp4)
-  SYLQ label assets

### Recommended Mailchimp Attachments

1. **Primary artwork**: consumption_anime_artwork 04b + type.jpg
2. **Artist photo**: CONCERTA_High-res_1.jpg
3. **Press kit** (if available)

---

## Key Messaging Points

**DO Include:**

-  South Korean artist (unique angle)
-  Cross-cultural electronic fusion
-  Anime-inspired visuals (strong identity)
-  Available NOW on all platforms
-  Fresh perspective on dance music

**DON'T Include:**

-  Overly technical production details
-  Too much Seoul scene context (keep concise)
-  Comparison to other K-pop/K-electronic (let it stand alone)

---

## Send Schedule

**Monday, 11 November 2025**

- **9:00 AM**: Send to UK electronic/dance stations (primary batch ~60 contacts)
- **11:00 AM**: Send to European electronic/dance stations (~40 contacts)
- **2:00 PM**: Send to US/International electronic/dance stations (~32 contacts)

**Staggered sends help with**:

- Gmail deliverability
- Time zone optimisation
- Personal response monitoring

---

## Pre-Send Checklist

### Information Needed from Concerta:

- [ ] **Spotify link** for "Consumption"
- [ ] **Apple Music link** for "Consumption"
- [ ] **SoundCloud/Bandcamp** link (if applicable)
- [ ] **YouTube video** link (if available)
- [ ] **Instagram handle** (@concerta...?)
- [ ] **TikTok** (if active)
- [ ] **Artist bio** (2-3 sentences max)
- [ ] **Press release** (if available - check email)
- [ ] **Any press quotes/coverage** to date
- [ ] **Preferred contact method** for radio programmers

### Technical Setup:

- [ ] Mailchimp campaign created
- [ ] 132 contacts imported to Mailchimp
- [ ] Email template uploaded with images
- [ ] Test email sent to chris@libertymusicpr.com
- [ ] Links tested (all working)
- [ ] Assets uploaded to Mailchimp media library
- [ ] Send scheduled for Monday 11 Nov, 9:00 AM

---

## Expected Results

Based on 132 contact database:

**Conservative Estimates:**

- Opens: 46-59 (35-45% open rate)
- Replies: 13-20 (10-15% reply rate)
- Airplay confirmations: 7-13 stations (5-10%)

**Success Indicators:**

- 40%+ open rate = Strong subject line
- 15%+ reply rate = Compelling pitch
- 10+ airplay adds = Successful campaign

---

## Follow-Up Strategy

### Week 1 (11-15 November)

- Monitor replies daily
- Respond to all enquiries within 24 hours
- Track which stations add to playlist

### Week 2 (18-22 November)

- Send polite follow-up to non-responders
- Share any early airplay wins
- Update with any press coverage

### Week 3 (25-29 November)

- Final follow-up to key targets
- Report results to Concerta
- Plan second wave (if needed)

---

## Alternative Subject Lines (A/B Testing Options)

If we want to test different approaches:

1. **Current**: Fresh Electronic Sound: Concerta (South Korea) - Consumption
2. **Alternative 1**: New Dance Track from Seoul: Concerta - Consumption
3. **Alternative 2**: Concerta - Consumption (Cross-Cultural Electronic Fusion)
4. **Alternative 3**: South Korean Electronic Artist Concerta Drops "Consumption"

**Recommendation**: Stick with current subject - it's clear, includes key info (artist origin, genre), and has professional tone.

---

## Personalization Variables

For Mailchimp merge tags:

- `*|STATION|*` = Station name
- `*|FNAME|*` = Contact first name (if available)
- `*|GENRE|*` = Station genre focus

Example personalized opening:

```
Hi *|FNAME|* at *|STATION|*,
```

---

## Social Media Support

Suggested posts to coordinate with email send:

**Instagram Post (Monday AM)**

```
 Radio campaign for "Consumption" goes live today!

Huge thanks to @libertymusicpr for supporting this track across electronic/dance stations worldwide 

South Korea → Europe → the world 

[Link in bio]

#Consumption #ElectronicMusic #DanceMusic #RadioPromo
```

**TikTok (Monday)**

- Behind-the-scenes of radio campaign
- Show email going out to 132 stations
- "South Korean artist → European dance scene" angle

---

## Important Notes for Concerta

### What to Expect:

- **Replies start within 24-48 hours** (stations check email Mon-Fri)
- **Most responses by end of week** (Friday 15 November)
- **Airplay decisions take 1-2 weeks** (programming schedules)
- **Not all replies = immediate airplay** (some need more time)

### What We Need from You:

- **Quick response to station requests** (bios, interviews, track info)
- **Monitor streaming numbers** (stations like to see traction)
- **Share any coverage** (press, blogs, playlists help momentum)

### Communication:

- Liberty will handle all initial station contact
- You'll be CC'd on important conversations
- Weekly update reports on Friday afternoons
- Direct you to stations requesting artist contact

---

## FINAL APPROVAL NEEDED

**Please review and confirm by Sunday 10 November:**

1.  Email copy approved (or suggest changes)
2.  Subject line approved
3.  Provide all streaming links
4.  Confirm artist socials
5.  Approve Monday send date
6.  Share any additional materials (press release, etc.)

**Once approved, campaign goes live Monday 9:00 AM GMT**

---

## Additional Campaign Information

**Liberty will also:**

- Monitor all email replies and forward relevant ones
- Track which stations add to rotation
- Report back weekly with progress updates
- Handle follow-ups professionally
- Support with any station requests (interviews, track info, etc.)

**You retain:**

- Full control over artist messaging
- Right to approve/decline interview requests
- Access to all correspondence
- Weekly campaign reports

---

**Questions or changes?**
Reply to this document or email chris@libertymusicpr.com before Sunday evening.

---

**Campaign prepared by**: Liberty Music PR
**Date**: 7 November 2025
**Review deadline**: Sunday, 10 November 2025 (evening)
**Send date**: Monday, 11 November 2025 (9:00 AM GMT)
