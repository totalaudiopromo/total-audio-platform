...
class TDDImplementationPlanner {
  constructor() {
    ...
    this.existingAgents = {
      core: [
        'database-agent', 'campaign-agent', 'contact-agent',
        'analytics-agent', 'agency-agent'
      ],
      integration: [
        'integration-agent', 'integration-agent-real'
      ],
      content: [
        'content-generation-agent', 'social-media-agent',
        'radio-promo-agent', 'viral-content-automation'
      ],
      strategy: [
        'music-industry-strategist', 'music-marketing-mastermind',
  ...
  listPlans() {
    ...
    files.forEach(file => {
      try {
        ...
        const phases = plan.phases ? Object.keys(plan.phases).length : 
                      plan.quick_phases ? Object.keys(plan.quick_phases).length : 0;
        console.log(`   ${plan.feature} - ${plan.product}${type}`);
        console.log(`     Phases: ${phases}, Agents: ${Object.keys(plan.agent_assignments || plan.key_agents || {}).length}`);
      } catch (e) {
...
if (require.main === module) {
  ...
  if (args.length === 0) {
    ...
    console.log('Usage:');
    console.log('  node tdd-implementation-planner.js create "feature name" [audiointel|playlistpulse]');
    console.log('  node tdd-implementation-planner.js quick "feature name" [audiointel|playlistpulse]');
