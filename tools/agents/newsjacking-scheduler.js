...
class NewsjackingScheduler {
  ...
  async executeBreakingNewsCheck() {
    try {
      ...
      const urgentStories = trendingStories.filter(story => story.relevanceScore >= this.notifications.urgentThreshold);
  ...
  async executeUrgentTrendCheck() {
    try {
      const trendingStories = await this.newsjackingAgent.monitorTrendingTopics();
      const urgentStories = trendingStories.filter(story => 
        story.relevanceScore >= this.notifications.urgentThreshold &&
        this.isStoryFresh(story, 30) // Within last 30 minutes
      );
