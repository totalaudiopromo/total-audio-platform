...
const relevanceKeywords = {
  ...
  mediumRelevance: [
    ...
    'music discovery', 'algorithm', 'playlists', 'curator'
  ]
};
...
function identifyUnsignedAngle(story) {
  ...
  if (text.includes('major label') && (text.includes('cuts') || text.includes('problems') || text.includes('struggle') || text.includes('layoff'))) {
    return {
      ...
      angle: 'While major labels struggle with internal problems, independent artists can move faster and be more agile',
...
The opportunity here is simple: ${angle.opportunity.toLowerCase()}.`,
    ...
    audioIntelConnection: story.relevanceScore > 0.6 ? 'Audio Intel helps indies move faster on opportunities like this' : null
  };
}
