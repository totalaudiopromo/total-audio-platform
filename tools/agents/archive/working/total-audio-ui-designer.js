...
class TotalAudioUIDesigner {
  ...
  async createWireframes(featureName, productLine, requirements) {
    ...
    if (featureName.toLowerCase().includes('dashboard')) {
      ...
      wireframes.desktop.components.push(
        'Full stats dashboard',
        'Multi-chart view',
        'Control panel'
      );
    } else if (
      featureName.toLowerCase().includes('list') ||
      featureName.toLowerCase().includes('table')
    ) {
      ...
      featureName.toLowerCase().includes('form') ||
      featureName.toLowerCase().includes('create')
    ) {
  ...
  async selectComponents(featureName, requirements) {
    ...
    if (
      ...
      featureLower.includes('step') ||
      featureLower.includes('wizard')
    ) {
    ...
    try {
      switch (command) {
        ...
        case 'design':
          ...
          if (!featureName) {
            console.error(
              'Usage: node total-audio-ui-designer.js design "feature name" [audiointel|playlistpulse]'
            );
            process.exit(1);
          }
        ...
        case 'component':
          ...
          if (!componentName) {
            console.error(
              'Usage: node total-audio-ui-designer.js component "Component Name" [audiointel|playlistpulse]'
            );
            process.exit(1);
          }
          const spec = await designer.createComponentSpec(
            componentName,
            componentProductLine || 'audiointel'
          );
