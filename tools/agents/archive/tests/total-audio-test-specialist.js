...
class TotalAudioTestSpecialist {
  ...
  async generateUnitTests(featureName, productLine, options) {
    ...
    if (
      ...
      featureLower.includes('util') ||
      featureLower.includes('helper')
    ) {
      unitTests.testFiles.push({
  ...
  async generateIntegrationTests(featureName, productLine, options) {
    ...
    if (
      ...
      featureLower.includes('repository')
    ) {
      integrationTests.testFiles.push({
  ...
  async generateTestData(featureName, productLine) {
    ...
    if (productLine === 'audiointel') {
      testData.fixtures.push({
        ...
        count: 100,
        fields: ['name', 'email', 'company', 'role', 'genre', 'location', 'lastContact'],
      });
      ...
      testData.fixtures.push({
        ...
        count: 20,
        fields: ['title', 'artistName', 'genre', 'status', 'startDate', 'endDate', 'budget'],
      });
      ...
      testData.fixtures.push({
        ...
        count: 50,
        fields: ['name', 'platform', 'playlistName', 'followerCount', 'genres', 'contactInfo'],
      });
    }
    ...
    testData.fixtures.push({
      ...
      count: 5,
      fields: ['name', 'domain', 'branding', 'subscription', 'settings', 'users'],
    });
  ...
  async generateMocks(featureName, productLine) {
    return {
      apiMocks: {
        ...
        stripe: {
          endpoints: ['POST /payment_intents', 'POST /subscriptions', 'POST /webhook_endpoints'],
  ...
  async assessTestingRisks(featureName, productLine) {
    ...
    if (
      featureLower.includes('multi') ||
      featureLower.includes('tenant') ||
      featureLower.includes('agency')
    ) {
      risks.push({
    ...
    try {
      switch (command) {
        ...
        case 'generate':
          ...
          if (!featureName) {
            console.error(
              'Usage: node total-audio-test-specialist.js generate "feature name" [audiointel|playlistpulse]'
            );
            process.exit(1);
          }
          const testSuite = await testSpecialist.generateTestSuite(
            featureName,
        ...
        case 'strategy':
          ...
          if (!stratFeatureName) {
            console.error(
              'Usage: node total-audio-test-specialist.js strategy "feature name" [audiointel|playlistpulse]'
            );
            process.exit(1);
          }
          const strategy = await testSpecialist.createTestStrategy(
            stratFeatureName,
            stratProductLine || 'audiointel'
          );
        ...
        case 'config':
          ...
          if (!configFeatureName) {
            console.error(
              'Usage: node total-audio-test-specialist.js config "feature name" [audiointel|playlistpulse]'
            );
            process.exit(1);
          }
          const config = await testSpecialist.generateTestConfig(
            configFeatureName,
        ...
        default:
          ...
          console.log(
            '  node total-audio-test-specialist.js generate "contact filtering" audiointel'
          );
