  ...
  listTestSpecs() {
    ...
    files.forEach(file => {
      try {
        ...
        const testCount = spec.business_scenarios?.length || spec.essential_tests?.length || 0;
...
if (require.main === module) {
  ...
  if (args.length === 0) {
    ...
    console.log('Usage:');
    console.log(
      '  node tdd-test-writer.js write "feature name" [audiointel|playlistpulse] [standard|detailed]'
    );
