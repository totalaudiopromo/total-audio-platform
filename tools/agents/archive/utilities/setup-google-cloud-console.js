...
async function setupGoogleCloudConsole() {
  ...
  try {
    ...
    await page.goto('https://console.cloud.google.com/apis/credentials?project=gleaming-realm-471715-p3');
    ...
    await page.goto('https://console.cloud.google.com/apis/credentials/consent?project=gleaming-realm-471715-p3');
    await page.waitForLoadState('networkidle');
    
    // Look for edit button or configure button
    try {
      // Try different possible button texts
      const editButton = await page.locator('text=Edit App').or(
        page.locator('text=Configure').or(
          page.locator('text=Edit').or(
      ...
      await page.goto('https://console.cloud.google.com/apis/credentials/consent/edit?project=gleaming-realm-471715-p3');
      await page.waitForLoadState('networkidle');
    }
    ...
    const domainsSection = page.locator('text=Authorized domains').or(
      page.locator('text=Authorized domains').locator('..')
    ).first();
    
    // Add localhost domain
    try {
      const addDomainButton = page.locator('text=Add domain').or(
        page.locator('text=+').or(
          page.locator('[data-testid="add-domain"]')
        )
      ).first();
    ...
    try {
      const addDomainButton2 = page.locator('text=Add domain').or(
        page.locator('text=+').or(
          page.locator('[data-testid="add-domain"]')
        )
      ).first();
    ...
    try {
      const saveButton = page.locator('text=Save and Continue').or(
        page.locator('text=Save').or(
    ...
    const oauthClient = page.locator('text=309298460159-4gcfsvpup4og77r0mifta91s8f651875.apps.googleusercontent.com').or(
      page.locator('text=OAuth 2.0 Client ID').locator('..')
    ).first();
    ...
    for (const uri of redirectUris) {
      try {
        const addUriButton = page.locator('text=Add URI').or(
          page.locator('text=+').or(
            page.locator('[data-testid="add-uri"]')
          )
        ).first();
    ...
    for (const apiName of apisToEnable) {
      try {
        await page.goto(`https://console.cloud.google.com/apis/library/${apiName.toLowerCase().replace(/\s+/g, '-')}?project=gleaming-realm-471715-p3`);
        await page.waitForLoadState('networkidle');
        
        const enableButton = page.locator('text=Enable').or(
          page.locator('[data-testid="enable-button"]')
        ).first();
    ...
    console.log('3. Go to: https://console.cloud.google.com/apis/credentials?project=gleaming-realm-471715-p3');
