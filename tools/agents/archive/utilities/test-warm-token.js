...
async function testWarmWithToken() {
  ...
  try {
    ...
    const stationsResponse = await fetch(
      `${apiUrl}/radio-stations?countryCode=GB&isMonitored=true`,
      {
        headers: {
    ...
    console.log(
      '📻 Stations count:',
      Array.isArray(stationsList) ? stationsList.length : 'Not an array'
    );

    if (Array.isArray(stationsList) && stationsList.length > 0) {
      console.log(
        '📻 Sample stations:',
        stationsList.slice(0, 3).map(s => s.name || s.radioStationName || s.stationName || s)
      );
    }
    console.log('');
    ...
    const playCount =
      plays.totalNumberOfEntities ||
      plays.total ||
      (Array.isArray(playsList) ? playsList.length : 0);
