...
async function testLibertyIntelligence() {
  ...
  try {
    ...
    industryContacts.extractedContacts.forEach((contact, index) => {
      console.log(
        `   ${index + 1}. ${contact.name} at ${contact.station} (${contact.relationship})`
      );
    });
    ...
    console.log(
      `   Total Industry Contacts: ${intelligence.industryContacts.extractedContacts.length}`
    );
    console.log(
      `   Total Campaign Insights: ${intelligence.campaignDiscussions.extractedInsights.length}`
    );
    ...
    console.log(
      `✅ Radio Superstars: ${industryContacts.extractedContacts.length} contacts identified`
    );
