...
async function testMailchimpTraining() {
  ...
  try {
    ...
    console.log(
      `   Subject line patterns: ${Object.keys(styleData.subjectLinePatterns).length} metrics`
    );
    ...
    console.log(
      `   Liberty style analysis: ${Object.keys(styleData.libertyStyle).length} categories`
    );
    ...
    if (campaignsData.analyzedCampaigns > 0) {
      ...
      if (styleData.subjectLinePatterns.commonWords) {
        const topWords = Object.entries(styleData.subjectLinePatterns.commonWords)
          ...
          .slice(0, 5);
        console.log(
          `   Top subject line words: ${topWords.map(([word, count]) => `${word} (${count})`).join(', ')}`
        );
      }
      ...
      if (styleData.libertyStyle.commonPhrases && styleData.libertyStyle.commonPhrases.length > 0) {
        console.log(
          `   Common phrases: ${styleData.libertyStyle.commonPhrases
            .slice(0, 3)
            .map(p => p.value)
            .join(', ')}`
        );
