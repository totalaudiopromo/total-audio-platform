...
async function testWeeklyReports() {
  ...
  try {
    ...
    if (healthStatus.status === 'healthy') {
      try {
        const performanceSummary = await warmApi.generateCampaignPerformanceSummary(testArtistName, testStartDate);
        console.log('Performance Summary:', JSON.stringify(performanceSummary, null, 2));
      } catch (error) {
        console.log('⚠️ Performance summary generation failed (expected if no plays):', error.message);
      }
    } else {
    ...
    testScenarios.forEach(scenario => {
      ...
      console.log(`   ${status} ${scenario.plays} plays, ${scenario.stations} stations → ${rating} (expected: ${scenario.expected})`);
    });
    ...
    console.log('   • warmApi.generateWeeklyReport(artist, track, startDate, driveAPI, campaignData)');
    ...
    console.log('   • node radio-promo-agent.js update-weekly-report "Artist Name" "start-date" [campaign-id]');
