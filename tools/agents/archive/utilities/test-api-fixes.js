  ...
  try {
    ...
    try {
      if (agent.drive) {
        ...
        const seniorDunceFolders = await agent.drive.searchCampaignFolders('Senior Dunce', 'Bestial');
        console.log(`   ✅ Senior Dunce folder search works - found ${seniorDunceFolders ? seniorDunceFolders.length : 0} folders`);
          ...
          try {
            ...
            for (const response of responses) {
              ...
              if (responseText.includes('senior dunce')) {
                ...
                try {
                  const detailedResponse = await agent.typeformApi.getResponse(form.id, response.token);
                  ...
                  console.log(`   ❌ Individual response access still failing: ${responseError.message}`);
                }
              }
            }
