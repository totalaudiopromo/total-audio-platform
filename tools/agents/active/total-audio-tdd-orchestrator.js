...
class TotalAudioTDDOrchestrator {
  constructor() {
    ...
    // TDD Planning Agents
    this.tddAgents = {
      uiPlanner: path.join(__dirname, 'tdd', 'tdd-ui-planner.js'),
      componentSelector: path.join(__dirname, 'tdd', 'tdd-component-selector.js'),
      testWriter: path.join(__dirname, 'tdd', 'tdd-test-writer.js'),
      implementationPlanner: path.join(__dirname, 'tdd', 'tdd-implementation-planner.js')
    };
  ...
  async planFeature(featureName, productLine = 'audiointel', options = {}) {
    ...
    try {
      ...
      if (!options.skipComponents) {
        const componentSpecs = await this.runTDDAgent('componentSelector', featureName, productLine);
      ...
      this.status.plannedFeatures = Object.values(this.status.features)
        .filter(f => f.status === 'planned' || f.status === 'built').length;
  ...
  async buildFeature(featureName, options = {}) {
    ...
    try {
      ...
      if (feature.status !== 'planned') {
        logger.warn(`Feature "${featureName}" is not in planned status (current: ${feature.status})`);
      }
      ...
      try {
        ...
        this.status.builtFeatures = Object.values(this.status.features)
          .filter(f => f.status === 'built').length;
  ...
  async runTDDAgent(agentName, featureName, productLine) {
    ...
    // Map agent names to their required commands
    const agentCommands = {
      'uiPlanner': 'create',
      'componentSelector': 'select',
      'testWriter': 'write',
      'implementationPlanner': 'create'
    };

    const agentCommand = agentCommands[agentName];
    if (!agentCommand) {
      throw new Error(`No command mapping for TDD agent "${agentName}"`);
    }

    try {
      const command = `node ${agentPath} ${agentCommand} "${featureName}" "${productLine}"`;
      const result = execSync(command, { encoding: 'utf8', cwd: this.agentsDir });

      // Try to parse as JSON, but handle text output gracefully
      try {
    ...
    try {
      switch (command) {
        ...
        case 'plan':
          ...
          if (!featureName) {
            console.error('Usage: node total-audio-tdd-orchestrator.js plan "feature name" [audiointel|playlistpulse]');
            process.exit(1);
          }
        ...
        case 'quick':
          ...
          if (!quickFeatureName) {
            console.error('Usage: node total-audio-tdd-orchestrator.js quick "feature name" [audiointel|playlistpulse]');
            process.exit(1);
          }
        ...
        default:
          ...
          console.log('  build "feature"                Build planned feature with existing agents');
