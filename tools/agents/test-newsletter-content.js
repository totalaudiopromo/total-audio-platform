...
function identifyUnsignedAngle(story) {
  ...
  if (text.includes('major label') && (text.includes('cuts') || text.includes('problems'))) {
    return {
      ...
      angle:
        'While major labels struggle with budget cuts, independent artists can move faster and be more agile',
...
function generateNewsletterSection(story, angle) {
  const hook = voicePatterns.openingHooks[0].replace('{industry_development}', story.title);
...
async function main() {
  ...
  if (highRelevanceStories.length === 0) {
    ...
    console.log(
      "This is normal - most music industry news isn't directly relevant to indie artists."
    );
    console.log(
      'The system filters aggressively to only show stories with actionable indie opportunities.\n'
    );
    ...
    scoredStories.slice(0, 5).forEach((story, i) => {
      ...
      console.log(
        `   Source: ${story.source} | Published: ${story.publishedAt.toLocaleDateString()}`
      );
  ...
  highRelevanceStories.forEach((story, i) => {
    ...
    console.log(
      `   Published: ${story.publishedAt.toLocaleDateString()} ${story.publishedAt.toLocaleTimeString()}`
    );
    console.log(`   URL: ${story.url}`);
